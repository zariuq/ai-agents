#!/usr/bin/env python3
"""
Optimized PLN-native k-NN premise selector.
...
Usage:
  python3 scripts/select_pln_knn_opt.py --top-k 256 --output selections.json
  python3 scripts/select_pln_knn_opt.py --top-k 256 --merge-nb --output sel.json
  python3 scripts/select_pln_knn_opt.py --smoke-test --output /dev/null
"""
...
COMBINED_SELECTOR = "demos/pln_knn_selector_opt.metta"
...
def run_petta_batch_jobs(job_rows, timeout_sec, jobs_path):
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimized PLN-native k-NN + NB selector (batched PeTTa)")
    ...
    mode = "PLN-kNN+NB-opt" if args.merge_nb else "PLN-kNN-opt"
    ...
    for bidx, batch in enumerate(batches):
        ...
        try:
            petta_t0 = time.time()
            stv_by_job_qid = run_petta_batch_jobs(batch, args.petta_timeout, jobs_path)
