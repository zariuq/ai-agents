#!/usr/bin/env python3
"""
PLN rule premise selection (selection only, no E prover).

Uses PeTTa to execute PLN ModusPonens + Revision inference for scoring.
Python prepares TSV inputs, invokes PeTTa, parses STVs, selects top-k.

Usage:
  python3 scripts/select_pln_rule.py --top-k 256 --output selections.json
...
PETTA_SELECTOR = "demos/pln_premise_selector_jobs.metta"
...
def build_batch_files(pname, batch_idx, batch_axioms, goal_features,
                      ...
                      nb_def_prior_weight, feature_strength, work_dir):
    """Create evidence/query TSV files for one PeTTa scoring batch."""
    goal_list = sorted(goal_features)
    goal_set = set(goal_list)
    goal_csv = ",".join(str(f) for f in goal_list)

    # Optional IDF-normalized feature strengths
    idf_max = max(idf.values()) if idf else 1.0
    if idf_max <= 0.0:
        idf_max = 1.0
    if feature_strength == "idf_norm":
        strengths = [max(0.0, min(1.0, float(idf.get(f, 1.0)) / idf_max)) for f in goal_list]
        strengths_csv = ",".join(_fmt(x) for x in strengths)
    else:
        strengths_csv = None

    evidence_path = work_dir / f"rule_{pname}_b{batch_idx}_ev.tsv"
    query_path = work_dir / f"rule_{pname}_b{batch_idx}_q.tsv"
    ...
    with open(evidence_path, "w") as ef, open(query_path, "w") as qf:
        for i, phi in enumerate(batch_axioms):
            ...
            for f in goal_set & f_bar:
                ...
                ef.write(f"feat\t{phi}\t{f}\t{_fmt(sf)}\t{_fmt(neg)}\n")

            for f in goal_set - f_bar:
                ef.write(f"feat\t{phi}\t{f}\t{_fmt(0.5)}\t{_fmt(max(t_phi, 0.5))}\n")

            qid = f"q{batch_idx}_{i}"
            qid_to_axiom[qid] = phi
            if strengths_csv is not None:
                qf.write(f"{qid}\t{phi}\t{goal_csv}\t{strengths_csv}\n")
            else:
...
def score_one_problem(pname, candidate_axioms, goal_features, sfreq, tfreq,
    ...
    for bstart in range(0, len(candidate_axioms), batch_size):
        ...
        ev, q, qmap, skipped = build_batch_files(
            ...
            sfreq, tfreq, idf, extended_features,
            args.nb_def_prior_weight, args.feature_strength, work_dir,
        )
        batch_tasks.append(
            {
                "job_id": f"{pname}_rule_{batch_idx}",
                "ev": ev,
                "q": q,
                "qmap": qmap,
                "skipped": skipped,
...
def main():
    parser = argparse.ArgumentParser(description="PLN rule premise selection via PeTTa")
    ...
    parser.add_argument("--feature-strength", choices=["binary", "idf_norm"], default="binary")
    ...
    args.run_temp_root = TEMP_ROOT / f"pln_rule_{run_id}"
