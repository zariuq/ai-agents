#!/usr/bin/env python3
"""
MaSh k-NN premise selection (selection only, no E prover).

Scores all candidate axioms for each validation problem using MaSh k-NN,
selects top-k, and writes a selections JSON.

Usage:
  python3 scripts/select_mash_knn.py --top-k 256 --output selections.json
...
def main():
    ...
    for i, pname in enumerate(problems):
        ...
        scored = score_all_axioms(
            list(axiom_map.keys()), gamma_features, tables, max_suggs=args.top_k
        )
