...
Axiom sum_nat_pair : forall a b :e real, sum_nat (fun n => if n = 0 then a else if n = 1 then b else 0) = a + b.
...
claim H_rest: (forall A :e F, P A :e real /\ 0 <= P A)
            /\ (P Omega = 1
            /\ (P Empty = 0
            /\ (forall f : set -> set,
                 (forall n :e omega, f n :e F) ->
  ...
  exact andER (is_sigma_field Omega F)
              ...
              /\ (forall f : set -> set,
                   ...
                   P (bigcup_nat f) = sum_nat (fun n => P (f n))))))
              H.
claim H_rest2: P Omega = 1
            /\ (P Empty = 0
            /\ (forall f : set -> set,
                 (forall n :e omega, f n :e F) ->
                 pairwise_disjoint f ->
                 P (bigcup_nat f) = sum_nat (fun n => P (f n)))).
  exact andER (forall A :e F, P A :e real /\ 0 <= P A)
              (P Omega = 1
              /\ (P Empty = 0
              /\ (forall f : set -> set,
                   (forall n :e omega, f n :e F) ->
                   ...
                   P (bigcup_nat f) = sum_nat (fun n => P (f n)))))
              H_rest.
claim H_rest3: P Empty = 0
            /\ (forall f : set -> set,
                 (forall n :e omega, f n :e F) ->
                 pairwise_disjoint f ->
                 P (bigcup_nat f) = sum_nat (fun n => P (f n))).
  exact andER (P Omega = 1)
              (P Empty = 0
              /\ (forall f : set -> set,
                   (forall n :e omega, f n :e F) ->
                   pairwise_disjoint f ->
