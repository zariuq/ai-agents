...
fof(c_H1,axiom,p(c_Empty)).
fof(c_H2,axiom,(! [X4] : (! [X5] : (finite(X4) => (nIn(X5,X4) => (p(c_X4) => p(binunion(c_X4,c_Sing(X5))))))))).
fof(c_Hn,axiom,nat_5Fp(n)).
fof(c_IHn,axiom,(! [X4] : (equip(X4,n) => p(X4)))).
fof(c_H3,axiom,equip(c_5FX,ordsucc(n))).
fof(c_Hf1,axiom,(! [X4] : (c_In(X4,ordsucc(n)) => c_In(f(X4),c_5FX)))).
fof(c_Hf2,axiom,(! [X4] : (c_In(X4,ordsucc(n)) => (! [X5] : (c_In(X5,ordsucc(n)) => ((f(c_X4) = f(X5)) => (X4 = X5))))))).
fof(c_Hf3,axiom,(! [X4] : (c_In(X4,c_5FX) => (? [X5] : (c_In(X5,ordsucc(n)) & (f(X5) = X4)))))).
fof(conj_hammer_test_2017,conjecture,p(c_5FX)).
