...
fof(c_Hxpos,axiom,c_SNoLt(c_Empty,x)).
fof(c_Hxlow,axiom,c_Subq(c_SNoLev(x),ordsucc(c_Empty))).
fof(conj_hammer_test_4880,conjecture,(! [X1] : (c_SNo(X1) => (c_In(c_SNoLev(X1),binintersect(c_SNoLev(c_Empty),c_SNoLev(x))) => (c_SNoEq_5F(c_SNoLev(X1),X1,c_Empty) => (c_SNoEq_5F(c_SNoLev(X1),X1,x) => (c_SNoLt(c_Empty,X1) => (c_SNoLt(X1,x) => (nIn(c_SNoLev(X1),c_Empty) => (c_In(c_SNoLev(X1),x) => (x = ordsucc(c_Empty)))))))))))).
