...
fof(c_H1,axiom,(! [X2] : (c_In(X2,ordsucc(ordsucc(n))) => c_In(f(X2),ordsucc(n))))).
fof(c_H2,axiom,(! [X2] : (c_In(X2,ordsucc(ordsucc(n))) => (! [X3] : (c_In(X3,ordsucc(ordsucc(n))) => ((f(c_X2) = f(X3)) => (X2 = X3))))))).
fof(conj_hammer_test_1081,conjecture,(~ (? [X2] : (c_In(X2,ordsucc(ordsucc(n))) & (f(X2) = n))) => $false)).
