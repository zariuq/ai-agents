% R(3,4) > 8: Verify witness graph C_8(1,4) is triangle-free
%
% This file verifies that the circulant graph C_8(1,4) has no triangles.
% Expected result: Theorem (the graph is indeed triangle-free)
...
% Domain closure: only these 8 vertices exist
fof(domain, axiom, ![X]: (X=v0 | X=v1 | X=v2 | X=v3 | X=v4 | X=v5 | X=v6 | X=v7)).

% C_8(1,4) edges: i adjacent to i+1, i-1, i+4 (mod 8)
% Explicitly define adjacency for all pairs

% Edges (12 total for 3-regular on 8 vertices)
...
% Goal: The graph is triangle-free
