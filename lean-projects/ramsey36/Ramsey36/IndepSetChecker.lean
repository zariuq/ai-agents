/-
# Reflection-Based Independent Set Checker

A verified backtracking search that proves `IndepSetFree k G` by computation
with O(n) recursion depth, avoiding the O(2^n) stack depth of the naive
`Decidable` instance for `Fintype (Finset (Fin n))`.

## LLM Notes
- The search function lives in IndepSetFunc.lean (Mathlib-free) so that
  sub-problem files can import it without loading Mathlib .oleans.
- Fuel hypothesis uses `n ≤ start + fuel` (addition, not Nat subtraction) for omega.
- Pigeonhole uses `Finset.card_le_univ` on a subtype, avoiding Set.InjOn coercion.
- `Finset.mem_coe` bridges `v ∈ ↑s ↔ v ∈ s` for Set/Finset.
-/
...
theorem hasIndepSetAux_complete
  ...
  | zero =>
    -- fuel = 0 ⟹ start ≥ n ⟹ s empty ⟹ remaining = 0
    ...
    · simp only [h_rem, ↓reduceIte]
      ...
      · -- start < n
        ...
        have h_pig : ¬(n - start < remaining) := by
          intro h_lt
          -- s has `remaining` distinct elements in Fin n with val ∈ [start, n)
          -- but there are only n - start < remaining such values
        ...
        -- Branch on v = ⟨start, h_start⟩ ∈ s
        let v : Fin n := ⟨start, h_start⟩
        by_cases h_in : v ∈ s
        · -- v ∈ s: show compatible = true, then include branch works
          ...
          suffices hasIndepSetAux n adj (remaining - 1) (start + 1) (⟨start, h_start⟩ :: chosen) fuel = true by
            simp [this]
          exact h_inc
        · -- v ∉ s: all elements ≥ start + 1, skip branch works
...
theorem cliqueFree_of_checker_false
  ...
  -- A clique means all distinct pairs are adjacent: decide (G.Adj v w) = true
  -- So !(decide (G.Adj v w)) = false for all distinct pairs
...
theorem hasIndepSetAux_false_of_pig
    {n : ℕ} {adj : Fin n → Fin n → Bool}
    {remaining start : ℕ} {chosen : List (Fin n)} {fuel : ℕ}
