...
motifs, \code{Fault} is the latent disease hypothesis and the
\code{F\ldots} variables are benchmark-coded pathology findings from the
network file.
\par
\begin{table}[ht]
\centering
...
\codepath{Mettapedia/Logic/ProbHOL.lean}.  It re-exports this
logical-induction-ready layer under a shorter name, while still explicitly
avoiding the stronger claim that the repository already contains the full fused
theory of probability measures over Henkin models queried by HOL formulas.
...
\downarrow\scriptstyle{\text{static HOL}\leftrightarrow\text{WM lens}} \\
\text{static WM evidence/strength}
\end{array}
&
\begin{array}{c}
\textbf{belief side} \\
...
\item static WM evidence/strength for HOL queries, and
\item dynamic belief-process scaffolding for future probabilistic HOL.
\end{enumerate}
...
hard-coded prover or a single static model measure, but it does \emph{not} yet
claim a full logical-inductor construction or the full fused layer of
probability measures over Henkin models.
