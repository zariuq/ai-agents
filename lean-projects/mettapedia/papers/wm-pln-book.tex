...
\end{tabular}
\renewcommand{\arraystretch}{1}
\caption{Compact topology-conditioned higher-order split in the widened benchmark sweep.  One-shot broad-query winners are often anchored regime-mixture actions, while refined-query winners still mostly come from explicit context completion or exact fallback.}
...
\end{tabular}
\renewcommand{\arraystretch}{1}
\caption{Stabilized higher-order benchmark takeaways after explicit regime-mixture continuation, PLN-style multi-support revision, reveal-as-value-of-information evaluation, and higher-order chain composition.  The two numbers in the multi-support and VOI rows correspond to the hub-dominated and loopy-dense Pathfinder slices respectively.}
\label{tab:stabilized-higher-order-benchmark}
\end{table}

To keep the setup fully auditable, we make the protocol explicit here.
The pipeline is:
\code{scripts/wm\_pln\_pathfinder\_context\_regimes.py}
$\rightarrow$
\code{scripts/wm\_pln\_second\_order\_admissibility.py}
$\rightarrow$
\code{scripts/wm\_pln\_third\_order\_trust.py}
$\rightarrow$
\code{scripts/wm\_pln\_regime\_mixture\_eval.py}
$\rightarrow$
\code{scripts/wm\_pln\_reveal\_voi\_eval.py}
$\rightarrow$
\code{scripts/wm\_pln\_higher\_order\_chain\_composition.py}.
The corresponding stable artifacts are written under
\codepath{results/second\_order\_admissibility/}, including
\code{pathfinder\_context\_regimes.csv},
\code{second\_order\_admissibility.csv},
\code{third\_order\_trust\_summary.csv},
\code{regime\_mixture\_eval\_winners.csv},
\code{reveal\_voi\_eval\_summary.csv},
\code{multi\_support\_revision\_summary.csv}, and
\code{higher\_order\_chain\_composition\_summary.csv}.
\par
The one-shot continuation policy set in this pass is
\code{force\_rule\_only\_continue},
\code{force\_soft\_continue},
\code{marginalization\_aware\_soft\_continue},
\code{explicit\_regime\_mixture\_continue},
\code{force\_reveal\_context},
\code{force\_local\_exact\_fallback}, and
\code{adaptive\_topology\_broad\_planner}.
The full-chain policy set is
\code{exact\_only\_chain},
\code{rule\_only\_chain},
\code{soft\_chain},
\code{explicit\_regime\_mixture\_chain},
\code{reveal\_then\_exact},
\code{fallback\_chain}, and
\code{adaptive\_chain}.
\par
...
\]
with $\lambda_{\text{reveal}}=0.12$ for broad-query utility,
$0.04$ for refined-query fidelity, and $0.08$ for the balanced
objective.  Action-cost parameters are
\[
\begin{aligned}
...
The widened Pathfinder suite in this pass is $16$ path families
($4$ anchors + $12$ discovered) and $77$ regime rows
($16$ context-missing, $16$ hub-only, $16$ alt-support-only,
$16$ oracle-context-complete, $13$ context-pair).  Importantly, those
$12$ discovered paths come from a deterministic curated shortlist in
\code{wm\_pln\_pathfinder\_context\_regimes.py}; this is substantially
wider than the original four-path slice, but it is still not an
exhaustive Pathfinder crawl.
...
$55.2\%$ of rows are context-missing cases where contraction is blocked.
...
\item \textbf{MLN bridge (finite-support countable fragment):} the
  world-model infrastructure now also subsumes an infinite-first MLN
  semantics, specialized under locality/finite-support witnesses:
  \[
    \mathrm{queryStrength}(\{\mathrm{compiledMassSemantics}(M, h_s)\}, q)
      = \mathrm{queryProb}_{M}(q)
  \]
  where the equality factors through a finite restriction theorem and an
  exact factor-graph compilation.  This is a semantic import theorem for
  the countable finite-support fragment, \emph{not} a claim that MLN KB
  union is identical to WM additive revision.
...
The Lean development now contains an \emph{infinite-first} MLN import
layer.  The formal route is:
\begin{enumerate}
\item abstract mass semantics for MLN-style world weights and query mass;
\item countable semantics via $\sum$ over worlds;
\item a finite-support restriction theorem under locality witnesses;
\item exact specialization of that finite restriction to a factor graph;
\item transfer of the compiled factor graph into a singleton WM state.
\end{enumerate}

The terminal theorem says that if a countable MLN semantics $M$ admits a
finite-support witness $h_s$, then the compiled singleton world-model
state satisfies
\[
  \mathrm{queryStrength}(\{\mathrm{compiledMassSemantics}(M, h_s)\}, q)
    = \mathrm{queryProb}_{M}(q).
\]
So the finite factor graph is not the foundation; it is the first exact
computable shadow of the general semantics.

This result is intentionally scoped.  It covers the countable,
finite-support fragment needed for a precise subsumption theorem.  It
does \emph{not} yet claim:
\begin{itemize}
\item lifted MLN inference;
\item approximate inference;
\item open-universe MLNs;
\item or that MLN knowledge-base union is the same operation as additive
  WM revision.
\end{itemize}
