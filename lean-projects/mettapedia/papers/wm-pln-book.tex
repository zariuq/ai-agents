...
leaderboard claim.  A simple support-layer hybrid that combines the
Kalman score with the official Windowed Gaussian score also fails to
recover the corpus benchmark (raw sums $-43.1$ on the standard profile
and $-77.2$ on the low-FP profile), so the stronger empirical stories in
this chapter remain the Steel selective-prediction results and the Gas
drift/replay results.

For NAB, the most defensible \emph{positive} WM role is governance and
auditability rather than leaderboard detection score.  Concretely: keep a
strong base detector fixed, and use WM order-cost checks as an
audit/control layer.  The theorem-backed metrics are then:
\begin{enumerate}
\item certificate pass/fail rate from
...
\item downstream abstain/retrain decisions tied to certified budget
  violations near anomaly windows.
\end{enumerate}
This yields an auditable \emph{when-to-trust / when-to-escalate} signal,
which is a more realistic NAB contribution for WM-PLN than claiming a new
...
Higher-order extensional inference in PLN deals with relationships between
predicates and concepts, not just between atoms.  The key question: can
higher-order relationships be reduced to first-order ones without loss?

\section{Satisfying Sets}

The bridge between higher-order and first-order is the \emph{satisfying set}
construction.
...
$\mu : U \to \mathbb{R}_{\ge 0}$, the inheritance (conditional probability
proxy) is:
\[
  \mathrm{Inheritance}(A, B, \mu) =
  \frac{\mathrm{weakness}(A \cap B, \mu)}{\mathrm{weakness}(A, \mu)}
  = \frac{\sum_{u,v \in A \cap B} \mu(u) \cdot \mu(v)}
         {\sum_{u,v \in A} \mu(u) \cdot \mu(v)}
\]
When $\mu$ is uniform, this reduces to
$|A \cap B|^2 / |A|^2 = (|A \cap B| / |A|)^2$, so inheritance tracks the
square of the conditional probability under the squared-weakness convention.
\end{definition}

\begin{definition}[Similarity]
The Jaccard-style similarity is:
\[
  \mathrm{Similarity}(A, B, \mu) =
  \frac{\mathrm{weakness}(A \cap B, \mu)}{\mathrm{weakness}(A \cup B, \mu)}
\]
\end{definition}

\section{What Reduces Cleanly}

\begin{theorem}[Pointwise implication collapse (\claimConditional{})]
When $\forall x,\; R_1(A, x) \Rightarrow R_2(B, x)$ holds pointwise,
the weakness numerators align:
\[
  \{(u,v) \mid R_1(A,u) \wedge R_1(A,v) \wedge R_2(B,u) \wedge R_2(B,v)\}
  = \{(u,v) \mid R_1(A,u) \wedge R_1(A,v)\}
\]
and therefore $\mathrm{Inheritance}(A, B) = 1$.
\end{theorem}

\begin{theorem}[Perfect inheritance corner (\claimConditional{})]
When $\mathrm{weakness}(S).{\mathrm{pos}} \ne 0$,
$\mathrm{weakness}(S).{\mathrm{pos}} \ne \top$, and
$\mathrm{weakness}(S).{\mathrm{neg}} = 0$, then
$\mathrm{Inheritance}(A, B) = \mathrm{pTrue}$ (the top corner of
the evidence lattice).
\end{theorem}

\section{What Fails: Unconditional Equalities}

Some original PLN formulas for higher-order inference are stated as
unconditional equalities.  The formalization shows that several of these
fail without additional assumptions:
\begin{itemize}
\item The composition rule
  $\mathrm{Inh}(A, C) = f(\mathrm{Inh}(A,B), \mathrm{Inh}(B,C))$
  requires screening-off conditions analogous to those in the deduction
  rule: $P(C \mid A, B) = P(C \mid B)$.
\item Without these conditions, counterexamples exist where the formula
  gives incorrect results due to untracked correlations between
  $A$, $B$, $C$.
\item The corrected versions carry explicit side conditions: positivity
  ($P(B) \in (0,1)$), screening-off, and finite-model restrictions.
\end{itemize}

See \codepath{Mettapedia/Logic/HigherOrder/HigherOrderReduction.lean}
and \codepath{Mettapedia/Logic/HigherOrder/Basic.lean}.

\section{Kyburg Flattening}

The Kyburg flattening operation reduces higher-order distributional
truth values (distributions over distributions) to first-order ones.
The key results:
\begin{itemize}
\item \code{kyburg\_flattening}: the flattening preserves expectation.
\item \code{kyburg\_no\_advantage}: under the flattening, higher-order
  distributional reasoning offers no advantage over first-order reasoning
  for expectation-based decisions.
\item The de~Finetti singleton bridge connects the distributional layer
  to the Kyburg flattening.
\end{itemize}

See \codepath{Mettapedia/Logic/HigherOrder/PLNKyburgReduction.lean}.
