...
\emph{matcher bridge}: relating \CeTTa{}'s one-way matcher on ground
scrutinees to the official \code{matchAtoms}/\code{mergeBindings}
semantics.  Its keystone is a singleton-seed factorization
theorem---any official match run seeded with bindings $b$ factors
through the empty-seeded run followed by one deterministic merge back
into $b$---proven via an extensional lookup-specification of ground
merging under a key-uniqueness invariant, together with its
sorry-free instantiation into a full success bridge.  The final link
between the declarative judgment and the running evaluator's
\code{unify} is stated \emph{once} as a named two-field interface
(match-hit and match-miss stabilization); every sugar-rule
certification spends that interface explicitly, and discharging it is
a theorem in progress rather than a hidden assumption.
