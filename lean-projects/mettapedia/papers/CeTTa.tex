...
MORK-bridge build), backed by a 333-case rule-indexed conformance
catalog against upstream \HE{}~v0.2.10 covering 24/24 specification
rule capacities, and achieves \textbf{9.5$\times$ speedup over
...
pipeline proven in Lean, and the first frontier rule classes (grounded
dispatch, argument congruence) are welded to a reflectable LangDef
rule pack whose rule-removal tests prove the engine genuinely depends
on the declared rules.
...
\item A reflective modal LTS surface over the evaluator's one-step
  reduction relation---successor frontier, $\Box$/$\Diamond$,
  normal-form tests---with runtime De~Morgan self-tests mirroring the
  Galois connection proven in the Lean OSLF framework.
\item The first specification weld: a reflectable frontier rule pack
  gating grounded dispatch and argument congruence, with
  anti-decorative rule-removal tests.  This converts a cross-branch
  frontier soundness bug (missing argument congruence) into a
  structurally prevented class.
...
fail-by-default: \code{make test-langdef-pack} disables individual
frontier-pack rules and \emph{requires} the witnesses to fail
...
Two bug classes were found and fixed, both instructive.
...
\paragraph{The frontier congruence bug.}  The reflective one-step
frontier reported \code{(+ 1 (+ 2 3))} as a \emph{normal form}: the
...
\code{lts:he} (bound to the \HE{} evaluator's frontier) reflect the
one-step reduction relation into MeTTa as data:

\begin{itemize}
\item \code{lts:he:transitions} emits the one-step successor frontier
...
\subsection{The Frontier Rule Pack}
...
the proof reference).  \emph{HEFrontier} is the coarse user-visible
...
pack encodes HEFrontier; its relationship to the fine machine is a
...
\code{CETTA\_LANGDEF\_DISABLED\_RULES} environment variable; and an
honest claim level (currently \emph{bag-tested-adequate}: tested
against the legacy frontier and oracle bags, not yet Lean-proven).
Two rules are live---\code{HEF\_GroundedDispatch} and
\code{HEF\_LeftmostExprCongruence}---and gate the only implementations
of those rule classes in the one-step subsystem; quote/return and
empty/error quiescence are declared as structural metadata pending
migration.  The pack is reflected into MeTTa by
\code{lts:he:pack-info}.

The load-bearing test is \emph{anti-decorative}: a dedicated lane
re-runs the witness suite with each rule disabled and \emph{requires}
the witnesses to fail (the frontier must go empty; normal-form must
flip).  If the witnesses still passed with a rule removed, a duplicate
hidden code path would exist---precisely the disease that produced the
congruence bug.  The lane fails closed in both directions and runs on
both the default and MORK-bridge builds.

The roadmap is a deliberate hybrid: the pack-as-data drives the
(non-hot) frontier and reflection surfaces now; a
\code{--compile-langdef} backend---extending the existing
\code{--compile-stdlib} blob precedent---will later emit \emph{both}
the reflectable pack and specialized C for the hot evaluator under a
single shared source digest, so performance never costs the semantic
object.  An evaluate-by-iterating-the-frontier mode is planned purely
as a reference oracle and benchmark baseline, not as the production
evaluator.
...
\subsection{\RhoCalc{} as a Reaction Machine}

\RhoCalc{} adds a fourth pressure point to the architecture: not a new storage
backend, and not merely another syntax over atom-space evaluation, but an
active process language whose basic unit of work is a \emph{reaction}.  The
current \CeTTa{} branch exposes it as \code{--lang rhocalc}, with both a
Rholang-like \code{.rho} surface and a MeTTa-style \code{.mrho} surface.  The
\code{core} profile is treated as an explicit restriction, not as the identity
of the language.

The executable kernel currently supports the small asynchronous spine:
\[
  0,\quad P \mid Q,\quad x!(v),\quad
  \mathsf{for}(p \leftarrow x)\{P\},\quad
  \mathsf{new}\;x\;\mathsf{in}\;P,\quad
  @P,\quad *x
\]
together with joins and persistent receives/contracts.  Operationally, this is
implemented by a \code{RhoMachine} object rather than by scattering rho-specific
state through the ordinary evaluator.  A machine slice carries its run budget,
fresh-name counter, last run result, and reusable reaction-index capacity.

The important implementation distinction is between:
\begin{itemize}
\item \textbf{semantic freshness}: internally allocated rho fresh names are
  opaque grounded identities, printed as \code{rho:fresh:N} only for
  deterministic tests and human inspection; and
\item \textbf{surface spellings}: a user-written symbol such as
  \code{rho:fresh:1} is not a capability and does not communicate with the
  internally allocated name that happens to print the same way.
\end{itemize}

Quoted process names are similarly treated as names with an opaque boundary.
Substitution and pattern matching may compare quoted names as whole names, but
they must not casually bind through the quoted process body.  This is why the
fast send index only indexes directly comparable names such as free symbols and
opaque fresh identities; quoted process names remain on the fallback path where
alpha-normalized process equality is used.

\paragraph{Indexed reactions.}
The first reaction engine is still single-threaded, but no longer treats every
step as an undifferentiated scan.  Each slice builds a live index of sends and
reaction sites over the current normalized process, reusing the underlying
buffers across steps.  A receive or join first consults the indexed sends for
direct channels and falls back to a full scan only for quoted or otherwise
non-direct names.  Persistent receives and joins reuse the same machinery while
leaving their receive process in the residual process.

This design deliberately separates \emph{persistent capacity} from
\emph{stale pointers}: the index is rebuilt against the current process after a
reaction, but its allocated storage is retained.  A later refinement can add
stable component identities and delta updates, but the present design already
removes repeated allocation and gives the runtime an explicit place to hang
slice/resume and future parallel reaction scheduling.

\paragraph{Compatibility benchmark ladder.}
\CeTTa{} compares its \code{.rho} and lowered \code{.mrho} execution against
the local \code{f1r3node} \code{rholang-cli}.  The benchmark script runs three
stages: direct \code{.rho}, translated \code{.mrho}, and external
\code{rholang-cli}.  Ratios are reported only when both engines actually
complete; if the external engine panics or times out, the comparison is marked
\code{NA} rather than smuggling a failed run into a speed ratio.
...
join-pairs & 64 & 0.10\,s & fail & external \code{TryFromIntError} \\
\bottomrule
\end{tabular}
\end{center}

The persistent-server case is especially informative: it exercises the
contract/persistent-receive path without requiring blockchain machinery,
networking, or generalized Rholang effects.  It is therefore a good early
benchmark for whether the reaction engine handles reusable receive capacity
without collapsing into materialized intermediate lists.

\paragraph{Relation to the rest of \CeTTa{}.}
\RhoCalc{} should teach the shared runtime about channels, sources, sinks,
cancellation, and reaction scheduling, but it should not force every MeTTa
operation into process-calculus overhead.  The intended direction is to extract
the reusable substrate---budgets, machine slices, indexed reaction state,
cancellation tokens, and eventually worker pools---while keeping \HE{},
\PeTTa{}, and \RhoCalc{} free to choose different surface semantics.
...
143\,s), while maintaining oracle-verified correctness across a
375-file regression suite and a 333-case rule-indexed conformance
catalog (24/24 spec-rule capacities, all agree) against upstream
\HE{}~v0.2.10.
...
frontier rule pack makes the first reduction-rule classes named,
...
\begin{itemize}
\item \textbf{Widen the frontier rule pack:} migrate equation match,
  quote/return and empty/error quiescence into the pack, one rule
  class at a time, each with bag-equality and rule-removal gates.
\item \textbf{\code{--compile-langdef} codegen:} a generator emitting
  ...
  evaluate-by-frontier-iteration mode as reference oracle.
\item \textbf{Lean coarse frontier:} an \code{HEFrontierStep} relation
  in Lean with the OSLF span instantiated over it, and (separately) the
  delimited collapse/simulation theorems connecting it to the fine
