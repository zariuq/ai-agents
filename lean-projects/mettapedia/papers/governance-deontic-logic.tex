...
% \GSLT macro removed — GSLT section deleted (not relevant to governance)
...
\and Claude Code (Opus 4.6)\footnote{Anthropic AI assistant. Co-authored under human direction.}}
...
verdict pipeline (§\ref{sec:govlayer}); and an abstract \code{ModalQueryEncoder}
unifying event-calculus and deontic query dispatch (§\ref{sec:mqe}).
...
that what really exists actually holds.  In the Lean formalization,
...
the lease clause:
\begin{lstlisting}
StatementJudgment "lease_clause_3" eventualityJudgment
\end{lstlisting}
...
Our formalization verifies several algebraic properties (\DTS{}
interdefinability, evidence monotonicity) that correspond to structures
used in their implementation.  No formal refinement theorem connecting
the MeTTa rules to the Lean semantics has been established.
...
\paragraph{Contract clause conflict detection.}
Recent legal AI systems \cite{NormAI} use embedding-based similarity to
detect conflicting clauses across contracts and regulatory documents.
These systems achieve high recall but lack formal guarantees.
A hybrid approach would use NLP to extract candidate norm pairs,
then apply \DDL{} conflict detection (via \code{sem\_5a}: no obligation
can require the impossible, and \code{sem\_5c}: consistent obligations
are conjoinable) to formally verify whether detected conflicts are genuine.
...
\item \textbf{No executable refinement proof.}
      The MeTTa \code{governance-reasoning-engine} and the Lean semantics
      encode corresponding structures, but no formal refinement theorem
      connects them.  The Lean formalization verifies properties that the
      MeTTa code asserts, but does not prove the MeTTa code correct.
