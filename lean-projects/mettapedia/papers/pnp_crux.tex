...
Crux-First and Steelman Lean Notes on the Goertzel Weakness Program}
...
\begin{abstract}
This note records the current Lean work around Ben Goertzel's claimed proof of
...
The proof strategy under review is the October 2025 Goertzel manuscript
...
The current Lean answer to all eighteen is ``yes.''  Therefore the published proof cannot be
...
This is the correct positive abstraction: if Ben's switching argument is salvageable, it must
...
That gap is exactly where the Goertzel program remains vulnerable.
...
\subsection{Application to the Goertzel symmetrization wrapper}
...
The most natural council reply to Obstruction IV is:
\begin{quote}
...
This matters because it removes a very common escape hatch.  The council can no longer say:
\begin{quote}
...
The next natural council reply is:
\begin{quote}
...
So the council can no longer say:
\begin{quote}
...
Another creative council move is:
\begin{quote}
...
The next and now very natural council reply is:
\begin{quote}
...
At this point the natural council pivot is:
\begin{quote}
...
This is close to the form needed to challenge the route in Ben's own terms.  To save the current
...
Another natural council reply is:
\begin{quote}
...
rule on \(O(\log m)\) visible bits.  So if Ben wants a true small subclass theorem, it must use
...
This does not yet show that the Goertzel ensemble realizes the full transducer class.  What it
...
function space on the visible inputs.  So if Ben wants a genuine tree-message theorem, it must use
...
The current Lean modules do \emph{not} prove that the full Goertzel argument fails.  In particular,
...
The next natural formal target is the paper's implicit compression claim:
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Therefore the Goertzel program needs a separate theorem proving exceptional compressibility
