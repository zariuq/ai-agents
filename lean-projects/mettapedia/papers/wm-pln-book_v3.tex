...
\newcommand{\wmplnincludegraphics}[2][]{%
  \IfFileExists{#2}{%
    \includegraphics[#1]{#2}%
  }{%
    \fbox{%
      \parbox[c][0.22\textheight][c]{0.85\linewidth}{%
        \centering
        \footnotesize Missing external figure\\
        \texttt{\detokenize{#2}}%
      }%
    }%
  }%
...
\textbf{Markov extension (April 2026):} The Diaconis--Freedman (1980)
theorem---extending de~Finetti from i.i.d.\ to Markov-exchangeable
sequences---is now fully formalized in
\codepath{Mettapedia/Logic/MarkovDeFinetti*.lean} (28~files,
${\sim}$23{,}900~lines, 0~sorry).  Under Markov exchangeability, the
transition-count matrix $N(i,j)$ is the sufficient statistic,
extending the chain: Markov exchangeability $\to$ Markov mixture
$\to$ Dirichlet-per-row conjugacy $\to$ transition-count sufficiency.
}%
\footnote{Open program: the terminal
\code{WorldModel (Measure $\Omega$) Query} instance remains future
work.}

\subsection{Markov Transition World Models}

The Markov de~Finetti formalization provides a measure-theoretic foundation.
The WM-PLN bridge exposes three layers:

\paragraph{Layer 1: Single-step transition queries.}
The \code{markovDirichletQueryCarrier} (\codepath{PLNXiCarrierScreening.lean:483})
extracts row-conditioned categorical evidence from transition multisets.
Theorem: \code{markovTransitionAtom\_semE\_eq\_rowProjection}.

\paragraph{Layer 2: Multi-step predictive chaining.}
The \code{markovWMPosteriorChain} (\codepath{MarkovPredictiveChaining.lean:32})
computes $n$-step arrival probabilities via Chapman-Kolmogorov summation.

\paragraph{Layer 3: Path atom surface.}
The \code{markov-transition*} atom family (\codepath{MarkovPathXi.lean:43})
provides direct OSLF semantics for multi-step path queries with concrete
binary examples (\code{bit001}, \code{bit011}).

\paragraph{Key negative result.} PLN's additive deduction screening is
\emph{unfillable} for Markov carriers. Transition composition requires
matrix multiplication, not evidence addition. This validates the design
choice of separate predictive surfaces over retrofitting additive carriers.

\subsection{Controlled Hidden Markov Models and Credal Planning}

The same lesson becomes sharper under partial observability.  A
controlled finite hidden Markov model (HMM) adds an action-conditioned
latent transition kernel and an observation kernel:
\[
  \theta = (\pi_0,\; T(a,x,x'),\; E(x,y)).
\]
Completed action--observation cycles induce a history-level filtering
state, smoothing state, and observed trace law.  This gives a
structured finite POMDP substrate inside the world-model calculus: the
latent dynamics remain finite-state, but the agent only sees the
observations.

Three theorem families now matter operationally.

\paragraph{Observed-only inference.}
The controlled HMM surface provides exact filtering and smoothing
posteriors for completed action--observation traces, together with
their normalization theorems.  This is the first honest
observation-only belief-state layer for the finite controlled setting.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/ControlledFiniteHiddenMarkovObservedInference.lean}.}

\paragraph{Credal WM semantics.}
Observation-only additive evidence cannot in general determine hidden
belief states.  The formalization proves a concrete obstruction theorem
for observation-only carriers and replaces the impossible additive
surface with a \emph{credal} one: lower and upper latent-state
strengths are packaged as an indefinite truth value, and these bounds
are propagated into lower and upper finite-horizon decision values.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/WMControlledFiniteHiddenMarkovCredal.lean}
and packaged for downstream use in
\codepath{Mettapedia/UniversalAI/ControlledFiniteHiddenMarkovExports.lean}.}

\paragraph{BayesianAgents planning.}
Each controlled finite HMM is turned into a concrete
\code{BayesianAgents.Environment}.  The planning examples prove
nontrivial \code{qValue}, \code{optimalValue}, and
\code{optimalAction} facts, culminating in an ambiguity-sensitive
family where the recursive credal interval for \code{optimalValue} is
strictly wider than a point and no single observation-history-based
action is optimal across the whole family.  This is the decision-theoretic
teeth of the non-identifiability result.%
\footnote{Formalized in
\codepath{Mettapedia/UniversalAI/ControlledFiniteHiddenMarkovBridge.lean}
and
\codepath{Mettapedia/UniversalAI/ControlledFiniteHiddenMarkovPlanningExamples.lean}.}

\paragraph{Controlled Solomonoff leg.}
The universal-prediction triangle now has a concrete controlled leg at
the theorem level.  The formalization introduces
\code{ControlledSemimeasure} and \code{ControlledPrefixMeasure}, then
lifts the ordinary observation-alphabet Solomonoff mixture into the
controlled setting by ignoring the action side-information.  Along any
fixed action stream, the conditioned controlled predictor is exactly
the ordinary finite-alphabet Solomonoff semimeasure~\code{M2}; hence
the standard finite-horizon log-loss regret bound applies to any
controlled prefix law whose conditioned observation process is lower
semicomputable.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/UniversalPrediction/FiniteAlphabet/ControlledPrefixMeasure.lean},
\codepath{Mettapedia/Logic/UniversalPrediction/FiniteAlphabet/ControlledSolomonoffBridge.lean},
and
\codepath{Mettapedia/Logic/ControlledFiniteHiddenMarkovUniversalPredictionBridge.lean}.}

\medskip\noindent
This controlled Solomonoff result is intentionally conservative.
\emph{Positive example:} it gives a real controlled regret theorem now,
compatible with the existing finite-alphabet Solomonoff machinery.
\emph{Negative example:} it is not yet a genuinely action-aware
universal prior.  The lower-semicomputability hypothesis remains a
genuine domain condition, because the current controlled-HMM parameter
record allows arbitrary probability measures rather than computable
ones.
...
\noindent
No single view suffices to reconstruct the original evidence.  The
pair $(s,c)$ with a fixed evidence scale $k$ can recover the two
binary counts in the idealized scalar model, but it still does not
recover provenance, overlap/stamps, context, or the richer posterior
shape carried by larger world-model states.  Views are useful for
display, thresholding, and action.  They are not the semantics.

The choice $c=n/(n+k)$ is therefore not forced merely by the
two-count reconstruction problem.  The formal coordinate theorem in
\codepath{Mettapedia/Logic/PLNConfidenceWeight.lean} shows that any
confidence coordinate with a known left inverse on nonnegative evidence
weights can reconstruct the binary counts from $(s,c)$.  A qualitatively
different ``reserve-half'' coordinate, $c=n/(2n+k)$, is proved to reconstruct
the same counts while keeping all displayed confidence below $1/2$.
What the usual PLN/NARS formula adds is an extra interpretation:
confidence odds are linear in evidence weight,
$c/(1-c)=n/k$.
...
\end{tikzpicture}
\caption{Individual views of binary evidence are lossy projections.  Each carrier
has its own views (Dirichlet: per-category means; Normal-Gamma:
posterior mean and variance).  A fixed-$k$ strength--confidence pair
can recover binary counts in the idealized scalar model, but no scalar
view reconstructs provenance, overlap, context, or richer posterior
state.}
...
$c = n / (n + k)$ is the confidence.  It is compact and intuitive.
With fixed $k$ and ideal real arithmetic, it is equivalent to the two
binary count coordinates $(n^+,n^-)$.  It is still lossy as a
world-model view: it discards provenance, overlap/stamps, context, and
posterior shape beyond the two-count summary.%
\footnote{Formalized in
...
$(1, 4)$.  The confidence coordinate can recover the total observation
count here because $k$ is fixed and the numbers are exact, but the STV
still does not reveal which observations, stamps, or provenance
contributed---that information lives in the evidence, not the view.%
\footnote{Proved in
\codepath{artifacts/conformance/maple\_court\_bn\_motifs.metta}
(leak: strength $= 0.2$; fork: strength $\approx 0.833$).}}

\section{Degrees of Freedom Versus Forcing}
\label{sec:dof-forcing-views}

The formal theory now separates three questions that are often
conflated when one writes only $\langle s,c\rangle$:

\begin{enumerate}
\item What is the underlying evidence state?
\item Which view coordinate is being displayed?
\item Which extra law, if any, makes that coordinate canonical?
\end{enumerate}

\noindent
For binary counts with positive total evidence, the state decomposes
exactly as a cone point:
\[
(n^+,n^-)
\;\cong\;
\bigl(s,\; n\bigr)
\qquad
s=\frac{n^+}{n^+ + n^-},\quad n=n^+ + n^- .
\]
This is the precise ``polar-coordinate'' theorem for binary evidence:
strength is the simplex direction, and total evidence is the scale.%
\footnote{Formalized as
\code{positiveBinaryCountsEquivSimplexScale} and packaged in
\code{MeanConcentrationProfile}.}

Confidence is different.  It is not the direction; it is a chart on the
scale axis.  Reconstruction alone says only that the chart must decode
back to total evidence.  The family of valid confidence charts therefore
has torsorial freedom: differences between charts are meaningful
reparametrizations, but no chart is selected until an extra law is named.
The usual PLN chart becomes distinguished by two independent forcing
laws:

\begin{itemize}
\item \textbf{Canonical confidence-odds additivity:}
  if $c/(1-c)$ is additive under evidence-weight revision, with the
  stated nonnegative-domain regularity and nonsingularity gates, then
  $c=n/(n+k)$.
\item \textbf{Walley width complement:}
  for binary IDM predictive intervals, imposing
  credibility $=1-\mathrm{width}$ gives
  $c = 1 - s/(n+s)=n/(n+s)$.
\end{itemize}

\noindent
The two routes name the same horizon parameter: the confidence scale
$k$ is the concentration or IDM strength parameter.  Other charts are
not fake.  Exponential/noisy-OR, tanh/Einstein, arctan, and reserve-half
coordinates are proved valid reconstruction charts when supplied with
their own transported revision laws.  They simply do not satisfy the
standard confidence-odds additivity law.

\begin{center}
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{@{}p{0.20\textwidth}p{0.29\textwidth}p{0.34\textwidth}@{}}
\toprule
\textbf{Coordinate} & \textbf{Free before laws} & \textbf{Forced by extra law} \\
\midrule
Strength $s$ &
Prior mean may shrink the empirical direction &
Evidence direction $n^+/(n^++n^-)$; revision is weighted mixture \\
Confidence $c$ &
Any reconstructive chart on total evidence weight &
PLN $n/(n+k)$ under canonical-odds additivity or Walley width complement \\
ITV strength view &
Selector choice: lower, midpoint, or upper &
Midpoint is bounded and ordered once selected; Walley fixes width/credibility \\
Phase &
Absent from ordinary PLN views &
Only visible in an amplitude extension with interference semantics \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tikzpicture}[
  >=Stealth,
  every node/.style={font=\small},
  state/.style={draw, rounded corners=3pt, fill=blue!8,
    align=center, minimum width=33mm, minimum height=11mm},
  view/.style={draw, rounded corners=3pt, fill=green!8,
    align=center, minimum width=31mm, minimum height=11mm},
  law/.style={draw, rounded corners=3pt, fill=orange!10,
    align=center, minimum width=34mm, minimum height=11mm},
  boundary/.style={draw, dashed, rounded corners=3pt, fill=purple!7,
    align=center, minimum width=35mm, minimum height=11mm},
]
\node[state] (counts) {Binary evidence\\$(n^+,n^-)$};
\node[view, right=20mm of counts] (polar) {Simplex-scale\\$(s,n)$};
\node[view, above right=9mm and 20mm of polar] (stv) {Ordinary STV\\$\langle s,c\rangle$};
\node[law, below right=9mm and 20mm of polar] (laws)
  {Forcing laws\\Walley / odds-additive};
\node[boundary, right=34mm of stv] (phase)
  {Amplitude extension\\Born shadow $+$ phase};

\draw[->, thick] (counts) -- node[above, font=\scriptsize] {equiv.} (polar);
\draw[->, thick] (polar) -- node[above, sloped, font=\scriptsize]
  {choose chart $c=\chi(n)$} (stv);
\draw[->, thick] (laws) -- node[below, sloped, font=\scriptsize]
  {select PLN chart} (stv);
\draw[->, dashed, thick] (phase) -- node[above, font=\scriptsize]
  {forget phase} (stv);
\draw[->, dashed, thick] (polar) to[bend right=15]
  node[below, font=\scriptsize] {Hellinger/Born shadow} (phase);
\end{tikzpicture}
\caption{The proof-carrying DOF-vs-forcing map.  Evidence first
decomposes into direction and scale.  Strength is the direction view.
Confidence is a chart on scale, free until a law selects one chart.
Amplitude/phase states project back to the same ordinary STV by
forgetting phase; phase is extra extension structure, not hidden inside
$s$ or $c$.}
\label{fig:dof-forcing-map}
\end{figure}

\noindent
The bundled Lean surface
\code{PaperFacingDOFForcingSynthesisProfile} collects this map in one
place: strength as simplex direction, confidence as chart freedom,
revision and Walley laws as PLN-chart selectors, Beta prior
mean/concentration as learnable axes, Bernoulli e/m/Fisher geometry as
the differential reading, and phase as an explicit extension boundary.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/PLNTruthTheoryIndex.lean}, with the
supporting chart, information-geometry, and amplitude-boundary theorems
in \codepath{Mettapedia/Logic/PLNConfidenceWeightRevision.lean},
\codepath{Mettapedia/Logic/PLNInformationGeometry.lean}, and
\codepath{Mettapedia/Logic/PLNAmplitudePhase.lean}.}

The differential reading is also explicit.  The m-flat Bernoulli
geodesic is the straight mean-coordinate path
\[
  \gamma_m(t)=(1-t)p+tq,
  \qquad
  \frac{d\gamma_m}{dt}=q-p .
\]
The e-flat Bernoulli geodesic is straight only after applying the
natural coordinate $\theta=\log(p/(1-p))$:
\[
  \theta(\gamma_e(t))=(1-t)\theta(p)+t\theta(q),
  \qquad
  \frac{d}{dt}\theta(\gamma_e(t))=\theta(q)-\theta(p).
\]
These are formalized as derivative-backed geodesic handles, not just
as endpoint identities.

The amplitude boundary is now more than a metaphor.  In the formal
surface, a complex phase factor is identified with $\exp(i\theta)$, a
complex amplitude is identified with $r\exp(i\theta)$, and Born's rule
erases the phase for a single path:
\[
  \left\lvert \sqrt{p}\,e^{i\theta}\right\rvert^2 = p .
\]
For two paths the phase reappears exactly as interference:
\[
  \left\lvert r e^{i\theta} + s e^{i\phi}\right\rvert^2
  = r^2 + s^2 + 2rs\cos(\theta-\phi).
\]
So ordinary STV/ITV truth values are not ``missing a better confidence
formula'' here; they are the phaseless projection of a richer carrier.
The missing structure is phase, and phase changes how alternatives add.

\paragraph{Five tiny witnesses.}
The marble-bag witness separates strength from confidence.  With
horizon $k=2$, one red draw and no blue draw has empirical strength
$1/(1+0)=1$ but confidence $1/(1+2)=1/3$; one hundred red and one
hundred blue draws has strength $100/200=1/2$ but confidence
$200/(200+2)$.  Strength is the direction of the evidence cone.
Confidence is the scale of the evidence.  Confusing them makes one red
marble look like certainty.  The formal counterparts are the
simplex--scale equivalence
\code{positiveBinaryCountsEquivSimplexScale} and the prior-shift canary
\code{singlePositive\_uniform\_prior\_changes\_strength}.

The loudspeaker witness separates coherent phase from incoherent
addition.  Two speakers playing the same tone have two unit-amplitude
paths.  A phase-blind reader predicts weight $1^2+1^2=2$ everywhere.
At a point where the waves arrive in phase, however,
\[
  \lvert e^{i0}+e^{i0}\rvert^2 = 4;
\]
at a point where they arrive half a cycle out of phase,
\[
  \lvert e^{i0}+e^{i\pi}\rvert^2 = 0.
\]
Forgetting phase therefore confuses perfect reinforcement, ordinary
incoherent addition, and perfect cancellation.  This is the intuitive
content of the constructive/destructive interference canaries and the
two-path theorem
\code{complexTwoPathBornWeight\_interference}.

The confidence-chart witness separates a displayed number from the
evidence law that makes it meaningful.  Suppose two sources each display
confidence $1/2$.  Under the PLN/Mobius count chart, revising them gives
\[
  \frac{1/2+1/2-2(1/2)(1/2)}{1-(1/2)(1/2)}
  = \frac{2}{3}.
\]
Under the exponential/noisy-OR chart it gives $3/4$: two independent
chances to find the key succeed if at least one works.  Under the
tanh/Einstein chart it gives $4/5$: two half-speed bounded signals
combine by $(v+w)/(1+vw)$, not by ordinary addition.  Thus the number
$1/2$ is not self-interpreting.  PLN is the right chart for count-like
concentration; exponential is natural for hazard or independent
opportunity; tanh is natural for bounded signed support or trust.  These
are witnessed by
\code{plnConfidence\_revision\_closedForm},
\code{expCoordinate\_revision\_closedForm}, and
\code{tanhCoordinate\_revision\_closedForm}, with the $2/3$, $3/4$,
and $4/5$ canaries at one half.

The Goldilocks witness explains why the phase carrier is complex rather
than an arbitrary two-dimensional number system.  A dual-number carrier
has a nonzero infinitesimal direction whose conjugate norm cannot be a
positive-definite Born weight; the extra coordinate can be present while
the weight fails to see it faithfully.  A split-complex carrier has
norm $a^2-b^2$, so weights can become negative.  The ordinary complex
carrier has norm $a^2+b^2$, which is positive-definite and still carries
phase.  The formal ablation is
\code{ksComplexPhaseCarrier\_positiveDefinite} versus
\code{dualCarrier\_not\_positiveDefinite} and
\code{splitCarrier\_not\_positiveDefinite}.

The pool-versus-chain witness separates the m- and e-connections.
Start from prior odds $1:1$.  Two independent lab tests each with
likelihood ratio $3$ should be chained by multiplying odds:
$1\cdot 3\cdot 3=9$, hence posterior probability $9/10$.  If instead
one averages the two single-test posterior probabilities, each test
gives $3/(1+3)=3/4$, and the mixture average remains $3/4$.  Pooling
rates and chaining independent evidence are different operations.  The
m-flat story is straight in mean coordinate; the e-flat story is
straight in log-odds coordinate.  The derivative-backed handles are
\code{bernoulliMixtureGeodesic\_hasDerivAt} and
\code{bernoulliLogOdds\_exponentialGeodesic\_hasDerivAt}.  All five
witnesses are formalized as exact Lean facts in
\codepath{Mettapedia/Logic/PLNDidacticWitnesses.lean} and mirrored as
runtime canaries in
\codepath{/home/zar/claude/hyperon/PeTTa/examples/pln\_didactic\_witnesses.metta}
and
\codepath{/home/zar/claude/hyperon/CeTTa/tests/test\_wmpln\_didactic\_witnesses.metta}.
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=0.85\textwidth]{../results/figures/confidence_curves.pdf}
...
An interval $[s_L, s_U]$ gives endpoints for a selected uncertainty semantics.
Three semantics
...
This selector choice matters for the confidence coordinate.  Generic
ITVs do not require a law relating width and credibility.  But Walley's
IDM predictive interval has width $s/(n+s)$; if one imposes the
ITV-compatibility law ``credibility is the complement of predictive
width,'' then $c = 1 - s/(n+s) = n/(n+s)$.  The Lean theorem
\code{walley\_width\_complement\_forces\_plnOdds} proves that this
condition forces the usual PLN/NARS odds coordinate with $k=s$, while the
reserve-half coordinate is also proved incompatible with this Walley law.
Thus the ITV route does narrow the viable $c$-values in the Walley-IDM
case, but not in the generic Bayesian-credible-interval case.
...
\paragraph{A concrete illustration: historical PLN confidence bugs.}
The MeTTa-style PLN formulas have had formally proved
...
pairs and provenance rather than confidence scalars.  For fixed $k$,
$\code{c2w}$ and $\code{w2c}$ are inverse coordinate transforms between
confidence and total evidence weight, but weight is only one coordinate
of binary evidence and still omits provenance/overlap.  The practical
consequence of D4 (Evidence over Views) is that confidence is safe as a
display/query coordinate, not as a substitute for the state that
generated it.
...
\paragraph{Induction as online class induction.}
A single application of the induction rule is the \emph{atomic step} of a
statistical class induction.  Read $C$ as an observed individual and $A,B$ as
classes: the premises $C\!\to\!A$, $C\!\to\!B$ say only that one individual
instantiates both classes, while the conclusion $A\!\to\!B$ is a class law.
Each individual deposits evidence into \emph{both} directions ($A\!\to\!B$ and
$B\!\to\!A$); iterating over independent individuals and combining by revision,
which is sequential Bayesian updating (\S\ref{sec:additive-regime}), drives
$s_{AB}\to P(B\mid A)$ with confidence $\to 1$.  The \emph{direction} is fixed
by the base-rate factor $s_B/s_A$ already present in the inversion: the reverse
link converges to $P(A\mid B)$, which is diluted by every $B$-instance that is
not an $A$.  Thus ``ravens are black'' ($P(\mathrm{black}\mid\mathrm{raven})
\approx 1$) survives while ``black things are ravens''
($P(\mathrm{raven}\mid\mathrm{black})$) decays toward its base rate---the
extensional asymmetry of \code{Inheritance} reconstructed dynamically.
Convergence to the true conditional requires \emph{independent} evidence (else
revision double-counts); this is the role of the trail-free, fresh-evidence
discipline of Theorem~\ref{thm:damped-convergence}, and is why
\code{RavenInduction} assigns its two ravens disjoint provenance stamps.  We
leave as a target for
\codepath{Mettapedia/Logic/PLNDistributionalConvergence.lean} the statement
that iterated induction-plus-revision on i.i.d.\ instance evidence converges in
strength to $P(B\mid A)$ with confidence $\to 1$, the reverse link to
$P(A\mid B)$, their ratio fixed by the base rates.

A first installment of this target is now discharged, sorry-free and
axiom-free, in \codepath{Mettapedia/Logic/RavenAsymmetricInduction.lean}.
...
(\code{strength\_blackRaven\_antitone}) and \emph{converges to the base rate},
\code{strength\_blackRaven\_tendsto\_zero} proving $R/(R+M)\to 0$ as
$M\to\infty$ so that the asymmetry gap tends to $1$.  A dataset layer over
finite observation lists proves the strengths depend only on the two sufficient
counts (\code{aggregate\_blackRaven\_eq\_counts}) and are order-insensitive
(exchangeability, \code{aggregate\_blackRaven\_perm}); what remains open is the
full i.i.d.\ statement with confidence $\to 1$.  The runnable mirror is
\codepath{papers/examples/RavenAsymmetricInduction.metta}.

The query-side dual is abduction, formalized in
\codepath{Mettapedia/Logic/RavenAbduction.lean}.  With the unnormalized
posterior $\mathrm{score}(h)=\pi(h)\prod_i P(\mathrm{feature}_i\mid h)$, a single
common feature is dominated by the base rate
(\code{base\_rate\_dominates\_shared\_feature}): from $\mathrm{black}$ alone the
\emph{common} species is the better hypothesis (\code{crow\_best\_on\_black}),
precisely because $P(\mathrm{raven}\mid\mathrm{black})$ is small.  A
discriminating feature (``croaks'') flips the best explanation
(\code{raven\_best\_on\_black\_and\_croak}), so the best explanation is
non-monotone in evidence (\code{best\_explanation\_flips}).  The atomic NARS
abduction rule (\code{Truth\_Abduction}) generates the hypothesis but does not
rank competing explanations; ranking needs the prior.  Runnable mirror:
...
is an exact identity (Theorem~\ref{thm:mln-wm-strength}).  For the
\emph{countably infinite} case, the formalization proves conditional
existence theorems for the configured DLR/Gibbs setup, uniqueness under a
uniform contraction condition, and bridge theorems connecting Gibbs
probability back to WM-PLN query strength.
...
\medskip
\noindent
The first of the three infinite-case theorems establishes existence for the
configured Gibbs specification under the stated countability, exhaustion, and
...
The infinite story is certified across twelve active Lean files in three
layers:
\begin{description}
\item[Theory.]  \codepath{MarkovLogicInfiniteExistence.lean} (DLR existence
  ...
  uniqueness via descending-shell contraction).
...
passes through four stages.  Each stage is a single Lean
definition or theorem in the active WM-PLN development; together they form the return map from Gibbs
...
\begin{tikzpicture}[
  ...
  \node[stage, fill=yellow!12] (wm) at (0, 0)
    {$\mathrm{STV}(q)=\langle \mu(q),\,1\rangle$};
  ...
  \draw[arrow] (mass) -- (wm)
    ...
    {$\mu(q)$ = true mass,\quad $1{-}\mu(q)$ = false mass};
...
\caption{The infinite MLN $\to$ WM semantic pipeline.  Each arrow is a
  ...
  Dobrushin uniqueness guarantee. Under uniqueness, the query credal width is
  zero, so the confidence-like width-complement is $1$; the separate
  $1-\mu(q)$ quantity is false mass, not confidence.}
...
$2\times 0.6 = 1.2$ exceeds the threshold, so the proved Dobrushin uniqueness
theorem no longer applies. This points toward the classical low-temperature
Ising phase-transition story, where plus/minus infinite-volume phases can
coexist, but the present Lean development does not yet prove that specific 2D
plus/minus separation.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/MarkovLogicInfiniteGridExample.lean}
(0~sorry in the current slice).  The proof reuses the full pipeline:
...
\bigskip
\noindent\textit{The preceding subsections establish the purely semantic
  theory: existence, uniqueness, ontology growth, individuation, and
  transcendence for infinite Markov logic networks.  The subsections that
  follow apply this theory to self-modifying agents, universal prediction,
  ethics, and meaning.}
\bigskip
...
\bigskip
\noindent\textit{The remaining subsections ground the agent-theoretic stack
  in ethics: protected ethical commitments, paradigm-neutral conflict
  resolution, compilation correctness from source ethics to WM support, and
  computational tractability of practical ethical reasoning.}
\bigskip
...
\paragraph{Compilation correctness: from Gewirth PGC to WM positive evidence.}

The chain from source-side ethical justification to target-side WM support is
now a single proved theorem.  Given PGC assumptions and a purposive-agency
witness, the headline result delivers BOTH ontology-side satisfaction of the
Gewirth non-interference claim AND DDL-grounded WM positive evidence for the
corresponding obligation.%
\footnote{Formalized in
\codepath{Mettapedia/CognitiveArchitecture/GodelClaw/Ethics/WMGrounding.lean}
via \code{gewirthTrustTriangle\_primaryWMGrounding\_of\_PPA} (0~sorry).
The DDL$+$ $\to$ WM bridge at
\codepath{Mettapedia/Logic/DDLPlus/WMBridge.lean}
provides the intermediate step
\code{pgc\_nonInterference\_wmPositiveEvidence}
(0~sorry, 0~axioms).}

\noindent
This composes three already-proved bridges:
\begin{enumerate}
\item Ethics $\to$ DDL$+$: PGC IS DDLPlus ideal obligation (\code{pgc\_is\_governance\_norm}).
\item DDL$+$ $\to$ WM: deontic satisfaction $=$ WM positive evidence (\code{wmPositiveEvidence\_of\_deonticSat}).
\item WM $\to$ meta-stability: positive evidence inherits the full protected-goal stack.
\end{enumerate}
\noindent
The ethical model now uses \textbf{graded regulative ideals} rather than
boolean predicates: each dispositional, duty, and relational ideal carries a
degree of realization in $[0,1]$.  Satisfaction is ``degree $> 0$,'' connecting
naturally to WM's graded evidence.  The trust-triangle model realizes non-harm
to degree $0.9$, autonomy respect to $0.8$, epistemic universal love to $0.7$,
and friendship to $0.6$ --- honest partial realization rather than the earlier
all-or-nothing encoding.%
\footnote{Graded model in
\codepath{Mettapedia/CognitiveArchitecture/GodelClaw/Ethics/Ontology/ESOUpperShard.lean}
via \code{ESOUpperShardModel} with \code{UnitValue} degrees (0~sorry).}

\paragraph{Computational ethics: theory-guided decision problems.}

The GenEth-style practical resolver is now instantiated as a
\code{TheoryGuidedDecisionProblem} on the trust triangle.  The resolver
recommends safe escalation, the recommended action is admissible under the
top-down conflict discipline, and the decision budget (21~comparisons) is
explicitly bounded.  The deontic/value lowering equivalence is proved through
an explicit labeler-alignment hypothesis --- now instantiated on a live
labeler/encoder pair with proved end-to-end deontic/value query equalities.
The practical action-rendering bridges into the meaning/WM layer, and the
public WM-grounding surface now covers three lanes, not just one: Gewirth PGC,
universal-duty/non-maleficence, and universal-duty/consent.  All five ethical
grounds (asserted, Gewirth PGC, universal duty, care relation, consequentialist)
now carry non-trivial witnesses via \code{Witnessed}\(_0\).%
\footnote{Formalized in
\codepath{Mettapedia/CognitiveArchitecture/GodelClaw/Ethics/MetaEthicsTrustTriangleExample.lean}
and
\codepath{Mettapedia/CognitiveArchitecture/GodelClaw/Ethics/WMGrounding.lean}
(0~sorry).}

Three further case studies verify that the pipeline generalises beyond the
trust triangle:
\begin{itemize}
\item \textbf{Privacy vs.\ emergency disclosure.}  Actions: keep private /
  ask consent / alert trusted contact / broadcast widely.  Duties: privacy,
  autonomy, non-maleficence.  Resolver recommends \emph{alert trusted
  contact}, budget 52~comparisons.
\item \textbf{Shutdown instructability.}  Actions: continue / override /
  pause and ask / hard shutdown.  Duties: obedience, safety,
  non-disruption.  Resolver recommends \emph{pause and ask}, budget
  52~comparisons.
\item \textbf{Home-security force escalation.}  Actions: observe / warn /
  lock down / deploy force.  Duties: safety, proportionality,
  non-maleficence.  Resolver recommends \emph{lock down}, budget
  52~comparisons.  This case also exercises the candidate-local
  consequentialist witness, verifying that the
  \code{WitnessedForCandidateSet}\(_0\) machinery works on a live choice set.
\end{itemize}
\noindent
Each case provides a proved dominant admissible action, an explicit polynomial
comparison-count budget, and negative certificates for the losing candidates.%
\footnote{Formalized in
\codepath{Mettapedia/CognitiveArchitecture/GodelClaw/Ethics/EthicalDecisionProblemCaseStudies.lean}
(973~lines, 0~sorry).}
...
\begin{example}[Temporal chaining: what the \texttt{q r m} stream says]
\label{ex:qrm-temporal-chaining}
Consider the following finite event stream:
\begin{quote}
\small\ttfamily
k a q r m b h t q r m s p q n r c q r z d m a r\\
q h q r m b l q r m t c q r v n p q r m s
\end{quote}
There are several different questions one might accidentally call
``$q \to m$''.  They are not the same question.
...
\toprule
\textbf{Question} & \textbf{Count in the stream} & \textbf{Meaning} \\
\midrule
$q \to r$ & $7/9$ & after seeing $q$, expect $r$ next often \\
$r \to m$ & $5/9$ & after seeing $r$, expect $m$ next sometimes \\
$q \to m$ in one step & $0/9$ & $m$ never appears immediately after $q$ \\
$(q,r) \to m$ & $5/7$ & once $q$ is followed by $r$, $m$ is likely next \\
$q \to r \to m$ in two steps & $5/9$ & five of the nine $q$'s start a $qrm$ pattern \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Here $(q,r)$ means a two-event context: the last two events were
$q$ and then $r$.  It is not the same as asking what follows $q$
alone.  The common mistake is to flatten all of these into one
generic ``$q \to m$'' question.

\noindent
So the plain-language answer is:
\[
\boxed{\begin{gathered}
\text{Do not believe immediate } q \to m.\\
\text{Do believe the mediated pattern } q \to r \to m.
\end{gathered}}
\]
If you have seen only $q$, the next useful prediction is $r$, not
$m$.  If you have seen $q$ followed by $r$, then $m$ is the good
prediction.  This is the answer the world-model view wants us to
represent.

NARS gives a useful scalar warning for the collapsed claim.  Its
deduction rule derives a weak chain estimate for ``$q$ leads to
$m$'':
\[
  \langle 35/81,\;7/20\rangle
  \approx \langle 0.432,\;0.350\rangle.
\]
Revising that with the direct negative one-step evidence
$\langle 0,\;0.9\rangle$ gives
\[
  \langle 245/10044,\;124/137\rangle
  \approx \langle 0.024,\;0.905\rangle.
\]
As a one-number warning, this is sensible: the collapsed one-step
claim $q \to m$ should be near zero.

A deliberately naive scalar PLN calculation gives a different scalar
if we force the same collapsed question through the full deduction
formula using the
stream marginals $P(q)=P(r)=9/45$ and $P(m)=6/45$:
\[
  \text{chain estimate} =
  \langle 71/162,\;0.9\rangle
  \approx \langle 0.438,\;0.9\rangle,
\]
and naive scalar revision with the direct negative packet gives
\[
  \langle 71/324,\;18/19\rangle
  \approx \langle 0.219,\;0.947\rangle.
\]
This is not the answer we should trust for the one observed stream.
The reason is not that PLN arithmetic is meaningless.  The reason is
that the chain estimate and the direct one-step estimate were both
computed from the same 45 observations.  Revision is evidence
addition.  You cannot get more evidence by counting the same data
twice and pretending that it came from two independent experiments.
Evidence addition is licensed when the packets are fresh or
provenance-disjoint.  Here they are not fresh: the same $q$ positions
are being reused.

WM-PLN therefore gives more guidance than either bare scalar:
\begin{enumerate}
\item Ask the right query.  The immediate query $q \to m$ is false in
  this stream; the mediated query $q \to r \to m$ is supported.
\item Keep the structured answer.  Store $q \to r$ and
  $(q,r)\to m$ rather than collapsing everything into one
  unconditional $q \to m$ truth value.
\item Do not revise same-stream derived evidence with same-stream
  direct evidence as if it were new data.
\item If an independent validation stream later supplies a direct
  $q \to m$ estimate, then revision is legitimate; the provenance
  stamps are disjoint.
\end{enumerate}

\noindent
This example is formalized in Lean and mirrored by two runtime probes:
\begin{flushleft}
\footnotesize
\codepath{Mettapedia/Logic/TemporalChainingExample.lean}\\
\codepath{hyperon/PeTTa/examples/nars\_temporal\_sequence\_probe.metta}\\
\codepath{hyperon/PeTTa/examples/pln\_temporal\_sequence\_probe.metta}
\end{flushleft}
\end{example}
...
\item[Tier~1: Generic / infinite (formalized now).]
  ...
  MLN layer and FOL/PredCode infinitary
...
is still broader and more exploratory than the stable core narrative.
However, the finite-alphabet Solomonoff layer and its controlled-HMM
bridge are now theorem-grade and directly connected to the WM-facing
development: the finite-alphabet universal mixture is concrete, the
controlled fixed-action-stream regret theorem is proved, and the
controlled-HMM export surface reuses it.  What remains open is the
genuinely action-aware universal mixture and the broader integration of
the exploratory universal-prediction modules.  So the honest status is
mixed: the core controlled-HMM/Solomonoff seam is now stable and
sorry-free, while the larger universal-prediction program remains
work-in-progress.
...
The current formalization has begun this boundary without claiming the
full quantum calculus.  The file
\codepath{Mettapedia/Logic/PLNAmplitudePhase.lean} proves that ordinary
binary PLN strength is recovered as the Born shadow of the Hellinger
amplitude, that the ordinary typed-STV projection forgets relative
phase, and that the complex amplitude
$r\exp(i\theta)$ satisfies the exact two-path interference law
$|r e^{i\theta}+s e^{i\phi}|^2
= r^2+s^2+2rs\cos(\theta-\phi)$.  It also reuses the
two-dimensional Knuth--Skilling-style algebra classification:
the negative-$\mu$ carrier is equivalent to $\mathbb{C}$ and is exactly
the positive-definite conjugate-norm regime, while the dual-number and
split-complex carriers fail that gate.  Thus standard PLN is the real
phaseless shadow; a waveform or quantum PLN would need a separate phase
semantics rather than another confidence formula.

A useful future-work witness for the infinitary side is now more sharply
split. A first concrete infinitary contrast is already formalized: a
geometrically reinforced half-line yields positive origin-spin envelope
width, while the zero-weight zero-field grid collapses under the Dobrushin
budget. What remains open is the genuinely classical 2D low-temperature
Ising plus/minus separation, where two distinct infinite-volume Gibbs
measures are constructed and shown to separate a named query. That is the
right next phase-coexistence crown linking infinite MLNs, projective
completion, and imprecise de~Finetti.
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=0.85\textwidth]{../results/figures/broken_sensor.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/gas_sensor_drift.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/gas_drift_protocols.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/gas_policy_frontier.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/steel_classification.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/nab_kalman_tracking.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/gjp_forecasting.pdf}
...
Chapter~\ref{ch:intensional}) to the same genomic KB, augmented with
co-regulation and co-expression patterns mined by the Hyperon pattern
miner (Section~\ref{sec:selectors}).  Three typed PLN rules
...
edges from Reactome~\cite{Fabregat2018Reactome} (124 pathways, up to 2{,}000
...
Adding Hetionet~\cite{Himmelstein2017Rephetio} disease associations (42~edges
...
\paragraph{CDO1 $\to$ hypertension (falsified by colocalization).}
...
vasodilation, and taurine supplementation trials show
antihypertensive effects~\cite{Militante2002Taurine}.  The
backward chainer identified this chain from 58~eQTL SNPs
(tissue-broad, effect-strong) and Hetionet's CDO1--hypertension
annotation.  Tissue extraction confirmed 723 eQTL variants
across 42~tissues, with 248 in aorta (77\% expression-decreasing).
However, OpenTargets colocalization \emph{falsified} the causal
link: the BP GWAS signals at this locus are driven by SEMA6A
and KCNN2, not CDO1 (see above).  This illustrates the gap between
literature-derived associations and genetic causality---the
central methodological point of this chapter.
...
\paragraph{Drug repurposing: compound-binding chains without Hetionet treatment edges.}
Among the 5{,}833 chains, 356 involve genes that are known
drug targets (Hetionet compound binding) but whose associated
diseases have no compound--disease treatment edge in
Hetionet~v1.0 (2017).  This does \emph{not} mean these
diseases lack treatment---mesothelioma, for example, has
guideline-backed chemotherapy and immunotherapy
regimens---but rather that Hetionet's edge coverage is
incomplete.  With that caveat, the top candidates:
...
PTGER4 $\to$ mesothelioma is the strongest drug repurposing
finding, but the therapeutic direction requires care.
In hematologic biology, EP4 \emph{agonism} promotes
hematopoietic stem cell expansion (Misoprostol); in
solid-tumor immuno-oncology, EP4 \emph{antagonism} suppresses
PGE2-mediated immune evasion (TPST-1495).  These are
opposite interventions for the same receptor, depending on
disease context.  The 93 eQTL SNPs provide genetic instruments,
but the therapeutic hypothesis must be split by cell type:
hematopoietic (agonist logic) vs.\ tumor-immune (antagonist
logic).
...
them.  HCN1 $\to$ schizophrenia (a separate Hetionet association)
was independently confirmed as a known PGC3 GWAS hit
($p = 5.55 \times 10^{-11}$), validating the pipeline's ability
to recover established genetics.  The HCN1 $\to$ epilepsy chain
(common-variant expression modulation of a known rare-variant
epilepsy gene) remains untested by colocalization and is
scientifically distinct from the schizophrenia association.
...
\begin{enumerate}
\item \textbf{Systematic disease-specific colocalization.}
  Of 2{,}482 genes in the discovery sweep, 98\% have OpenTargets
  eQTL studies with 3.4M strong colocalizations.  Matching these
  against disease-specific GWAS requires a DOID$\to$EFO ontology
  bridge to identify which chains are colocalization-supported,
  falsified, or untestable.  This calibration table is the key
  deliverable for a methods paper.
\item \textbf{PTGER4 disease-subtype refinement}: split ``hematologic
  cancer'' into DLBCL, CLL, AML, MM, MDS.  The hematopoietic
  colocalization is confirmed; cancer subtype resolution determines
  the translational path.
\item \textbf{KB freshness}: the CDO1 locus redirected to
  SEMA6A/KCNN2, but these genes are absent from Hetionet~v1.0
  (2017).  Integrating OpenTargets L2G predictions as an
  additional knowledge source would capture the current GWAS
  landscape and unlock genes that post-date Hetionet.
\item \textbf{PLN signed truth values}: propagate effect direction
  (expression increase/decrease) through proof terms to
  automatically distinguish gain-of-function from
  loss-of-function chains and flag wrong-directional drug
  repurposing candidates (as occurred with IMPA1/lithium and
  DHFR/methotrexate).
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_normal_holdout.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=0.85\textwidth]{../results/figures/pplbench_normal_normal_adaptive_pll.png}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=0.85\textwidth]{../results/figures/pplbench_dirichlet_categorical_pll.png}
...
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_crowd_sourced_annotation_pll.png}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_n_schools_pll.png}
\end{minipage}

\medskip
\begin{minipage}{0.48\textwidth}
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_logistic_regression_pll.png}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_robust_regression_pll.png}
\end{minipage}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/pplbench_refresh_checks.pdf}
...
\begin{figure}[htbp]
\centering
\wmplnincludegraphics[width=\textwidth]{../results/figures/ai_outcomes_assumption_ladder.pdf}
...
\code{Logic/DDLPlus/WMBridge} & 133 & 4 & Ch~16 \\
...
\compactcodepath{CogArch/GodelClaw/Ethics}{EthicalDecisionProblems} & 161 & 5 & Ch~16 \\
\compactcodepath{CogArch/GodelClaw/Ethics}{EthicalDecisionProblemCaseStudies} & 973 & 18 & Ch~16 \\
