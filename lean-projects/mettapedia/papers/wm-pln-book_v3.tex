...
704~theorems with 0~sorry and 0~\code{native\_decide}
...
\paragraph{There are now two certified MLN cases.}
The current formalization separates the MLN story into two mathematically
different regimes:
\begin{enumerate}
\item \textbf{Finite-support / ground MLNs.}  Here the partition function is
  finite, and the WM bridge is an exact identity on query probabilities
  (Theorem~\ref{thm:mln-wm-strength}).
\item \textbf{Countably infinite MLNs.}  Here the semantics is no longer a
  single finite normalization problem; it becomes a Gibbs/DLR measure
  problem.  The formalization now proves an existence theorem for the
  classical weighted-clause surface and a uniqueness theorem under a uniform
  Dobrushin contraction hypothesis.
\end{enumerate}
...
\codepath{Mettapedia/Logic/PLNMarkovLogicInfiniteUniqueness.lean},
transferring the compactness packaging from
\codepath{Mettapedia/Logic/PLNMarkovLogicInfiniteExistence.lean}
...
\[
  ...
  \sum_b C(a,b) \le C,
\]
then it has at most one fixed-region cylinder DLR probability measure.%
\footnote{Formalized as
\code{paperUniformSmallTotalInfluence\_implies\_paperUniqueMeasure}
in \codepath{Mettapedia/Logic/PLNMarkovLogicInfiniteUniqueness.lean}
(0~sorry in the current slice).}
\end{theorem}

\noindent
So the finite and infinite cases certify different kinds of statements.
The finite theorem says: \emph{the extracted WM strength is exactly the MLN
marginal}.  The infinite theorem says: \emph{the local Gibbs laws extend to a
global probability measure, and under the uniform Dobrushin condition that
global semantics is unique}.  This is the right abstraction shift: exact
probabilities in the finite case, well-posed Gibbs semantics in the infinite
case.
...
is in \codepath{Mettapedia/Logic/PLNMarkovLogicUWCSE.lean}.}
...
The infinite MLN example
\codepath{Mettapedia/Logic/PLNMarkovLogicInfiniteBeliefLineExample.lean}
models an unbounded line of agents indexed by $\mathbb{N}$.  Each site has a
soft prior clause favoring belief, and each edge carries a soft influence
clause
\[
  \mathrm{Believes}(n) \to \mathrm{Believes}(n{+}1).
\]
The existence theorem gives a global Gibbs measure for this open-ended social
system, while the local DLR corollary shows that resampling the neighborhood
around site~$n$ preserves the marginal belief probability at~$n$.%
\footnote{Formalized as
\code{exists\_beliefLine\_fixedRegionCylinderDLR} and
\code{beliefLine\_singleAgentBelief\_fixedRegionDLR} in
\codepath{Mettapedia/Logic/PLNMarkovLogicInfiniteBeliefLineExample.lean}.}

\noindent
This matters for modeling because many relational systems are not naturally
finite.  Positive examples include an indefinitely long hallway of
Maple-Court-style apartments with local leak--humidity influence, an
open-ended trust or rumor-propagation chain, or a supply line where each site
depends mainly on nearby sites.  The infinite MLN formalization says when such
local log-linear laws define a \emph{global} semantics rather than merely a
sequence of finite approximations.

\noindent
Negative example: the current uniqueness theorem does \emph{not} say that
every infinite MLN has a unique global measure.  If the total incoming
influence is not uniformly below~$1$, the proved theorem does not apply, and
boundary-dependent or phase-transition-like behavior remains possible.
...
weighted evidence with increasing confidence.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/BDD/WMPLNBridge.lean}
(4~theorems, 0~sorry).  The full BDD compilation chain
(5,148~lines across 13~files, 0~sorry) is in
\codepath{Mettapedia/Logic/BDD/}.}
...
\midrule
\code{Logic/PLNMarkovLogicWorldModel} & 44 & 2 & Ch~16 \\
\code{Logic/PLNMarkovLogicInfiniteExistence} & 146 & 3 & Ch~16 \\
\code{Logic/PLNMarkovLogicInfiniteUniqueness} & 6758 & 236 & Ch~16 \\
\code{Logic/PLNMarkovLogicInfiniteBeliefLineExample} & 241 & 6 & Ch~16 \\
\midrule
...
\code{Logic/PLNWorldModelOrderCostProvenanceDemo} & 255 & 5 & Ch~12 \\
...
\midrule
\textbf{Total (book-referenced files)} & \textbf{$\sim$18,900} & \textbf{951} & \\
\bottomrule
\end{longtable}
