...
this one.  All eight desiderata, all three carriers, and the
...
651~theorems with 0~sorry and 0~\code{native\_decide}
...
overlap is small or non-actionable ($\sim$90~theorems, 0~sorry).
...
reduces MAE by $1.5$--$3.8\times$), and nine PPLBench lanes
...
world model via multiset counting.
...
\item[Tier~3: Genuinely open.]
  The measure-valued terminal world model (connecting evidence
  carriers to \code{MeasureTheory.Measure}), continuous-domain BN
  compilation with infinite state spaces, and quantale/Heyting
  generalization beyond binary evidence.
\end{description}
...
\emph{Proved:} the Solomonoff--de~Finetti foundation and the Giry
monad bridge (Chapter~\ref{ch:bridges}).
\emph{Missing:} $\sigma$-algebra-valued carriers with
measure-theoretic extraction.
...
The entire development is machine-verified: 651~theorems in Lean~4,
...
\midrule
\textbf{Total (book-referenced files)} & \textbf{$\sim$10,520} & \textbf{651} & \\
\bottomrule
\end{longtable}
