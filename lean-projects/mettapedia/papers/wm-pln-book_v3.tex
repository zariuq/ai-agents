...
\item[Examples.]  Belief line (1D chain), 2D grid (Ising lattice),
  ...
  variable-neighborhood variants), the trust-triangle capstone, and
  self-transcendence paths --- eight files, each reusing the same
  pipeline.
\end{description}
...
when a subsystem IS individuated --- but does not yet formalize the
\emph{process} by which a subsystem emerges (dynamic individuation), which
...
\caption{The trust triangle.  Agents $\{0,1,2\}$ form an interaction-closed
  core (blue, dashed boundary).  An infinite chain $\{3,4,5,\ldots\}$ operates
  independently --- no clause crosses the boundary.  Specifications $M_1$ and
  $M_2$ share the triangle weight $w_t$ but differ on the chain weight $w_c$.
  Theorem: $\mathrm{queryStrength}_{M_1}(q) = \mathrm{queryStrength}_{M_2}(q)$.}
...
\begin{longtable}{@{}p{0.44\textwidth}rrp{0.21\textwidth}@{}}
\toprule
...
\code{Logic/MarkovLogicInfiniteBoundaryStability} & 139 & 4 & Ch~16 \\
...
\code{Logic/MarkovLogicInfiniteBeliefLineExample} & 359 & 9 & Ch~16 \\
\code{Logic/MarkovLogicInfiniteBidirectionalWorldOfViews} & 378 & 12 & Ch~16 \\
\code{Logic/MarkovLogicInfiniteBoundedGraphExample} & 322 & 13 & Ch~16 \\
\code{Logic/MarkovLogicInfiniteGridExample} & 430 & 13 & Ch~16 \\
\code{Logic/MarkovLogicInfiniteVariableNeighborhoodWorldOfViews} & 369 & 6 & Ch~16 \\
