...
Probabilistic Logic Networks (PLN) has always computed with
truth values: strength, confidence, and the formulas that combine
them.  Every PLN implementation builds on the same foundation---combine
truth values via inference rules, derive new truth values,
threshold for decisions.  This book asks: \emph{what if truth
values are not the right foundation?}

The answer is the world-model calculus.  Instead of truth values,
the system carries a revisable state of knowledge---sufficient
statistics, not summaries.  Instead of truth-value combination
formulas, it uses additive revision.  Instead of ad hoc
confidence measures, it extracts answers via homomorphisms that
commute with revision.  Truth values are still available---as
\emph{views}, lossy projections of a richer underlying state.

The result is a calculus where:
\begin{itemize}
\item Revision is compositional: new evidence adds to old evidence.
\item Queries are principled: extraction is a homomorphism.
\item Forgetting is provenance-aware: you can retract a specific
  source's contribution without recomputing from scratch.
\item Every formal claim is machine-verified: 601~theorems in
  Lean~4, 0~sorry, 0~\code{native\_decide}.
\end{itemize}

\noindent
Eight desiderata (D1--D8, Chapter~\ref{ch:desiderata}) constrain
the design.  A running example---Maple Court, an apartment building
with sensors, tenants, and an elevator---instantiates all eight
and appears throughout the book in teal-boxed asides.

This book is not a PLN tutorial or a replacement for the PLN
book~\cite{Goertzel2008PLN}.  It is a mathematical refoundation
with formal verification, aimed at researchers who want to
understand PLN's semantic commitments and extend the calculus to
new domains.  The empirical validation
(Part~\ref{part:applications}) covers gas sensor drift, steel
fault classification, geopolitical forecasting, Bayesian network
benchmarks, genomic hypothesis generation, and nine PPLBench
lanes---but the core contribution is the algebra, not any single
application.

The remainder of this chapter presents the world-model calculus
in miniature: the core picture, revision regimes, evidence
carriers, the boundary, and how the rest of the book unfolds.

%% ─── Overview (formerly ch2_overview.tex, inlined) ───────────────
...
\noindent
This interface is intentionally minimal.  No additivity,
commutativity, or probabilistic interpretation is assumed at this
top level.

The book works mostly in one important specialization: the
\emph{additive regime}.  There, state revision is additive, the
value carrier is an additive commutative monoid, and extraction
commutes with revision:
...
\noindent
The decisive law is \code{extract\_add}.  It says that revising
states and then extracting is the same as extracting first and
combining in~$V$.

\begin{theorem}[Profile-hom equivalence]
\label{thm:overview-profile-hom}
Additive world models are exactly additive monoid homomorphisms
$\textit{State} \to^+ (\textit{Query} \to V)$.
\end{theorem}

\maplecourt{Maple Court Community Commons is our running example.
Each apartment carries a private posterior state over questions
such as whether a pipe is leaking, whether the shower is running,
or whether motion indicates occupancy.  Shared spaces carry a
building-level state over hallway humidity, laundry-room
availability, and elevator health.  The same architecture appears
at both scales: revise the state as reports and sensor updates
arrive, query the state for evidence, and project into views only
...
The calculus distinguishes several regimes:

\begin{description}
\item[Additive revision.]  Independent updates compose by addition.
\item[Overlap-aware revision.]  Shared information prevents naive addition.
\item[Trust-gated revision.]  The merge depends on source authority.
\item[Coalition revision.]  Consensus requires an actionable coalition.
\end{description}

\noindent
Only the first is developed in full generality in this book.


\section{Evidence Carriers}
\label{sec:overview-carriers}

Three carriers are central: \emph{binary} (positive/negative
counts), \emph{Dirichlet} (category counts), and
\emph{Normal-Gamma} (continuous sufficient statistics).  In Maple
Court these arise naturally: leak alarms are binary, elevator
health is multi-valued, hallway humidity is continuous.  The
calculus does not change across these cases; only the carrier
does.
...
A state can be updated, partially retracted, audited, replayed,
and explained in a way that a naked scalar view cannot.

\maplecourt{Suppose three hallway humidity alerts arrive, then
one sensor is found to be faulty.  If the system carries only a
strength estimate $0.83$, it is unclear how to retract just the
faulty source.  If the system carries a revisable state with
provenance, forgetting removes the appropriate contribution while
preserving answers outside the forgotten scope.}
...
\emph{more informed} and \emph{more supportive}---these are not
the same order.  Compare $e_1 = (1,0)$ and $e_2 = (1,1)$: the
second is more informative but less supportive of the positive
claim.  A view is a projection, not a proof object.


\section{Compiled Inference and the PLN Connection}
\label{sec:overview-compiled}

The calculus treats deduction, induction, and abduction as
\emph{compiled rewrites}: certified shortcuts that are exact
under explicit side conditions within declared model classes.

\maplecourt{The building model contains the chain
$\textit{PipeLeak} \to \textit{WallHumidity} \to
\textit{MoldRisk}$.  Rather than recomputing the full posterior,
the system uses a compiled chain rule justified by the BN
factorization.  When the side conditions hold, the compiled answer
agrees with the full extractor.  When they do not, the rewrite is
not licensed.}
...
Complete link evidence cannot in general be recovered from local
per-link surrogates alone.
\end{theorem}

\noindent
This boundary theorem explains why structural side conditions
matter: compiled shortcuts become exact only inside model classes
that carry enough global structure for those shortcuts to commute
...
\noindent
The state is primary.  Queries interrogate the state.  Extraction
returns values in a carrier.  Views project from those values.
Compiled rewrites accelerate families of queries under explicit
side conditions.  Boundaries tell us where such shortcuts cannot
be complete.

\paragraph{PLN heritage.}
This book is not a historical commentary on PLN, but neither is it
detached from that lineage.  Where the earlier rule family is
correct, the present calculus proves it correct under explicit
assumptions.  Where it is incomplete or semantically unstable, the
present calculus states the boundary and replaces the heuristic
with a typed, side-conditioned construction.

\section*{How the Rest of the Book Unfolds}

Part~I (this chapter and the desiderata) gives the orientation.
Part~II develops the calculus in depth: evidence carriers,
state operations, views, and natively typed world models.
Part~III covers compiled inference, BN exactness, the no-go
theorem, and large-scale inference control.
Part~IV extends to temporal, causal, and normative reasoning.
Part~V bridges to neighboring frameworks (MLNs, ProbLog, NARS,
FOL, Giry monad, $\rho$-calculus, ontology repair) and states
the limitations.
Part~VI validates the framework on real benchmarks: gas sensor
drift, steel faults, anomaly detection, geopolitical forecasting,
Bayesian networks, genomics, nine PPLBench lanes, and an
exploratory AI-outcomes analysis.
...
%
\begin{center}
exchangeability $\to$ de~Finetti mixture $\to$
Kyburg flattening $\to$ Giry bind $\to$ evidence sufficiency.
\end{center}
%
\noindent
...
Chapter~\ref{ch:introduction} introduced four revision regimes.  Here
...
Lean formalization contains $\sim$90 theorems across these
...
\begin{tabular}{lll}
\toprule
\textbf{Hypercube vertex} & \textbf{WM subclass} &
  \textbf{Extraction law} \\
\midrule
Kolmogorov ($\sigma$-additive) & \code{AdditiveWorldModel} &
  extract distributes over $+$ \\
Dempster-Shafer (subadditive) & \code{OverlapWorldModel} &
  overlap correction \\
Quantum (non-commutative) & (future) &
  interference terms \\
\bottomrule
\end{tabular}
\end{center}
...
The compatibility-gate demo
(\codepath{Mettapedia/Logic/WMUWCSEGateDemo.lean},
12~theorems, 0~sorry) kernel-checks the gate structure:
PairAffinity is the critical discriminator with marginal
contribution $4 > 3 > 2$; forgetting it drops prediction from
$6{:}1$ to $4{:}1$.}
...
The entire development is machine-verified: 601~theorems in Lean~4,
...
\caption{Cross-lane refresh verification.  All nine lanes pass
...
\claimConditional{} & Requires declared finite-domain or structural assumption \\
...
\maplecourt{\ldots} & Maple Court apartment example (teal-boxed running example) \\
\bottomrule
\end{longtable}
...
this book, with exact theorem counts from the current codebase
(Lean~4.28.0, Mathlib~v4.28.0).  All files have 0~sorry.

\begin{longtable}{@{}p{0.42\textwidth}rrp{0.25\textwidth}@{}}
\toprule
File & Lines & Thm & Backs \\
\midrule
\endfirsthead
\toprule
File & Lines & Thm & Backs \\
...
\code{Logic/PLNWorldModelOrderCostGasPolicyDemo} & 155 & 6 & Ch~20 \\
\code{Logic/PLNBioHypothesisGeneration} & 635 & 39 & Ch~22 \\
...
\midrule
\textbf{Total (book-referenced files)} & \textbf{9,962} & \textbf{601} & \\
\bottomrule
\end{longtable}
...
full project to $\sim$700 theorems.  The entire formalization
...
\begin{longtable}{@{}p{0.09\textwidth}p{0.20\textwidth}p{0.32\textwidth}p{0.28\textwidth}@{}}
\toprule
