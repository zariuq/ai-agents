...
(1{,}064{,}375 associations) identifies four gene--disease
pairs with \textbf{zero prior GWAS associations}---genuine
novelty candidates generated by PLN backward chaining:

\begin{center}
\begin{tabular}{llcrll}
\toprule
Gene & Disease & eQTL SNPs & Tissue & Effect & GWAS Catalog \\
\midrule
CDO1   & hypertension       & 58  & broad    & strong   & 0~hits \\
CLU    & hypertension       & 63  & moderate & strong   & 0~hits \\
PTGER4 & hematologic cancer & 93  & rare     & moderate & 0~hits \\
HCN1   & epilepsy           & 208 & moderate & strong   & 0~hits \\
\bottomrule
\end{tabular}
\end{center}
...
statistics for each disease, which is future work.  The novelty
claims are verified against the GWAS Catalog as of March~2026;
22.7\% of eQTL-mapped genes have Hetionet disease associations
(background rate), so the 2{,}482 genes with chains are not
individually surprising---the value is in the \emph{specific}
gene--disease pairings and the mechanistic proof terms connecting
them.


\section{Status and Future Work}

The current state: score-level parity is verified, the Lean bridge
is proved, the delta-method confidence is formalized, the 9-rule
backward chainer produces 5{,}833 SNP$\to$Gene$\to$Disease chains
with 4 GWAS-Catalog-novel hypotheses (CDO1, CLU, PTGER4, HCN1),
and 1.41M pathway-mediated hypotheses at depth~4.
What remains:

\begin{enumerate}
\item \textbf{Mendelian Randomization validation} of the 4 novelty
  candidates using full GWAS summary statistics from UK Biobank
  and the ILAE consortium.  The eQTL SNPs provide the genetic
  instruments; what is needed is the outcome-side summary statistics.
\item \textbf{Replication} in an independent eQTL dataset (eQTLGen
  consortium, 31{,}684 blood samples) to confirm that the
  eQTL signals driving the chains are robust.
\item \textbf{PLN revision aggregation} of multi-path proof terms---the
  depth-3 query produces up to 7 independent paths per gene, each
  carrying evidence that should be combined via the quantale revision
  operator (Chapter~\ref{ch:evidence}).
\item A full native WM-PLN genomics pipeline replacing ProbLog's
  inference engine, with ProbMeTTa as the runtime.
\end{enumerate}
