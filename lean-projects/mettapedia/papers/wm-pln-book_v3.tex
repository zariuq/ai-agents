...
\paragraph{The runtime consequence.}
A runtime implementation of evidence operations (whether in
mettail-c, mettail-rust, or any other backend) proves a single
property---\code{RuntimeSound}, which says that its revision and
extraction agree with the algebraic specification---and all four
structural rules transfer automatically.  The proof of
\code{RuntimeSound} is the certification that the runtime correctly
implements the evidence proof system.%
\footnote{Formalized in
\codepath{Mettapedia/Logic/RuntimeSoundnessBridge.lean}
(5~theorems, 0~sorry).  See also~\codepath{Mettapedia/Logic/%
PLNProofSystemShowcase.lean} for the unified entry point.}
...
\item \textbf{A proof system.}
  ...
  (\S\ref{sec:evidence-proof-system}).  Runtime implementations
  that prove soundness inherit these rules automatically.
