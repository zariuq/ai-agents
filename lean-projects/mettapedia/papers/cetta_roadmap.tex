...
One promising discipline, especially for future relation-compiled lanes,
is to split unification into two interfaces: a \emph{guard predicate}
(``does a match exist?'') and a deterministic \emph{binding extractor}
(``what substitution does this chosen match induce?'').  Positive
example: a rho-style COMM lane can let the guard participate in
search/saturation while delaying substitution construction until a
rewrite actually fires.  Negative example: one monolithic unification
hook that entangles search branching, fixpoint existence, and binding
materialization.
...
This is not only a usability issue.  In fragments with multiset or
bag-style pattern matching (rho-style COMM, certain PLN conjunctions),
enumerating all compatible matches is combinatorial in the worst case.
For those lanes, \code{once}, \code{select k}, and related demand bounds
are part of the tractability contract, not just presentation knobs.
...
But \code{--lang petta} is not the hardest dialect pressure test.
Because it stresses reflection, multiset matching, channel/name
capabilities, and concurrency semantics, a future \code{--lang
rhocalc} may be the more revealing test of whether the shared search
waist is genuinely dialect-parametric rather than merely tolerant of
surface syntax variation.
...
Reflection also sharpens the future liveness/GC story.  If reified
spaces can be inspected ambiently, hidden names can leak and naive
reachability notions break under lookahead.  Any future GC theory for
reified spaces should therefore be deployment-relative and
capability-disciplined: a process inspects a space only if it was
explicitly given that capability.  This is a plausible later Lean
formalization target once reflective spaces become first-class in the
runtime story.
