...
formalization comprises 23,899 lines of Lean code with zero unproved
...
\item A complete formalization of Theorem~\ref{thm:df80} in Lean~4 with Mathlib:
  \textbf{23,899 lines}, \textbf{0 sorry}, \textbf{28 files}.
...
\midrule
\textbf{Total} & \textbf{28 files} & \textbf{23,899} \\
\bottomrule
\end{tabular}
...
theorem (Diaconis--Freedman 1980), comprising 23,899 lines with zero unproven
...
\subsubsection{Giry Monad Integration}

The Giry monad $\mathcal{G}$ on measurable spaces provides a categorical
framework for probability. The Markov de~Finetti theorem can be restated
categorically:

\begin{quote}
\emph{A Markov-exchangeable, recurrent kernel $\kappa : 1 \to \mathcal{G}(\mathrm{Seq}(A))$
factors through the Markov parameter space as
$1 \xrightarrow{\pi} \mathcal{G}(\Theta_k) \xrightarrow{\mathrm{MC}} \mathcal{G}(\mathrm{Seq}(A))$
where $\mathrm{MC}$ is the Markov-chain kernel.}
\end{quote}

Existing infrastructure in \texttt{CategoryTheory/DeFinetti*.lean} (15 files,
$\sim$9,100 lines) provides the Kleisli(Giry) diagram and Markov-category
bridge. Connecting this to the measure-theoretic formalization would:
\begin{itemize}
\item Enable abstract reasoning about conditioning and disintegration
\item Provide a bridge to synthetic measure theory
\item Allow composition with other categorical probability constructions
\end{itemize}

\subsubsection{Abstract Recurrence (Fully Formalized)}

The core formalization requires recurrence to the \emph{starting state}.
We have now formalized the generalization to \emph{recurrent classes}:

\begin{lstlisting}
-- MarkovDeFinettiRecurrence.lean:183
...
-- MarkovDeFinettiRecurrence.lean:190
...
\noindent\textbf{Infrastructure now in place:}
\begin{itemize}
\item Class-based visit-time machinery: \texttt{classVisitCountBefore},
  \texttt{nthClassVisitTime}, singleton equivalences
  (\texttt{BridgeCore.lean:45--107})
\item Key bridge lemma: \texttt{nthVisitTimeExists\_of\_StrongRecurrenceInClass}
  (\texttt{BridgeCore.lean:323})
\item Process-level exchangeability: \texttt{exchangeable\_rowProcess\_restrict\_class}
  (\texttt{PEBridge.lean:2097})
\item Finite-sum decomposition: \texttt{rowProcessLaw\_restrictClass\_eq\_finsetSum}
  (\texttt{BridgeCore.lean:2245})
\item Class-level row-process Ces\`aro bridge:
  \texttt{rowProcessCoordwiseCesaroLimit\_restrictClass\_of\_directingRowKernel\_of\_exchangeable}
  (\texttt{FiberEventBridge.lean:6902})
\item Class-level finite-coordinate factorization:
  \texttt{rowProcessLaw\_restrictClass\_factorizes\_directingRowKernel}
  (\texttt{FiberEventBridge.lean:7142})
\end{itemize}

\noindent\textbf{Crown theorem status (2026-04):} The row-level class-restricted
infrastructure is now fully formalized with 0 sorries. The class-restricted
factorization theorem threads through the cylinder-mixing and mixture-reconstruction
assembly via the surface theorems in \texttt{MixtureRepresentation.lean}. This
enables periodic chains and multi-component Markov systems to use the full
representation theorem, not just its row-process subtheory.

\subsubsection{API Usability: The \texttt{MarkovMixture} Interface}
...
\item \textbf{Hidden Markov models}: Partial exchangeability for observation
  sequences given latent state processes. This would formalize the Baum-Welch
  setting within the de~Finetti framework.
