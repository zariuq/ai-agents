...
\item[\code{project\_core}.] A larger bilingual abstract grammar exported
  ...
  We additionally generate a theorem-usable \code{ProjectCoreSig.funsList},
  so the larger slice is not only regression-checked but also available to
  Lean as a kernel-reducible constructor surface.
...
\item theorem-level ambiguity separation on real witness families:
  the telescope and Anna pairs are distinct abstract readings, and the
  corresponding Lean patterns inhabit different native predicates
  (VP-attachment versus NP-attachment) in the same \code{S} fiber;
\item theorem-level bilingual preservation at the real GF$\to$IR layer:
  the recovered English and Czech abstract nodes agree reading-by-reading
  for both telescope and Anna witnesses;
\item theorem-level constructor-family facts over the larger
  \code{project\_core} slice, including unary crossings such as
  \code{ExistNP : NP \to Cl}, \code{QuestCl : Cl \to QCl}, and
  \code{UsePron : Pron \to NP};
\item theorem-level native-type classification of larger real patterns,
  including existential sentences, interrogatives (\code{QS}), and
  progressive clauses, all still on the syntax-only lane;
