...
\subsection{Fortini Bridge Interface (Current Lean Surface)}
In the current Lean development, we now separate the public theorem
surface from internal proof plumbing.  The canonical path is:
\begin{enumerate}
\item \texttt{BuildRowKernelOnRecurrentExtension}: on a recurrent extension
$P$, construct a row-kernel family with the exact local data needed by
the crux;
\item \texttt{RecurrentLatentCoherenceBridgeTheorem}: package the
recurrent extension plus constructed row-kernel data in one interface;
\item \texttt{fortiniSuccessorMatrixInvarianceTheorem\_of\_recurrentLatentCoherenceBridge}:
derive the Markov mixture representation.
\end{enumerate}

This cleanup removes redundant aliases and keeps a single theorem route.
\textbf{Status (April 2026):} The formalization is now \emph{complete}
with 0~sorry across 23,899~lines.  The crown theorem
\texttt{startRestrictedRowKernelData\_directingRowKernel} establishes
that the canonical directing kernel (from the i.i.d.\ de~Finetti
machinery) satisfies the start-restricted row-kernel data conditions,
closing the derivation gap.  No ad-hoc assumptions remain.

What this adds over the existing binary $\nuPLN$ chain is a genuine
Markov-memory layer.  The theorem
\texttt{fortini\_and\_solomonoff\_of\_recurrentLatentCoherenceBridge}
now provides, for the same finite-alphabet prefix law $\mu$, both:
\begin{enumerate}
\item a Fortini-style Markov-mixture representation, and
\item a Solomonoff $M_2$ finite-horizon log-loss bound
(\texttt{FiniteAlphabet.FiniteHorizon.relEntropy} against
\texttt{FiniteAlphabet.SolomonoffBridge.M2}).
\end{enumerate}
This is stronger than the earlier exchangeable-binary collapse
(\texttt{solomonoff\_exchangeable\_predictBit\_same\_counts}), and
orthogonal to the world-model calculus layer, which provides typed
rewrite infrastructure but not Markov latent-mixture semantics nor
Solomonoff regret guarantees.

\subsection{Categorical de Finetti in Kleisli(Giry)}
\label{sec:categorical-definetti}

The measure-theoretic de~Finetti theorem of Section~\ref{sec:wip} operates at
the level of individual probability measures.  A natural categorical question is
whether the mixing representation extends to a \emph{universal property}: does
the i.i.d.\ sequence kernel $\theta \mapsto \text{iid}(\theta)$ define a limit
cone in the Kleisli category of the Giry monad?

We formalize this question in \texttt{Kleisli(MeasCat.Giry)} and obtain a
complete answer comprising both positive and negative results.

\paragraph{Setup.}
The \emph{global finitary diagram} is the diagram of finitary permutation
automorphisms of $\text{Bool}^{\mathbb{N}}$ in $\text{Kleisli}(\text{Giry})$.
A \emph{cone} over this diagram is a Kleisli morphism $\kappa : A \to
\text{Bool}^{\mathbb{N}}$ that commutes with all finitary permutations.  The
i.i.d.\ sequence kernel $\text{iid} : [0,1] \to \text{Bool}^{\mathbb{N}}$
forms such a cone.

\paragraph{Markov-only universal property (proven).}
For any cone $\kappa$ whose underlying kernel is \emph{Markov} (i.e.\ a
probability kernel), there exists a unique mediating Kleisli morphism through
$\text{iid}$.  This follows from the injectivity of the moment embedding
$\theta \mapsto (\theta^n)_{n \geq 1}$ on probability measures on $[0,1]$,
established via the Hausdorff moment theorem.
...
\paragraph{Unrestricted universal property (refuted).}
The fully unrestricted all-sources Kleisli mediator property---requiring a
unique mediator for \emph{every} commuting Kleisli morphism, including
non-probability measures---is \textbf{false}.  The counting measure on
$\text{Bool}^{\mathbb{N}}$ (from $\text{PUnit}$) commutes with all finitary
permutations, but every singleton $\{\omega\}$ has $\text{iid}(\theta)$-measure
zero for all $\theta \in [0,1]$ while the counting measure assigns mass~1.
Therefore no mediator through $\text{iid}$ can exist.
...
\paragraph{Finite-mass equivalence (the maximal correct strengthening).}
Restricting the source cones to \emph{finite measures} (not necessarily
probability measures) recovers equivalence with the Markov-only version:
every finite-mass commuting kernel is a scalar multiple of a probability
kernel, and the moment-embedding injectivity argument applies after
normalization.
...
\paragraph{Solomonoff connection.}
The categorical endpoint connects directly to the Solomonoff--de~Finetti
bridge of Section~\ref{sec:wip}: for a restricted Solomonoff prior $M$ with
total-output programs and normalized root mass, the induced measure is
exchangeable, and the categorical de~Finetti machinery yields both the
\texttt{nupln\_master\_chain} conclusion and a unique latent-$\theta$ mediator.
...
All theorems above are fully mechanized in Lean~4 with zero sorries and zero
axioms.  The formalization spans approximately 4500 lines in
\texttt{DeFinettiKleisliGirySkeleton.lean} plus export/counterexample files.
