...
(iv) executable rewrite engines (specialized and generic) proven sound with respect
to a declarative reduction specification;
...
(vii) a \emph{Beck--Chevalley analysis} of substitution as change-of-base, with a
proven counterexample showing the strong condition fails; and
(viii) a MeTTa Core interpreter specification with confluence and progress.
...
\item A \emph{Beck--Chevalley analysis} of the COMM rule as change-of-base along the
  ...
  Beck--Chevalley condition does not hold for the constructor fibration.
\item An executable reduction engine (\code{reduceStep}) handling COMM, DROP, and PAR
  rules, with a machine-checked soundness theorem.
\item A declarative reduction relation (\code{DeclReduces}) independent of the engine,
  with proven soundness and completeness of the generic engine.
\item \emph{Four} \OSLF{} instantiations validating generality:
  \rhocalc{}-calculus, lambda calculus, Petri nets, and TinyML (a multi-sort CBV
  $\lambda$-calculus with booleans, pairs, and thunks).
...
\begin{lstlisting}[caption={Generic rewrite step (Match.lean)}]
def applyRule (rule : RewriteRule) (term : Pattern)
    : List Pattern :=
  if rule.premises.isEmpty then
    (matchPattern rule.left term).map
      fun b => applyBindings b rule.right
  else []

def rewriteStep (lang : LanguageDef) (term : Pattern)
    : List Pattern :=
...
The file \codepath{MeTTaIL/DeclReduces.lean} provides a declarative inductive reduction
relation independent of the executable engine.  The engine is proven both \emph{sound}
and \emph{complete} with respect to this specification (0~sorries).
...
\section{$\pi$-Calculus and \rhocalc{}-Encoding}

The formalization includes a $\pi$-calculus (Syntax, Reduction,
StructuralCongruence, MultiStep) and the encoding $[\![\cdot]\!] :
\pi \to \rhocalc{}$ following Lybech~\cite{lybech-encoding}.  The encoding
covers all 6~$\pi$-calculus constructors (Nil, Out, In, Par, Repl, Restrict).

\begin{itemize}
\item \textbf{Proposition 2} (substitution invariance of the encoding):
  \emph{fully proven}, 0~sorries, 0~axioms.
\item \textbf{Proposition 4, forward direction} (operational correspondence):
  \emph{fully proven for the restriction-free fragment}, 0~sorries.
  A restriction-free structural congruence (RFSC) and restriction-free
  reduction relation (\code{ReducesRF}) provide a self-contained fragment
  where the STRUCT case is provable.  See the companion paper
  \emph{Verified Forward Simulation for the $\pi \to \rhocalc$ Encoding}
  for details.
\item \textbf{Propositions 1, 3, 5} and full Proposition~4:
  partial, sorries remain in auxiliary correctness proofs.
\end{itemize}
...
\item[\checkmark] \textbf{Milestone 6}: Categorical lift.
  \code{langModalAdjunction}: Galois connection $\to$ Mathlib \code{Adjunction}.
  \code{predFibration}: \code{SubobjectFibration} over \code{Discrete R.Sorts}.
  Unconditional specialized engine bridges (\code{specialized\_possibly}, etc.).

\item[\checkmark] \textbf{Milestone 7}: Constructor category and GSLT bridge.
  ...
  \code{ModalEquivalence}: constructor change-of-base $\leftrightarrow$ modalities.
  \code{DerivedTyping}: typing rules derived from constructor classification.
  \code{BeckChevalleyOSLF}: substitution as change-of-base, with counterexample.
  All 0~sorries.
\end{enumerate}
