...
from rewrite rules.  Our formalization spans 41{,}700+ lines across 118 Lean files
(89 in the core \OSLF{} tree, 29 in the process-calculi layer),
...
\code{Framework/}      & 14{,}708 & 0  & Abstract \OSLF{} + categorical bridge + 6 instances \\
\code{OSLF/RhoCalculus/} &    60 & 0  & \OSLF{} \rhocalc{}-calculus facades \\
\code{NativeType/}     & 1{,}444 & 0  & Native type construction + Grothendieck \\
...
\hline
\textbf{Total}         & \textbf{41{,}729} & \textbf{0} & \textbf{0 sorries, 0 axioms}
\end{tabular}
\end{center}
...
\end{enumerate}
Additionally, a full Grothendieck construction $\mathit{NT}(T) = \int \mathrm{Sub}$
over $P(T)$ (packaging pairs $(P, \varphi)$ with $P \in P(T)$ and
$\varphi \in \mathrm{Sub}(P)$) would complete the connection to
Native Type Theory~\cite{WilliamsStay2021}.  Mathlib provides
\code{CategoryTheory.Grothendieck} for $\mathcal{C} \to \mathbf{Cat}$ functors,
which could be instantiated with the subobject fibration.
...
maps 27 verifiable claims from three source papers to Lean theorem names:
...
NTT.pdf & Thm 23 & Full presheaf Grothendieck category & proven \\
NTT.pdf & Prop 12 & Scoped $\leftrightarrow$ full comparison & proven \\
NTT.pdf & \S 4 & Internal language: $\top/\bot/\land/\lor$ & proven \\
NTT.pdf & \S 4 & Frame-derived $\to$/$\lnot$ & proven \\
NTT.pdf & Prop 19 & $\Pi$/$\Sigma$ type formation & assumption-scoped \\
...
\item \code{types\_nonempty\_necessary\_for\_piSigma}: for empty families,
  $\bigsqcap \emptyset = \top$ and $\bigsqcup \emptyset = \bot$, so
...
spanning 41{,}700+ lines of Lean~4 across 118 files with 0~sorries and 0~custom axioms.
...
internal-language bridge (Section~\ref{sec:paper-parity}).  The remaining
frontier item---lifting Set-level modalities into genuine internal $\Omega \to
\Omega$ natural transformations in $P(T)$---is precisely characterized in
Section~\ref{sec:topos-scope}.
...
All eight \emph{core} implementation milestones are complete and sorry-free.
...
machine-checked endpoint.  The strict paper-claim tracker
