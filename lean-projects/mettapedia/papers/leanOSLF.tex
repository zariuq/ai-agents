...
Native Type Theory papers explicitly list open directions that remain outside the
current core claim surface, including:
\begin{itemize}
\item internal conjunction/disjunction completion in the paper-level topos route
  (\emph{``Conjunction. TBD'' / ``Disjunction. TBD''} in \code{oslf.pdf}),
\item additional theory-translation conditions preserving \(\Pi\)/\(\Omega\) in the
  Native Type Theory setting (flagged as future work in
  \code{Native\_Type\_Theory.pdf}),
\item larger extensions such as dependent/parametric, stochastic, and quantum
  variations listed as future-work directions in the source papers.
\end{itemize}
Accordingly, this document distinguishes \emph{verified core endpoints} from
\emph{research-frontier extensions}.
...
The seven \emph{core} implementation milestones are complete and sorry-free, while
assumption-audit and literature-alignment items are tracked explicitly as unresolved
in \code{Framework/FULLStatus.lean}.
