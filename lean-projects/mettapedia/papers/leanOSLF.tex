...
from rewrite rules.  Our formalization spans 37{,}300+ lines across 111 Lean files
(82 in the core \OSLF{} tree, 29 in the process-calculi layer),
...
\item complete Grothendieck/fibered internal-language packaging for the Native
  Type Theory route beyond the current theorem-level translation endpoint bundles,
...
\code{Framework/}      & 12{,}301 & 0  & Abstract \OSLF{} + categorical bridge + 6 instances \\
...
\code{Main/CoreMain}   &   824   & 0  & Focused \OSLF{} re-exports \\
\hline
...
\hline
\textbf{Total}         & \textbf{37{,}353} & \textbf{0} & \textbf{0 sorries, 0 axioms}
\end{tabular}
\end{center}
...
The \code{Framework/} directory (12{,}301 lines, 25~files) is the largest component.
Key files:
...
\hline
\code{PiRhoCanonicalBridge.lean}  & 2{,}081 & $\pi \to \rhocalc{}$ canonical bridge \\
\code{CategoryBridge.lean}        & 1{,}436 & Presheaf topos, Yoneda, $\Omega$ classifier \\
...
\code{LanguageMorphism.lean}      &   280 & Language translation infrastructure \\
...
The file \codepath{Framework/CategoryBridge.lean} (1{,}436 lines, 0~sorries)
...
spanning 37{,}300+ lines of Lean~4 across 111 files with 0~sorries and 0~custom axioms.
...
fiber characterization (Section~\ref{sec:topos}).  The remaining gap---lifting
the Set-level modalities into genuine internal operations of $P(T)$ and
instantiating the full Grothendieck construction of
Native Type Theory~\cite{WilliamsStay2021}---is precisely characterized in
...
The seven \emph{core} implementation milestones are complete and sorry-free, and
assumption-audit/literature-alignment frontier items tracked in
\code{Framework/FULLStatus.lean} are now formalized as theorem-level endpoints in
the current strict tracker.
