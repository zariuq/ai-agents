...
\code{Main.lean}       &   387   & 0  & Focused OSLF re-exports \\
\hline
\textbf{Total}         & \textbf{22{,}320} & \textbf{29} & \textbf{Core: 0 sorries}
\end{tabular}
\end{center}

This table is a historical snapshot.  The process-calculus branch has evolved
substantially since this draft (including derived non-RF forward/admin endpoints,
branch-sensitive backward reflection infrastructure, and canonical pre-OSLF
package exports).  For the current theorem-level status of the
$\pi \to \rhocalc$ embedding, see:
\begin{quote}
