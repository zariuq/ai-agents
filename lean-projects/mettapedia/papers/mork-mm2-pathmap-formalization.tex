...
\large Syntax, Space Semantics, Trie Data Structure, Zipper Execution Model,\\
...
semantics, a bridge to MQ-calculus COMM non-determinism,
a Z3 oracle semantics module,
an invertible compat-head fragment compiler for PeTTa evaluation-during-matching,
and a direct-exec surface packaging scheduler-level exactness.
%
The \PathMap{} library provides a six-class zipper typeclass hierarchy
mirroring the Rust \texttt{pathmap::zipper} trait stack, with a Zipper Abstract
Machine (ZAM) soundness theorem, a complete OSLF language instance with
NTT soundness covering all four algebraic operations via 10~conditional
rewrite rules, PLN \texttt{BinaryEvidence} bridges with Knuth--Skilling
valuations, Solomonoff semimeasure weighting, and a \texttt{PathMapValuation}
typeclass with counting and weighted instances.
%
A conformance test suite of kernel-checked theorems is verified against
the \MORK{} CLI (\texttt{mork run}) on 9+ \texttt{.mm2} test programs.
The \MORK{}/MM2 development comprises 12{,}373 lines across 19 files with
0~sorries and 0~custom axioms.
The \PathMap{} library adds 7{,}165 lines across 27 files with
160+~theorems and 40+~instances, including a Huet zipper with
navigation round-trips, catamorphism/anamorphism framework,
graft correctness, and a candidate-selection architecture for
\HE{} backends.
The combined development totals 19{,}538 lines across 46 files.
...
\begin{enumerate}[nosep]
  ...
  \item A \PathMap{} zipper typeclass hierarchy with six classes
        (\texttt{ZipperMoving}\ldots\texttt{ZipperComplexity}) mirroring
        the Rust \texttt{pathmap::zipper} trait stack, with ZAM
        (Zipper Abstract Machine) soundness (\S\ref{sec:zipper});
  \item A complete \PathMap{} OSLF language instance with 10~conditional
        rewrite rules and NTT completeness biconditionals for all four
        algebraic operations (\S\ref{sec:oslf});
  \item PLN \texttt{BinaryEvidence} bridges from \PathMap{} stores with
        Knuth--Skilling valuations, Solomonoff semimeasure weighting,
        and a \texttt{PathMapValuation} typeclass (\S\ref{sec:evidence});
  \item Z3 oracle semantics: \texttt{OracleEnv}, payload-to-space
        conversion, and oracle-level pattern matching (\S\ref{sec:z3oracle});
  \item An invertible compat-head fragment formalizing the compilation
        of PeTTa evaluation-during-matching to standard MORK three-phase
        execution via staged inverse compilation (\S\ref{sec:invertiblehead});
  \item A direct-exec surface packaging scheduler-level exactness with
        PeTTa eval fragment composition (\S\ref{sec:directexec});
...
\midrule
\texttt{Syntax.lean}              & \texttt{Sink}, \texttt{FoldAggregator}, \texttt{ExecRule}, \texttt{SourceFactor}, \texttt{InputSpec} & 318 \\
...
\texttt{ThreePhaseRefinement.lean}& Sink persistence, phase$\leftrightarrow$scheduler, exactness & 519 \\
\texttt{WorkQueueOrder.lean}      & \texttt{atomKey}, shortlex ordering, location-based exec ordering & 561 \\
...
\texttt{MeTTaILBridge.lean}       & Multi-premise bridge, \texttt{PremiseChain}, ext bridge & 1{,}430 \\
\texttt{PLNRules.lean}            & Concrete PLN rules, guarded bridge instantiation & 378 \\
...
\texttt{CollectionBridge.lean}    & Zipper/lens \texttt{congElem} bridge, Layer~A--H theorems & 638 \\
\texttt{ExtendedBridge.lean}      & Extended bridge for \texttt{.subst} + rest-variable resolution & 83 \\
\texttt{ExecutionBoundary.lean}   & \texttt{morkTranslatable} + scheduler + source + ext re-exports & 510 \\
\texttt{Z3Oracle.lean}            & Z3 oracle semantics, payload-to-space, conformance & 347 \\
\texttt{DirectExecSurface.lean}   & Scheduler exactness package, PeTTa fragment bridge & 234 \\
\texttt{InvertibleHead.lean}      & PeTTa compat-head compilation, staged inverse soundness & 2{,}766 \\
\midrule
\textbf{Total} & & \textbf{12{,}373} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{\PathMap{} library files.}
\label{tab:pathmapfiles}
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{File} & \textbf{Key Content} & \textbf{Lines} \\
\midrule
\multicolumn{3}{@{}l}{\textit{Core Algebraic Theory}} \\
\texttt{Core.lean}              & \texttt{AlgebraicResult}, lattice hierarchy, \texttt{Finset} instance & 440 \\
\texttt{CoinductiveTrie.lean}   & \texttt{CTrie V}, pointwise operations, bisimilarity laws & 226 \\
\texttt{FiniteTrie.lean}        & \texttt{FTrie V}, operations, \texttt{upsertChild}, \texttt{graftAtPath} & 470 \\
\texttt{TrieRefinement.lean}    & \texttt{Sorted}, \texttt{join\_lookup}, \texttt{toCTrie} & 243 \\
\texttt{TriePathMapInstance.lean} & \texttt{PathMapQuantale (FTrie V)}, \texttt{JoinIdem}, \texttt{MeetIdem} & 129 \\
\midrule
\multicolumn{3}{@{}l}{\textit{Zipper Hierarchy and ZAM}} \\
\texttt{Zipper.lean}            & 6-class typeclass hierarchy (\texttt{ZipperMoving}\ldots\texttt{ZipperComplexity}) & 482 \\
\texttt{ZipperExecution.lean}   & \texttt{ZipperIterationSound}, ZAM soundness & 216 \\
\texttt{FlatZipperInstance.lean} & Reference list-cursor zipper, all contracts proved & 117 \\
\texttt{TrieZipper.lean}        & \texttt{SimpleTrieZipper V}, trie-backed soundness & 148 \\
\texttt{ZamContracts.lean}      & ZAM soundness contracts, compositionality & 193 \\
\texttt{HuetZipper.lean}        & Huet zipper, round-trip, \texttt{focus\_lookup\_general} & 324 \\
\midrule
\multicolumn{3}{@{}l}{\textit{Morphisms and Correctness}} \\
\texttt{Morphisms.lean}         & \texttt{cata}/\texttt{cataChildren}, count/depth/hash algebras & 149 \\
\texttt{Anamorphism.lean}       & \texttt{ana}/\texttt{anaChildren}, singleton coalgebra & 81 \\
\texttt{MorphismCorrectness.lean} & \texttt{cata\_recCount}, \texttt{cata\_rebuild\_lookup} & 167 \\
\texttt{GraftCorrectness.lean}  & \texttt{setValueAt\_lookup}, \texttt{ftrieWritingSpec\_root} & 87 \\
\texttt{SortedPreservation.lean} & \texttt{singleton\_lookup\_ne}, sortedness infrastructure & 68 \\
\texttt{UnitBridge.lean}        & \texttt{FTrie Unit} $\leftrightarrow$ path sets, \texttt{fromPathList} & 132 \\
\midrule
\multicolumn{3}{@{}l}{\textit{OSLF Instance and Evidence Bridges}} \\
\texttt{OSLFInstance.lean}       & \texttt{pathMapLang}, 10 rewrite rules, NTT completeness & 853 \\
\texttt{RelationBridge.lean}    & \texttt{RelationalSpace} typeclass, \texttt{toRelationEnv} & 200 \\
\texttt{PLNBridge.lean}         & \texttt{finsetPathEvidence}, K\&S sum rule, disjoint additivity & 275 \\
\texttt{SolomonoffBridge.lean}  & Weighted evidence, Solomonoff posterior additivity & 239 \\
\texttt{Measure.lean}           & \texttt{PathMapValuation}, counting/weighted instances & 143 \\
\texttt{WorldModelBridge.lean}  & \texttt{BinaryWorldModel} via \texttt{Multiset} lifting & 151 \\
\midrule
\multicolumn{3}{@{}l}{\textit{HE Backend Bridge}} \\
\texttt{ZipperWritingLaws.lean} & In-place ops, \texttt{InPlaceStatusSpec} & 118 \\
\texttt{HEBridge.lean}          & \texttt{supportToTrie}, \texttt{same\_support\_same\_trie} & 126 \\
\texttt{HEInterface.lean}       & Monad morphism chain, \texttt{SpaceQuerySupport} & 152 \\
\texttt{CandidateArchitecture.lean} & Two-phase correctness, zero-crossing, invalidation & 481 \\
\midrule
\textbf{Total} & & \textbf{7{,}165} \\
\bottomrule
\end{tabular}
\end{table}
...
The work-queue scheduler (\texttt{WorkQueueExec.lean}, 1{,}192~lines)
...
\texttt{WorkQueueOrder.lean} (561~lines) defines the
...
\paragraph{Byte-order refinement.}
\texttt{PathMapByteOrderRefinement.lean} closes the gap noted in
\texttt{WorkQueueOrder.lean}: ``This is NOT byte-identical to PathMap
serialization.''  On the scheduler exec-location fragment
(\texttt{schedulerLocFragment}: two-symbol expressions), the theorem
\texttt{scheduler\_byte\_faithful} proves that the abstract ordering
\texttt{lexLt} $\circ$ \texttt{atomKey} agrees with the concrete PathMap
byte-order traversal used by the Rust \texttt{metta\_calculus}.
The companion theorem \texttt{scheduler\_pop\_order\_correct} says:
if \texttt{locLt loc\textsubscript{1} loc\textsubscript{2} = true}, then the
abstract key ordering confirms loc\textsubscript{1} is popped first.
The unsupported remainder (nested expressions, non-ASCII, length $\geq 64$)
...
\texttt{ThreePhaseRefinement.lean} (519~lines) proves that atoms persist
through sink pipelines unless explicitly removed, and provides
phase$\leftrightarrow$scheduler exactness theorems
(\texttt{base\_step\_exactness}, \texttt{unfold\_step\_exactness},
\texttt{fold\_step\_exactness}):
...
%% ============================================================
\section{PathMap Zipper Hierarchy and ZAM Soundness}
\label{sec:zipper}
%% ============================================================

The Rust \texttt{pathmap} crate exposes a \emph{zipper}~\cite{Huet1997}
cursor model:
a cursor navigates a byte-indexed trie, reading or writing values at the
current focus, and advancing via depth-first
iteration~\cite{McBride2001}.
The Lean formalization mirrors this trait hierarchy with a six-class
typeclass stack (\texttt{Zipper.lean}, 482~lines):

\begin{lstlisting}[caption={Zipper typeclass hierarchy (excerpt).}]
...
class ZipperBounded (Z : Type*) [ZipperMoving Z] : Prop where
  ascend_none_at_root : @$\forall$@ z, atRoot z = true -> ascend z = none
...
class ZipperIteration (Z : Type*) [ZipperMoving Z] where
  toNextVal       : Z -> Z @$\times$@ Bool
  descendLastPath : Z -> Z @$\times$@ Bool
...
\noindent
The key invariant, encoded by \texttt{ZipperBounded}, is that
\texttt{ascend} never moves above the zipper's root.
The dual property \texttt{ZipperLiveness} asserts that ascending
from a non-root position always succeeds.
\texttt{ZipperComplexity} formalizes O($k$)/O(depth) structural depth
bounds---necessary preconditions for the time-complexity claims in the
Rust implementation.

\subsection{ZAM Soundness}

\texttt{ZipperExecution.lean} (216~lines) connects the zipper cursor model
to the OSLF engine's \texttt{RelationalSpace} query interface.
The central inductive predicate \texttt{ZipperReachableValue} defines which
values a depth-first iteration can reach:

\begin{lstlisting}[caption={Reachability and soundness contract.}]
...
class ZipperIterationSound (Z V : Type*) ... : Prop where
  reachable_in_store : @$\forall$@ root v, atRoot root -> Reachable root v
      -> v @$\in$@ allValues root
  store_in_reachable : @$\forall$@ root v, atRoot root -> v @$\in$@ allValues root
      -> Reachable root v
\end{lstlisting}
...
\begin{theorem}[\texttt{zam\_oslf\_sound}]
If a \texttt{ZipperSpace} agrees with a flat \texttt{RelationEnv}, then
the OSLF reduction relation is identical for both:
$\texttt{langReducesUsing}\;\mathit{zs}\;\mathit{lang}\;p\;q
\iff
\texttt{langReducesUsing}\;\mathit{flat}\;\mathit{lang}\;p\;q$.
\end{theorem}

\begin{theorem}[\texttt{zam\_diamond\_sound} / \texttt{zam\_box\_sound}]
The modal operators $\Diamond$ and $\Box$ are preserved by the
zipper-to-flat correspondence.
\end{theorem}
...
\begin{itemize}[nosep]
  \item \texttt{FlatZipper}~$\alpha$ (\texttt{FlatZipperInstance.lean},
        117~lines): a degenerate list-cursor serving as reference implementation.
  \item \texttt{SimpleTrieZipper V} (\texttt{TrieZipper.lean}, 148~lines):
        wraps \texttt{FTrie V} with an entries list, proving
        \texttt{ZipperIterationSound}.
\end{itemize}

\noindent
\texttt{ZamContracts.lean} (193~lines) provides compositionality theorems
for transferring ZAM soundness across cursor transformations.

%% ============================================================
\section{PathMap OSLF Language Instance}
\label{sec:oslf}
%% ============================================================

\texttt{OSLFInstance.lean} (853~lines) instantiates the OSLF pipeline for
the \PathMap{} algebraic language, yielding complete NTT (Native Type Theory)
soundness theorems~\cite{WilliamsStayNTT2021}.
The language definition \texttt{pathMapLang} has:
\begin{itemize}[nosep]
  \item \textbf{Types}: one sort \texttt{"V"} for \PathMap{} node values.
  \item \textbf{Constructors}: four algebraic operations (\texttt{PJoin},
        \texttt{PMeet}, \texttt{PSubtract}, \texttt{PRestrict}) plus four
        result constructors (\texttt{PUnion}, \texttt{PIntersect},
        \texttt{PDiff}, \texttt{PFilter}).
  \item \textbf{Rewrite rules}: 10~conditional rules gated by
        \texttt{relationQuery} premises encoding algebraic conditions:
\end{itemize}

\begin{center}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Premise} & \textbf{Meaning} & \textbf{Used by} \\
\midrule
\texttt{"sub" [x,y]}   & $x \subseteq y$          & PJoin, PMeet, PRestrict \\
\texttt{"nsub" [x,y]}  & $\neg(x \subseteq y)$    & PJoin \\
\texttt{"ndisj" [x,y]} & $x \cap y \neq \emptyset$ & PMeet \\
\texttt{"disj" [x,y]}  & $x \cap y = \emptyset$    & PSubtract \\
\bottomrule
\end{tabular}
\end{center}

\noindent
For each operation, the \texttt{AlgebraicResult} cases are exhaustively
covered:
\begin{itemize}[nosep]
  \item \textbf{Identity} (\texttt{self}/\texttt{other}): a rewrite rule fires,
        reducing to the operand.
  \item \textbf{Element}: a rewrite rule fires, reducing to the result
        constructor (e.g., \texttt{PUnion}).
  \item \textbf{None}: \emph{no rule fires}---the diamond is vacuously false.
\end{itemize}

\begin{theorem}[\texttt{pathMap\_pjoin\_complete} et al.]
For each of the four operations, the NTT completeness theorem is an exact
biconditional:
$\Diamond_{\mathit{relEnv}}\;\texttt{PJoin}(a,b)\;\phi
\;\iff\;$
the correct \texttt{AlgebraicResult}-determined rule fires.
All four theorems (\texttt{pjoin}, \texttt{pmeet}, \texttt{psubtract},
\texttt{prestrict}) are proved with 0~sorries.
\end{theorem}

\noindent
The \texttt{RelationBridge.lean} module (200~lines) provides the abstract
\texttt{RelationalSpace} typeclass that decouples the OSLF engine from
concrete backends, with a canonical embedding \texttt{toRelationEnv}.

%% ============================================================
\section{PathMap Evidence Bridges}
\label{sec:evidence}
%% ============================================================

Three modules connect \PathMap{} stores to probabilistic reasoning
frameworks, grounding the evidence semantics for
PLN~\cite{GoertzelPLN2009} and Solomonoff
induction~\cite{Solomonoff1964}.

\subsection{PLN BinaryEvidence (\texttt{PLNBridge.lean}, 275~lines)}

For a \PathMap{} store $W : \texttt{Finset}\;\alpha$ and query
$q : \texttt{Finset}\;\alpha$, define:
\[
  \texttt{finsetPathEvidence}\;W\;q \;=\; \bigl(\lvert W \cap q\rvert,\;
  \lvert W \setminus q \rvert\bigr)
\]
where $n^+ = \lvert W \cap q\rvert$ is positive evidence and
$n^- = \lvert W \setminus q\rvert$ is negative evidence.

\begin{theorem}[\texttt{evidence\_total} (K\&S sum rule)]
$n^+ + n^- = \lvert W \rvert$.
\end{theorem}

\begin{theorem}[\texttt{pjoin\_evidence\_additive}]
For disjoint $W_1, W_2$:
$\texttt{ev}(W_1 \cup W_2, q) = \texttt{ev}(W_1, q) \oplus \texttt{ev}(W_2, q)$.
\end{theorem}

\noindent
Additional theorems cover boundary conditions (\texttt{evidence\_empty\_store},
\texttt{evidence\_self\_query}), monotonicity (\texttt{evidence\_monotone}),
and Bayesian strength interpretation.

\begin{remark}[Honest scope]
\texttt{Finset} union is idempotent ($W \cup W = W$), so full additivity
fails.  A true \texttt{BinaryWorldModel} instance requires replacing
\texttt{Finset} with \texttt{Multiset} (allowing duplicates); this
extension is in \texttt{WorldModelBridge.lean}.
\end{remark}

\subsection{Solomonoff Weighting (\texttt{SolomonoffBridge.lean}, 239~lines)}

Generalizes counting evidence to arbitrary weight functions
$w : \alpha \to \mathbb{R}_{\ge 0}^{\infty}$:
\[
  \texttt{weightedPathEvidence}\;w\;W\;q \;=\;
  \Bigl(\sum_{p \in W \cap q} w(p),\;\sum_{p \in W \setminus q} w(p)\Bigr)
\]

\begin{theorem}[\texttt{solomonoffPathEvidence\_additive}]
Disjoint additivity holds for the Solomonoff semimeasure instantiation.
\end{theorem}

\begin{theorem}[\texttt{finsetPathEvidence\_eq\_uniform}]
Uniform weight $w \equiv 1$ recovers counting evidence.
\end{theorem}

\subsection{Valuations and World Model (\texttt{Measure.lean}, 143~lines;
\texttt{WorldModelBridge.lean}, 151~lines)}

\texttt{PathMapValuation}~$\alpha$ is a typeclass with Knuth--Skilling
axioms (zero store $\to$ zero weight, disjoint additivity):

\begin{lstlisting}[caption={PathMapValuation typeclass (excerpt).}]
class PathMapValuation (@$\alpha$@ : Type*) [DecidableEq @$\alpha$@] where
  weight : Finset @$\alpha$@ -> @$\mathbb{R}_{\ge 0}^{\infty}$@
  weight_empty : weight @$\emptyset$@ = 0
  weight_disjoint_add : Disjoint W1 W2 ->
    weight (W1 @$\cup$@ W2) = weight W1 + weight W2
\end{lstlisting}

\noindent
Two instances are proved: \texttt{countingPathMapValuation}
($\texttt{weight}\;W = \lvert W\rvert$) and \texttt{mkWeightedValuation}\;$w$
($\texttt{weight}\;W = \sum_{p\in W} w(p)$).
\texttt{WorldModelBridge.lean} lifts \texttt{Finset} evidence to
\texttt{Multiset}-based \texttt{BinaryWorldModel} instances where full
(non-disjoint) additivity holds.
...
\paragraph{Seeded rematch correctness.}
When the PathMap bridge provides candidate atoms for an imported space,
\CeTTa{} rematches each candidate natively with \texttt{space\_subst\_match\_with\_seed}.
The theorem \texttt{seedRematch\_extends\_seed} (\texttt{SeedRematchContract.lean})
proves that the resulting bindings extend the incoming seed from prior
conjunction steps---prior bindings are never dropped.  The companion theorem
\texttt{seedRematchSafe\_no\_loop} guarantees that binding loops are detected
and rejected before entering the conjunction accumulator.  Together with
\texttt{importedFallbackParity} from \texttt{ImportedRowContract.lean},
these establish that \CeTTa{}'s imported-backend path is binding-correct
under its current always-rematch policy.

\subsection{Candidate Selection, Support, and Counts}

Recent companion work on faithful backends for \HE{}~\MeTTa{} sharpens what
this bridge should and should not claim.  A trie such as \PathMap{} is
responsible for \emph{support-level} duties:
\begin{enumerate}[nosep]
  \item maintain the canonical support of stored atoms;
  \item narrow queries to sound candidate sets;
  \item remain unchanged under duplicate adds or non-last removes.
\end{enumerate}
It is \emph{not} the whole semantic carrier of a MeTTa space.  Bag-level
multiplicity remains a separate refinement layer.

The concrete selector development in
\texttt{CandidateArchitecture.lean} now proves:
\begin{enumerate}[nosep]
  \item \texttt{concrete\_twoPhase\_eq\_direct}: trie candidate selection
        followed by native exact matching agrees with direct querying;
  \item \texttt{trieCandidates\_subset\_support}: trie candidates are
        phantom-free;
  \item \texttt{realPrefixSelector\_sound}: real subtrie descent under an
        encoded prefix is sound for candidate narrowing;
  \item add/remove zero-crossing theorems: duplicate adds and non-last
        removes do not change trie support, while first adds and last
        removes do.
\end{enumerate}

\paragraph{Positive example.}
If a space stores two copies of the same atom, \PathMap{} should still hold
one canonical support element while a counted-space refinement records
multiplicity~2.

\paragraph{Negative example.}
Encoding multiplicity by inventing different trie keys for duplicates would
destroy the structural-sharing and canonical-support story that makes
\PathMap{} valuable in the first place.

This division of labor is useful for \MORK{} as well.  The trie should
perform prefix descent and support-level candidate narrowing; exact
variable-sensitive matching belongs in the native matcher that consumes those
candidates.  \texttt{PathMapMatcherInstance.lean} now makes that bridge
explicit by instantiating the abstract two-phase theorem with the concrete
\HE{} matcher.  The broader binder-hygiene and cache-correctness obligations
remain in the companion backend note rather than in this trie-focused paper.
...
The \MeTTaIL{} bridge (\texttt{MeTTaILBridge.lean}, 1{,}430~lines, plus
\texttt{CollectionBridge.lean}, 638~lines) connects \MORK{}'s execution
...
\texttt{PLNRules.lean} (378~lines, 23~theorems) instantiates the bridge for
...
\texttt{CollectionBridge.lean} (638~lines) and \texttt{AtomZipper.lean}
...
%% ============================================================
\section{Z3 Oracle Semantics}
\label{sec:z3oracle}
%% ============================================================

\texttt{Z3Oracle.lean} (347~lines, 27~theorems) formalizes the oracle seam
of the \MORK{} runtime, connecting abstract oracle queries to concrete Z3
behavior.
The Z3 subprocess interaction in the Rust runtime follows a three-step
protocol:
(i)~sink-side: buffer SMT-LIB statements and write to Z3 stdin;
(ii)~source-side: send \texttt{(check-sat)} and \texttt{(get-model)}, parse
the response;
(iii)~pattern-match against the parsed model using the existing
\texttt{matchOneInSpace} infrastructure.

\begin{definition}[\texttt{OracleEnv}]
An oracle environment is a pure function
$\texttt{OracleQuery} \to \texttt{OracleResponse}$
abstracting the Z3 subprocess lifecycle.
\end{definition}

\noindent
The key bridge function \texttt{oraclePayloadSpace} converts an oracle
response's payload atoms into a \texttt{Space} ($\texttt{Finset Atom}$),
enabling reuse of the existing pattern matching infrastructure:

\begin{theorem}[\texttt{oracleMatchPattern\_sound}]
Oracle pattern matching agrees with \texttt{matchOneInSpace} on the
payload space: a match against the oracle response succeeds iff it
succeeds against the flattened space.
\end{theorem}

\noindent
Oracle queries are routed by resource type (\texttt{z3}/\texttt{act}):

\begin{theorem}[\texttt{routeOracleQuery\_z3\_routes}]
A query with resource tag \texttt{"z3"} routes to the Z3 oracle
environment.
\end{theorem}

\noindent
A mock Z3 environment (\texttt{mockZ3Env}) with 7~conformance tests
validates the architecture against the \MORK{} Rust test
\texttt{sink\_z3\_basic}.

\begin{remark}[Honest scope]
This file does NOT model the Z3 subprocess lifecycle (spawn/kill),
SMT-LIB parsing, or PathMap zipper internals.  The oracle is a pure
function parameter---the implementation gap is the Lean--Rust FFI for
subprocess I/O.
\end{remark}

%% ============================================================
\section{Invertible Compat-Head Fragment}
\label{sec:invertiblehead}
%% ============================================================

\texttt{InvertibleHead.lean} (2{,}766~lines, 61~theorems) is the largest
module in the formalization.  It formalizes the compilation of PeTTa's
\emph{evaluation-during-matching} (\texttt{matchHeadArgWithEval} in the
PeTTa Dispatch module) to standard \MORK{} three-phase execution.

\subsection{Problem}

PeTTa's pattern matcher can \emph{evaluate} subterms during matching:
when matching \texttt{(f \$x)} against a query, PeTTa calls the
interpreter to evaluate \texttt{f} and matches the result against the
query.  \MORK{}'s \texttt{matchAtom} is purely structural---it cannot
evaluate during matching.

\subsection{Key Insight}

For a significant subfragment---head functions that are \emph{pure,
structurally recursive, and produce constructor-wrapped outputs}---the
evaluation-during-matching can be compiled to standard \MORK{}/MM2 by
\emph{reversing} the computation: instead of evaluating $f(x)$ forward
and matching, we reverse-match the query argument against $f$'s output
...
\begin{lstlisting}[caption={Core definitions (\texttt{InvertibleHead.lean}).}]
...
structure HeadFuncDef where
  ...
  consistent : @$\forall$@ eq @$\in$@ equations, eq.funcName = funcName

structure InvertibleHead (def_ : HeadFuncDef) : Prop where
  hasBase              : @$\exists$@ eq @$\in$@ def_.equations, eq.isBase
  varsCovered          : ...  -- RHS vars @$\subseteq$@ LHS vars
  basesAreCtorApps     : ...  -- base RHS = ctor application
  stepsWrapRecursion   : ...  -- step RHS wraps one recursive call
  lhsVarsRecoverableBase : ...  -- LHS vars recoverable from RHS
  lhsVarsRecoverableStep : ...  -- LHS vars recoverable from shell
\end{lstlisting}
...
\begin{enumerate}[nosep]
  \item \textbf{Forward evaluation} (\texttt{forwardEval}): fuel-bounded
        recursive evaluation following the defining equations.
  \item \textbf{Reverse matching}: \texttt{reverseMatchBase} decomposes a
        constructor application to recover LHS variables;
        \texttt{reverseMatchStep} strips the outer constructor shell and
        extracts the recursive sub-query.
  \item \textbf{Staged compilation}: the three-phase UNFOLD/BASE/FOLD
        architecture maps to: UNFOLD~=~strip outer shell and emit
        recursive sub-query + wait token; BASE~=~reverse-match base case;
        FOLD~=~reassemble original input from sub-result.
\end{enumerate}

The main soundness chain:

\begin{theorem}[\texttt{reverseEval\_inverts\_forwardEval}]
For a deterministic invertible fragment: if \texttt{forwardEval} on a
ground input produces output~\texttt{out}, then \texttt{reverseEval} on
\texttt{out} recovers the original input.
\end{theorem}

\begin{theorem}[\texttt{staged\_inverse\_sound}]
Every successful forward evaluation gives rise to a
\texttt{StagedInverseTrace}---a structured proof object recording the
UNFOLD/BASE/FOLD decomposition that recovers the original input.
\end{theorem}

\begin{theorem}[\texttt{staged\_inverse\_exec\_sound}]
The staged trace refines to a concrete scheduler-facing
\texttt{StagedInverseExecTrace}, provided explicit singleton witnesses
are supplied for each base/unfold/fold node.
\end{theorem}

\begin{remark}[Honest scope]
The bridge to \texttt{matchHeadArgWithEval} (PeTTa Dispatch module) is
not yet formalized.  The current scope covers the \emph{fragment boundary}
and \emph{compilation scheme soundness}; the connection to the PeTTa
surface language is future work.  Non-injective head functions are
excluded by the \texttt{InvertibleHead} predicate.
\end{remark}

%% ============================================================
\section{Direct-Exec Surface}
\label{sec:directexec}
%% ============================================================

\texttt{DirectExecSurface.lean} (234~lines) packages the scheduler-level
exactness theorems (Tasks~A/B/C from \texttt{ThreePhaseRefinement.lean})
into a single reusable \texttt{SchedulerSeam} and composes the first
real PeTTa fragment through it.

\subsection{Scheduler Seam}

\begin{definition}[\texttt{SchedulerSeam}]
A coherent family of re-exported theorems:
\begin{itemize}[nosep]
  \item \texttt{orderAgreement}: scheduler byte-key order agrees with
        shortlex on (priority, name).
  \item \texttt{baseExactness} / \texttt{unfoldExactness} /
        \texttt{foldExactness}: each phase's scheduler step equals the
        corresponding phase operation.
  \item \texttt{sourceRelation}: \texttt{fireExecFact} = source $-$ exec
        atom.
  \item \texttt{progress}: if \texttt{selectNextExec} returns \texttt{some},
        then \texttt{cWorkQueueStep} succeeds.
\end{itemize}
\end{definition}

\subsection{PeTTa Fragment Bridge}

The key theorem lands a PeTTa eval/evalc step on the MM2 scheduler:

\begin{theorem}[\texttt{pettaEvalStep\_schedulerFires}]
A PeTTa \texttt{evalStep} that translates to a \MORK{} exec rule fires
through the scheduler seam.
\end{theorem}

\begin{remark}[Honest scope]
This file explicitly lists what is NOT covered:
\texttt{add-atom}/\texttt{remove-atom} (MORK sinks ready, MeTTaIL-level
rule missing), collection-pattern matching (excluded by
\texttt{morkTranslatable}), and beta-reduction (\texttt{.subst} nodes
rejected).
\end{remark}
...
The conformance test suite (\texttt{Conformance.lean}, 1{,}694 lines)
...
\PathMap{} zipper              & 6-class typeclass hierarchy, ZAM soundness, 2 concrete instances & Mutation ops (\texttt{ZipperWriting}), product/overlay zippers, conflict detection \\
...
\PathMap{} OSLF instance       & \texttt{pathMapLang}, 10 rewrite rules, NTT completeness & --- \\
...
\MeTTaIL{} bridge              & Flat rules, multi-premise, freshness guards, \texttt{congElem} (zipper/lens), extended bridge (\texttt{.subst}/rest-var) & Source-level positional congruence \\
...
Execution boundary             & \texttt{morkTranslatable} packaging, pattern normalization, \texttt{morkTranslatable\_applyBindings} & --- \\
...
MM2 bridge round-trip          & \texttt{mm2\_raise\_lower}, \texttt{mm2\_lower\_injective}, \texttt{mm2\_raise\_surjective} (\texttt{Mm2BridgeRoundTrip.lean}, companion boundary result) & Context-sensitive lowering (ACT dual mapping) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{PathMap Rust coverage.}
    The \PathMap{} Lean formalization covers the algebraic semantics
    (\texttt{AlgebraicResult}, lattice/quantale hierarchy) and adds
    OSLF/evidence bridges not present in Rust.
    However, significant Rust subsystems are not formalized:
    \texttt{ZipperWriting} mutation operations (444~Rust LOC),
    paths serialization (19{,}000~Rust LOC),
    catamorphism/anamorphism morphisms (800~Rust LOC),
    the Arena-Compact on-disk format,
    merkleization/deduplication,
    product/overlay zipper composition,
    and concrete node type selection (Dense/Line/Tiny).
    The Lean formalization targets \emph{algebraic correctness and
    semantic soundness}, not full Rust implementation verification.

  \item \textbf{Invertible head scope.}
    The \texttt{InvertibleHead} predicate
    (\S\ref{sec:invertiblehead}) covers pure, structurally recursive
    functions with constructor-wrapped outputs and injective
    base cases.  Non-injective heads, multi-argument recursive calls,
    and the bridge to PeTTa's \texttt{matchHeadArgWithEval} are not
    yet formalized.

  \item \textbf{PathMap OSLF completeness.}
    The NTT completeness theorems (\S\ref{sec:oslf}) are exact
    biconditionals parameterized by a \texttt{relEnv : RelationEnv}.
    The connection from a concrete \PathMap{} store to a
    \texttt{RelationEnv} satisfying the required premises is handled by
    \texttt{RelationBridge.lean}, but automatic instantiation (given a
    concrete \texttt{Finset}) is not yet provided.

  \item \textbf{Zipper complexity.}
    \texttt{ZipperComplexity} (\S\ref{sec:zipper}) formalizes
    structural depth bounds (path length grows by at most~$k$).
    Actual time-complexity proofs require a cost model not available
    in Lean.
\end{enumerate}
...
\texttt{ExecutionBoundary.lean} (510~lines) provides a thin re-export
...
file-backed external datasets.
The Z3 oracle semantics are now partially formalized
(\texttt{Z3Oracle.lean}, \S\ref{sec:z3oracle}): query routing,
payload-to-space conversion, and pattern matching are proved, but the
subprocess lifecycle and SMT-LIB parsing remain unformalized.
ACT external files are not yet formalized.
...
%% ============================================================
\section*{Future Work: Multiset Semantics via Semiring Algebra}
\label{sec:future-multiset}
%% ============================================================

\PathMap{} implements set semantics: inserting the same expression twice
yields one copy, and algebraic operations collapse duplicates.  This is
by design for \MORK{}/MM2, where deterministic execution requires
deduplication.  However, \MeTTa{} has \emph{multiset} semantics: adding
the same atom twice creates two copies, and queries return one result
per copy.

\paragraph{Problem.}
When \PathMap{} backs ordinary \MeTTa{} spaces (not \MORK{}-managed MM2
spaces), the set semantics create a semantic mismatch.  \CeTTa{}
currently works around this via a parallel index mapping serialized
atoms to row-ID vectors, but this index does not compose with
\PathMap{}'s algebraic operations.

\paragraph{Approach A: Embedded count.}
Encode multiplicity in a CeTTa-owned counted key formed by
\[
  \texttt{atom\_expr\_bytes} \;\Vert\; \texttt{count\_expr\_bytes}
\]
where the count remains an ordinary serialized number expression.  This keeps
the atom bytes at the front of the key, preserving prefix scans for PathMap
narrowing.  Using the ordinary pair expression \texttt{(\$atom \$count)}
directly would \emph{not} preserve the atom prefix under the current MORK expr
serialization, because the outer arity tag appears before the atom payload.
On insert, look up the existing count and re-insert with the incremented value.
On query, decode the count and emit results accordingly.

\begin{itemize}[nosep]
  \item Works with current \PathMap{} API.
  \item Count rewrites remain a CeTTa-owned concern; algebra ops still require
    special handling.
\end{itemize}

\paragraph{Approach B: \texttt{PathMap<CountValue>} with semiring algebra.}
A cleaner architecture uses \texttt{PathMap<V>} with \texttt{V = CountValue}
(a newtype over \texttt{u32}).  This requires:

\begin{enumerate}[nosep]
  \item \textbf{Semiring \texttt{Lattice} instance for \texttt{CountValue}.}
    The current numeric \texttt{Lattice} implementations in
    \texttt{ring.rs} are placeholders that ignore the ``other'' operand.
    A proper multiset semiring requires:
    \begin{itemize}[nosep]
      \item \texttt{pjoin} (union): $\text{count}_a + \text{count}_b$
      \item \texttt{pmeet} (intersection): $\min(\text{count}_a, \text{count}_b)$
      \item \texttt{psubtract} (difference): $\max(0, \text{count}_a - \text{count}_b)$
    \end{itemize}

  \item \textbf{Query-level count multiplication for conjunctions.}
    Storage gets cleaner under this model, but query evaluation must
    still multiply counts across matched factors.
\end{enumerate}
Note that \texttt{join\_val\_at} is a convenience, not a strict blocker:
\PathMap{} already supports in-place insert-or-mutate via
\texttt{write\_zipper\_at\_path} (\texttt{trie\_map.rs:450}).

\paragraph{Theoretical foundation.}
Multiset (bag) semantics over annotated relations are characterized by
\emph{provenance semirings}~\cite{GreenProvenanceSemirings2007}.
Query evaluation is parameterized over a commutative semiring
$(\mathbb{K}, +, \times, 0, 1)$; for counting multiplicities, the
natural numbers $(\mathbb{N}, +, \times, 0, 1)$ suffice.  Join operations
correspond to multiplication (Cartesian product count), union to addition,
and projection sums multiplicities over projected-away witnesses.

See also Mumick et~al.~\cite{MumickMagicDuplicates1990} on bag semantics
in Datalog and Abiteboul--Hull--Vianu~\cite{AHV1995} on bag relational
algebra.

\paragraph{Implementation sketch.}
\begin{lstlisting}[caption={Semiring \texttt{Lattice} for multiset counts (Rust pseudocode).}]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct CountValue(u32);

impl Lattice for CountValue {
    fn pjoin(&self, other: &Self) -> AlgebraicResult<Self> {
        // Union = sum of counts
        AlgebraicResult::Element(CountValue(
            self.0.saturating_add(other.0)))
    }
    fn pmeet(&self, other: &Self) -> AlgebraicResult<Self> {
        // Intersection = min of counts
        let m = self.0.min(other.0);
        if m == 0 { AlgebraicResult::None }
        else { AlgebraicResult::Element(CountValue(m)) }
    }
}

impl DistributiveLattice for CountValue {
    fn psubtract(&self, other: &Self) -> AlgebraicResult<Self> {
        // Difference = saturating subtraction
        let d = self.0.saturating_sub(other.0);
        if d == 0 { AlgebraicResult::None }
        else { AlgebraicResult::Element(CountValue(d)) }
    }
}
\end{lstlisting}
...
\begin{itemize}[nosep]
  \item Multiset semantics compose with all \PathMap{} algebra operations
    (no parallel index synchronization).
  \item Conjunction multiplicity computes correctly: product of factor
    counts, per semiring multiplication.
  \item Clean separation: \MORK{}/MM2 continues using \texttt{PathMap<()>}
    (set semantics), while \MeTTa{} spaces use \texttt{PathMap<CountValue>}
    (multiset semantics).
\end{itemize}

\paragraph{Immediate CeTTa-owned path.}
Approach A is already enough to give CeTTa a mainline-compatible prototype
for \texttt{new-space pathmap}: one counted key per distinct atom, local
insert/remove-one rewrite, local query rematch, and local conjunction logic
that multiplies factor counts.  In that lane, duplicate row ids become a
derived compatibility view rather than stored truth, and the helper
\texttt{query\_multi\_with\_factor\_exprs} is no longer structurally required.
If CeTTa later chooses to store multiple key families in one shared \PathMap{},
an explicit discriminator byte can be added at that point; the simplest
fresh-space prototype does not require one.

\paragraph{Formalization opportunity.}
The Lean formalization could extend \texttt{PathMapQuantale} with a
\texttt{CountValue} instance and prove semiring laws distribute over
the \texttt{FTrie} operations.  The existing \texttt{AlgebraicResult}
infrastructure handles the zero-count $\to$ deletion case via the
\texttt{.none} variant.  This would provide mechanically-verified
multiset semantics for \PathMap{}-backed spaces within the Mettapedia
codebase.

%% ============================================================
\section{Storage vs Computation Cost Analysis}
\label{sec:cost-analysis}
%% ============================================================

This section formalizes when \PathMap{} storage should be preferred over
on-demand computation.  The theory is mechanized in Lean~4 with zero
\texttt{sorry} statements.

\subsection{Crossover Theorem}

For a function $f : \alpha \to \beta$ with finite domain $|\alpha| = n$:

\begin{itemize}
  \item \textbf{Storage cost}: $S = n \cdot \textit{entrySize}$ (upfront)
    plus $O(1)$ per lookup
  \item \textbf{Computation cost}: $O(T)$ per evaluation (no upfront cost)
\end{itemize}

\noindent
\textbf{Theorem} (\texttt{storage\_better\_iff}):
Storage beats computation after $q$ queries if and only if
\[
  S < q \cdot (T - 1)
\]
\noindent
Equivalently, storage wins when $q > S / (T - 1)$.

\begin{lstlisting}
theorem storage_better_iff (storageCost computeTime queries : Nat)
    (h : computeTime > 1) :
    totalCost_stored storageCost queries <
    totalCost_compute computeTime queries @$\leftrightarrow$@
    storageCost < queries * (computeTime - 1)
\end{lstlisting}

\subsection{Reverse Query Advantage}

\textbf{Theorem} (\texttt{reverse\_stored\_wins}):
For reverse queries (given output, find inputs), stored tables
\emph{always} beat enumeration:
\[
  |\{x : f(x) = y\}| \le |\alpha|
\]
\noindent
Proof: \texttt{Finset.card\_filter\_le}.

\subsection{Concrete Arithmetic Analysis}

For $n$-bit integers with domain size $N = 2^n$:

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Operation} & \textbf{Compute/Query} & \textbf{Table Size} & \textbf{Crossover} \\
\midrule
Addition     & $O(n)$    & $N^2 \cdot (n+1)$ bits & $\approx N^2$ queries \\
Multiplication & $O(n^2)$ & $N^2 \cdot 2n$ bits & $\approx N^2 / n$ queries \\
Division     & $O(n^2)$  & $N^2 \cdot n$ bits  & $\approx N^2 / n$ queries \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{8-bit arithmetic ($n = 8$, $N = 256$).}
\begin{itemize}
  \item Addition table: $256^2 \times 9 \approx 590$ KB; compute: 8 ops/query
  \item Multiplication table: $256^2 \times 16 \approx 1$ MB; compute: 64 ops/query
  \item \textbf{Recommendation}: Storage is practical; multiplication benefits most
\end{itemize}

\paragraph{16-bit arithmetic ($n = 16$, $N = 65536$).}
\begin{itemize}
  \item Addition table: $65536^2 \times 17 \approx 73$ GB --- \emph{impractical}
  \item Multiplication table: $65536^2 \times 32 \approx 137$ GB --- \emph{impractical}
  \item \textbf{Recommendation}: Always compute on demand
\end{itemize}

\subsection{Trie vs Flat Table}

For tries (as used by \PathMap{}), lookup cost is $O(d)$ where $d$ is path depth,
versus $O(1)$ for flat hash tables.  The trade-off:

\begin{lstlisting}
theorem trie_wins_when_cost_bounded
    (flatSize trieNodes depth queries : Nat)
    (h_depth : depth @$\ge$@ 1)
    (h_trie_smaller : trieNodes < flatSize)
    (h_cost_bounded : queries * (depth - 1) < flatSize - trieNodes) :
    trieQueryCost trieNodes depth queries <
    flatTableQueryCost flatSize queries
\end{lstlisting}

\noindent
\textbf{Guidance}: Trie storage wins when paths share prefixes (reducing
\texttt{trieNodes}) and depth is small.  For \PathMap{}-backed spaces with
highly structured atoms (many shared prefixes), this is typically satisfied.

\subsection{Formalized Files}

The cost analysis is mechanized in:
\begin{itemize}
  \item \texttt{Mettapedia/Complexity/FiniteFunctionStorage.lean} --- core crossover theory
  \item \texttt{Mettapedia/Complexity/SymbolicComputation.lean} --- arithmetic costs
  \item \texttt{Mettapedia/Complexity/PathMapCost.lean} --- trie integration
\end{itemize}

\noindent
All theorems are proven without axioms or \texttt{sorry}.  The analysis
informs CeTTa/\MORK{} design: use \PathMap{} storage for heavy query
workloads and reverse queries; compute on demand for large domains
...
\bibitem{WilliamsStayNTT2021}
M.~Williams and M.~Stay.
\newblock Native Type Theory.
\newblock In \emph{Applied Category Theory (ACT)}, 2021.
