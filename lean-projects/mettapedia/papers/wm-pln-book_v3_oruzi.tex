...
Probabilistic Logic Networks, as described by Goertzel, Iklé,
Goertzel, and Heljakka~\cite{goertzel2009pln}, represent uncertain
judgments using truth values: a \emph{strength} (probability-like
estimate) and a \emph{confidence} (reliability-like quantity derived
from sample size).  The ambition was generous---to build a
general-purpose uncertain reasoner capable of deduction, induction,
abduction, and revision, all within a unified algebraic framework.

That ambition produced real insights.  The evidence-count
representation, the separation of strength from confidence, the
recognition that revision must be compositional, and the aspiration to
connect logical structure with probabilistic semantics---these were the
right instincts.  But in the passage from intuition to formalization,
something went wrong.

\section{Where Semantic Drift Came From}

The difficulty is not that PLN's rules are wrong.  Many of them are
correct---under assumptions that were left implicit.  The difficulty is
that the original presentation mixes three conceptually distinct layers
without always marking where one ends and another begins:

\begin{enumerate}
\item \textbf{The world-model layer.}  A probabilistic model---a joint
  distribution, a Bayesian network, or some structured posterior---that
  can answer any query by conditioning.  This is the \emph{complete}
  semantics.

\item \textbf{The evidence-algebra layer.}  An algebraic structure on
  observation counts $(n^+, n^-)$ with revision as addition,
  information ordering, and lattice operations.  This is a
  \emph{compositional bookkeeping} layer.

\item \textbf{The operational-view layer.}  Strength, confidence,
  weight, interval bounds---these are \emph{extracted summaries} of
  evidence, useful for thresholding, display, and approximate
  computation, but not themselves the semantics.
\end{enumerate}

\noindent
The calculus additionally has a \emph{typed rewrite presentation}: the
core rules encode as a typed language definition whose native types
classify world-model terms by their rewrite behavior
(Chapter~\ref{ch:typed}).  This typed realization is not a fourth
semantic layer but a computational presentation of the kernel---it is
how the calculus becomes a typed, mechanically checkable object.
...
\begin{tikzpicture}[
  ...
  layer/.style={draw, rounded corners=3pt, minimum width=72mm,
                minimum height=14mm, align=center, text width=68mm},
  ribbon/.style={draw, rounded corners=3pt, minimum width=44mm,
                 minimum height=14mm, align=center, text width=40mm},
]
...
\node[ribbon, fill=purple!8, right=18mm of wm] (typed)
  {{\bfseries Typed Realization}\\[1pt]
   {\scriptsize rewrite rules, OSLF (Ch.~\ref{ch:typed}),}\\[-1pt]
   {\scriptsize native types}};

% correct downward flow
\draw[->, thick] (wm) -- node[right, font=\scriptsize]
  {$\mathrm{extract}(W,q)$} (ev);
\draw[->, thick] (ev) -- node[right, font=\scriptsize]
  {extract views} (view);

% kernel to typed realization
\draw[->, thick] (wm.east) -- node[above, font=\scriptsize]
  {encode} (typed.west);
...
\draw[->, dashed, very thick, red!70!black]
  ...
  (wm.west);
\end{tikzpicture}
\caption{The architecture of the world-model calculus: three semantic
layers (left) plus the typed realization (right).  Solid arrows show
the correct information flow: query the world model for evidence, then
extract operational views.  The typed realization encodes the kernel as
a rewrite system with native types
(Chapter~\ref{ch:typed}).  The dashed red arrow marks the conflation
that produces semantic drift.}
\label{fig:three-layer-architecture}
\end{figure}

When these layers are conflated, several things go wrong.  Operational
views get treated as if they \emph{were} the logic: interval bounds
are composed with rules designed for point probabilities, confidence
heuristics are chained without tracking what information they lost.
Link-level inference attempts to compute complete posteriors from local
link truth values, ignoring the correlations that live only in the
joint state.  The result is semantic drift: the system \emph{almost}
works, but its correctness depends on assumptions that are neither
stated nor checked.
...
community---a co-op with private apartments and shared spaces: a
hallway, laundry room, elevator, and a rooftop community garden.  Two
autonomous agents serve the commons: a hallway cleaning robot and a
garden-tending robot.  Residents, building stewards, and the robots
cooperate through layered world models.
...
\item \textbf{Evidence layer:} motion detections and leak alarms are
  Beta evidence (binary); bathroom and hallway humidity are
  Normal--Gamma evidence (continuous); laundry room status
  (free/busy/full) and elevator health (normal/slow/faulty) are
  Dirichlet evidence (multi-valued).
...
\emph{private} world model (fast-changing, high-resolution), while the
building maintains a \emph{shared} world model for common spaces
(medium timescale).  Apartments export only evidence summaries---not
raw sensor traces---to the shared model, preserving privacy by design.
...
% Labels
\node[font=\scriptsize, text=teal!70!black] at (3,0.8)
  {\textbf{collider}};
\node[font=\scriptsize, text=teal!70!black] at (7.3,-1.8)
  {\textbf{chain}};
\node[font=\scriptsize, text=teal!70!black] at (0,-0.9)
  {\textbf{fork}};
\end{tikzpicture}
...
This scenario exercises evidence aggregation
(Chapter~\ref{ch:evidence}), state operations
(Chapter~\ref{ch:state-ops}), BN compiled inference
(Chapter~\ref{ch:bn-models}), temporal filtering
(Chapter~\ref{ch:temporal}), and the full spectrum of inference
...
provides a kernel-checked reference instance.


\section{What the Refoundation Changes}

The world-model calculus is a response to semantic drift.  Its core
move is to replace free-floating truth-value propagation with a
\emph{revisable state of knowledge}.  The system stores what it knows
in a posterior state, not in isolated truth values.  When a question is
asked, it queries that state and returns evidence.  Strength,
confidence, and intervals are summaries extracted from evidence for
display, thresholding, and action---but they are not themselves the
thing being stored or proved.
...
\item \textbf{Kernel semantics.}  A world-model posterior state is the
  ...
  (\S\ref{sec:additive-regime}).  Four verbs operate on states:
  \emph{revise}, \emph{query}, \emph{forget}, \emph{explain}.
...
operational PLN rules approximate, under explicit assumptions, within
declared model classes.  Where the original book's rules are correct,
the calculus proves them correct and states the conditions.  Where they
are not, it provides counterexamples and corrected, side-conditioned
replacements.


\section{What This Book Claims}

This book is a self-contained mathematical presentation of the
world-model calculus.  It does not require the reader to have the PLN
book open, though familiarity with the PLN tradition will enrich the
reading.

Every theorem stated here has been formalized in Lean~4 with Mathlib.
Where a result is conditional on explicit assumptions, those
assumptions are stated.  Where a result establishes a boundary (a
counterexample or no-go theorem), that boundary is stated plainly.
Where work remains operational or future, that status is marked.  The
claim taxonomy (\claimCore{}, \claimConditional{}, \claimBoundary{},
\claimOperational{}) is defined in
...
\begin{itemize}
\item \emph{Formalized} means that the stated theorem or
  counterexample is checked by the Lean~4 kernel against imported
  Mathlib facts.
\item \emph{Lean trust base}: the Lean kernel, Mathlib, and the
  standard elaboration/runtime path needed to check the proof terms.
\item \emph{Domain scope}: theorems requiring finite domains are
  marked \finitedomain{} in their titles.  The world-model kernel
  (Parts~\ref{part:calculus}--\ref{part:compiled}) and compiled BN
  inference are domain-agnostic.
\end{itemize}
\end{minipage}}
\end{center}
...
\begin{itemize}
\item \textbf{Part~\ref{part:orientation}} (this chapter, the
  Overview, and Desiderata) orients the reader: what went wrong with
  truth-value-only PLN, what the refoundation changes, and the
  mathematical requirements.

\item \textbf{Part~\ref{part:calculus}}
  (Chapters~\ref{ch:world-models}--\ref{ch:no-go}) presents the
  calculus in depth: the world-model interface, evidence carriers,
  state operations, views, typed world models, and the
  local-completeness no-go theorem.

\item \textbf{Part~\ref{part:compiled}} (Chapters~\ref{ch:sigma-rewrites}--\ref{ch:scale-control})
  shows how fast PLN inference arises from the kernel via compiled
  rewrites over Bayesian-network world models, addresses error
  transport, and formalizes large-scale inference control.

\item \textbf{Part~\ref{part:families}}
  (Chapters~\ref{ch:intensional}--\ref{ch:governance}) presents the
  additional semantic families: intensional inheritance, temporal and
  causal inference, and normative/governance reasoning.

\item \textbf{Part~\ref{part:bridges}}
  (Chapters~\ref{ch:bridges}--\ref{ch:conclusion}) bridges to
  neighboring frameworks, takes stock of what is proved, and concludes.

\item \textbf{Part~\ref{part:applications}} provides benchmark
  validation: evidence carrier validation, BN inference validation,
  genomics applications, and PPLBench results.

\item \textbf{Part~\ref{part:appendices}} collects notation, theorem
  maps, code navigation, and proof ideas.
\end{itemize}

\chapter{Overview of the Calculus}
\label{ch:overview}
\input{ch2_overview}
...
What must any foundation for uncertain reasoning preserve?  We state
eight desiderata.  Not all of them can be satisfied simultaneously in
full generality---the no-go theorem in Chapter~\ref{ch:no-go} shows
that local completeness conflicts with compositionality for arbitrary
world models.  But any proposed foundation should address each
desideratum explicitly, either satisfying it or stating clearly why and
where it fails.

\begin{enumerate}
\item \textbf{Posterior-state semantics.}  The foundational proof
  object is a posterior state---a structured distribution over possible
  worlds, or a sufficient statistic thereof.  Truth values for specific
  queries are obtained by extraction from this state, not by direct
  algebraic manipulation of link-level numbers.

\item \textbf{Revisability.}  New evidence can always be incorporated.
  Revision is the fundamental operation: it combines two posterior
  states into a single revised state.  It must be associative and
  commutative (evidence order should not matter for the final state).

\item \textbf{Queryability.}  Any well-formed query---a proposition, a
  link, a conditional---can be answered by projecting from the
  posterior state.  The query language is separate from the evidence
  representation.

\item \textbf{Evidence compositionality.}  Evidence extraction commutes
  with revision:
  \[
    \mathrm{extract}(W_1 + W_2, q) = \mathrm{extract}(W_1, q) \hplus
    \mathrm{extract}(W_2, q), \qquad \mathrm{extract}(0, q) = 0.
  \]
  Equivalently: the curried extraction map
  $W \mapsto (\lambda q.\;\mathrm{extract}(W,q))$ is an additive
  monoid homomorphism from states to answer profiles.  This single
  algebraic condition is the entire semantic interface
  (\S\ref{sec:additive-regime}).

\item \textbf{Tractable compiled fragments.}  For restricted
  world-model classes (e.g., Bayesian networks with bounded treewidth),
  query answering is tractable.  Classic PLN rules arise as compiled
  rewrites within such classes.

\item \textbf{Explicit side conditions.}  Every compiled rewrite
  carries its assumptions as explicit hypotheses: d-separation,
  screening-off, positivity, finiteness.  There are no ``silent
  assumptions.''

\item \textbf{Extracted truth-value views.}  Strength, confidence,
  interval bounds, and distributional summaries are derived views, not
  primitive objects.  They are indispensable for operational use, but
  they are not the semantics.

\item \textbf{Theorem-level boundaries.}  Where completeness or
  compositionality fails, there is a formal counterexample or no-go
  theorem stating what cannot be done.  These boundaries guide the
  design of tractable fragments.
\end{enumerate}

\noindent
The world-model calculus satisfies all eight desiderata, with the
caveat that tractability (desideratum~5) always comes at the cost of
restricting the model class, and the no-go theorem
(Chapter~\ref{ch:no-go}) establishes that local completeness is
inherently impossible for unrestricted world models.
...
\textbf{Claim taxonomy.}\quad
Every theorem, definition, and lemma in this book carries one of four
tags indicating its verification status:
\begin{description}
\item[\claimCore{}] Kernel-checked in Lean~4.  The statement and
  proof are fully formalized; the Lean kernel has verified every step.
\item[\claimConditional{}] Kernel-checked under stated side
  conditions.  The Lean proof goes through given explicitly declared
  assumptions (e.g.\ screening-off, positivity).  Remove the
  assumptions and the theorem may fail.
\item[\claimBoundary{}] Impossibility or no-go result, fully
  formalized.  These are proved counterexamples or lower bounds.
\item[\claimOperational{}] Implementation-level property.  Verified
  at the operational layer (runtime tests, MeTTa conformance) rather
  than the algebraic kernel.
\end{description}
\smallskip\noindent
Untagged results marked \emph{Conjecture} are open: neither
kernel-checked nor refuted.  They represent the frontier of the
...
formalization proves that Kyburg's \emph{flattening}~\cite{Kyburg1988}
--- the operation that collapses a probability-of-probabilities
distribution into a single first-order distribution by taking the
expectation of the inner measure --- \emph{is} the Giry bind,
...
\paragraph{The explanation layer.}
The fourth verb is now a one-line definition.  An explanation layer
lifts ordinary extraction to provenance-enriched extraction and
requires that erasing provenance recovers the original answer:

\begin{lstlisting}[language={}]
structure ExplanationLayer (State Scope Query V)
    [WorldModel State Query V] where
  explain    : State -> Query -> Scope × V
  erasure    :
    forall W q, (explain W q).2 = WorldModel.extract W q
\end{lstlisting}

\noindent
The field \code{explain} returns, for each query, a scope (the set of
source labels that contributed) together with the extracted value.  The
law \code{erasure} says that discarding the scope recovers exactly the
ordinary query answer---explanation is a refinement, not a replacement.
\code{TrackedWhichState} gives the canonical instance: the scope is
the union of provenance labels across chunks, and the value is their
summed evidence.
...
PLN v0.9\footnote{PLN v0.9: \code{github.com/trueagi-io/PLN},
tag \code{v0.9}.  430~lines of MeTTa implementing forward chaining
...
\code{StampDisjoint}---whether their stamps share any IDs.  If they
...
\midrule
\code{StampDisjoint} & \code{Finset.Disjoint} on scope supports \\
\code{StampConcat} & \code{Finset.union} \\
...
version of PLN v0.9's runtime \code{StampDisjoint}.
...
A \emph{simple truth value} (STV) is a pair $\STV{s}{c}$
...
$\STV{s}{c} = \STV{1/5}{5/15}
= \STV{0.2}{0.33}$ (with $k = 10$): low confidence in a
low-strength hypothesis.  The fork query ``Is the room occupied?''
extracts $(5, 1)$ with $\STV{0.83}{0.38}$: higher strength,
...
The mechanism connecting algebra to typed rewrites is the
\emph{Operational Semantics in Logical Form} (OSLF)
algorithm~\cite{MeredithStayOSLF2020,leanOSLF2025}.  OSLF takes any
rewrite system as input and automatically derives a spatial-behavioral
type system: a modal logic whose $\Diamond$/$\Box$ operators track
what states are reachable and what invariants are maintained.  The
WM calculus's rewrite rules instantiate OSLF to obtain the typed
realization described in this chapter.
...
\label{thm:universalEnsembleWorldModel}
For any type~$T$:
...
\begin{conjecture}[LanguageDef universality]
\label{conj:langdef-universality}
Every \code{LanguageDef} (i.e., every vertex of the typed family)
gives an additive world model via the OSLF-derived modal operators.
\end{conjecture}

\noindent
If the conjecture holds, replacing $V$ with $\mathbb{C}$ gives
...
Modus ponens & $A,\; A \!\Rightarrow\! B$
  & none
  & $s_{AB} \cdot s_A + (1\!-\!s_A) \cdot c$\;$^{\dagger}$ \\[3pt]
Revision & $e_1, e_2$
  & stamp-disjoint
...
\noindent
$^{\dagger}$In the modus ponens formula, $c \in [0, 1]$ is a
\emph{default confidence} parameter representing $P(B \mid \neg A)$
---the prior probability of~$B$ when~$A$ is absent.  Setting
$c = s_B$ (the marginal probability of~$B$) recovers the
independence baseline; $c = 0$ gives a strict modus ponens.  The
formula is bounded: since $s_{AB}, s_A, c \in [0, 1]$, the output
lies in $[0, 1]$.

\smallskip\noindent
For the \textbf{inversion} confidence formula
$c_{BA} = c_{AB} \cdot n_A / n_B$: note that this quantity can
exceed~$1$ when $n_A > n_B$.  In practice it must be clamped:
$c_{BA} = \min(c_{AB} \cdot n_A / n_B,\; 1)$.
...
\maplecourt{%
Consider the chain $\mathit{OldPlumbing} \to \mathit{Leak} \to
\mathit{HighHumidity}$ in Maple Court.  Suppose the building manager
has estimated the following strengths from maintenance logs and sensor
data:
\[
  s_{AB} = P(\mathit{Leak} \mid \mathit{OldPlumbing}) = 0.8, \quad
  s_{BC} = P(\mathit{HighHumidity} \mid \mathit{Leak}) = 0.9,
\]
\[
  s_B = P(\mathit{Leak}) = 0.5, \quad
  s_C = P(\mathit{HighHumidity}) = 0.6.
\]
Screening-off is plausible here: once we know whether a unit leaks,
knowing it has old plumbing tells us nothing extra about humidity.
Applying the deduction formula:
\begin{align*}
  \mathrm{plnDed}(0.8,\, 0.9,\, 0.5,\, 0.6)
  &= 0.8 \cdot 0.9
     + (1 - 0.8) \cdot \frac{0.6 - 0.5 \cdot 0.9}{1 - 0.5} \\
  &= 0.72 + 0.2 \cdot \frac{0.6 - 0.45}{0.5} \\
  &= 0.72 + 0.2 \cdot \frac{0.15}{0.5} \\
  &= 0.72 + 0.2 \cdot 0.30 \\
  &= 0.72 + 0.06 = 0.78.
\end{align*}
So $P(\mathit{HighHumidity} \mid \mathit{OldPlumbing}) = 0.78$.
The first term $0.72$ is the direct pathway through leaks; the
residual $0.06$ accounts for other causes of high humidity
(e.g.\ poor ventilation) that correlate with old plumbing through the
base rates $s_B$ and~$s_C$.}
...
\code{github.com/trueagi-io/PLN}, tag \code{v0.9}.} provides
...
The positive result under fresh evidence requires the damped SP/SPN
protocol.  Here \textbf{SP} (serial propagation) is the protocol
variant that propagates updates along a fixed rule ordering, and
\textbf{SPN} (serial propagation with normalization) additionally
renormalizes strengths after each propagation step.  ``Damped'' means
each step blends the newly derived state with the previous state via
a mixing coefficient $\lambda \in (0, 1)$, preventing oscillation.
...
captures.  Concretely, the PAT (property-pattern) channel measures
how closely two concepts share \emph{structural patterns} of property
membership---defined as $P_{\mathrm{PAT}}(W \mid F) = P(W) \cdot
2^{D_{\mathrm{pat}}(F; W)}$, where $D_{\mathrm{pat}}$ is a
pattern-distance kernel over the property lattice.  While ASSOC
captures pairwise statistical dependence via mutual information, PAT
captures higher-order structural similarity (e.g.\ two concepts that
share the same \emph{pattern} of properties even when pairwise
...
The notation $\varphi^{-\Delta}$ denotes the \emph{backward time
shift} of a proposition~$\varphi$: it is the proposition
$\varphi_{-\Delta} := \varphi \circ \varphi_\Delta^{-1}$, i.e.\
$\varphi$ re-indexed so that what was true at time~$t$ is now
evaluated at time~$t + \Delta$.  This is the inverse of the forward
shift~$\varphi_\Delta$ from Chapter~\ref{ch:temporal}:
applying $\varphi^{-\Delta}$ at $t + \Delta$ asks the same question
that $\varphi$ asked at~$t$.
