...
a direct C implementation of the \HE{} MeTTa evaluator that passes 288
oracle-verified tests and achieves 10--67$\times$ speedup over \HE{} on
shallow evaluation and 1.5--3.3$\times$ over \PeTTa{} on real genomic
backward chaining (\textbf{1.4M hypotheses in 3m\,22s} vs.\ \PeTTa{}'s
5m\,24s), while losing 30$\times$ to \PeTTa{}'s Prolog WAM on
search-heavy workloads (8-puzzle BFS: 181K states).  The runtime
features SymbolId-based dispatch (38\% instruction reduction vs.\
string comparison), structured space kinds (queue, hash, stack), and a
pluggable match backend with PathMap/MORK integration.  It is backed by
a sorry-free Lean proof (5{,}209+ lines) establishing semantic
correspondence between \HE{} and \PeTTa{} evaluation on the pure
fragment, a kernel-checked HE$'$ telescope formalization for dependent
binder semantics, and a bidirectional translator with CLI tooling.
We present cross-runtime benchmarks on real genomic data, a
callgrind/massif-based performance analysis identifying the remaining
30$\times$ gap as binding churn addressable by WAM-inspired
trail/rollback, a specification gap analysis of non-deterministic
evaluation with mutable state, and a three-machine architecture
(local search, indexed shared-space, type elaboration) for future
development.
\end{abstract}
...
\begin{enumerate}
\item A direct C runtime passing 288 \HE{}-oracle-verified tests.
...
\paragraph{Complete CeTTa vs.\ PeTTa benchmark matrix.}
The following table summarizes all genomic benchmarks on real data
(not synthetic stress tests):

\begin{center}
\begin{tabular}{lrrrl}
\toprule
Benchmark & KB atoms & \CeTTa{} & \PeTTa{} & Speedup \\
\midrule
Focused BC         &     143 &   66\,ms &   216\,ms & 3.3$\times$ \\
PLN evidence       &     143 &   61\,ms &   177\,ms & 2.9$\times$ \\
Pattern inference  &     143 &   17\,ms &   194\,ms & 11.4$\times$ \\
PLN 100K           & 128{,}000 &  13.9\,s &   20.8\,s & 1.5$\times$ \\
Multi-hop selector & 128{,}000 &  14.2\,s &   22.3\,s & 1.6$\times$ \\
Deep 9-rule BC     & 261{,}000 & 3m\,22s  &  5m\,24s  & 1.6$\times$ \\
\bottomrule
\end{tabular}
\end{center}

\CeTTa{} wins on all six genomic benchmarks, from 1.5$\times$ at 128K
atoms to 11.4$\times$ on small inference-heavy workloads.  The
synthetic scaling crossover (where \PeTTa{} wins at 100K+ atoms via
first-argument indexing) does not manifest on real genomic backward
chaining, because the equation dispatch path dominates over raw
\code{match~\&self} scans in the nilbc chainer.
...
evaluator can achieve 10--67$\times$ speedup over \HE{} on shallow
evaluation and 1.5--3.3$\times$ over \PeTTa{} on real genomic backward
chaining, while maintaining oracle-verified correctness across 288 tests.
The SymbolId conversion eliminated 38\% of instructions on
search-heavy workloads.  The sorry-free Lean proof of
\HE{}$\leftrightarrow$\PeTTa{} semantic correspondence (5{,}209+ lines,
0~sorries), the kernel-checked HE$'$ telescope formalization, and
executable cross-runtime benchmarks on real genomic data
(1.4M hypotheses from 261K-atom KB in 3m\,22s) establish that
MeTTa's three runtimes are semantically compatible on the shared
fragment and precisely characterize where they diverge.
...
\item \textbf{Indexed shared-space engine:} leapfrog triejoin
  ...
  planning, SpaceEngine boundary.  Target: 10--100$\times$ on
  large-KB multi-pattern conjunctions.
\item \textbf{MORK/PathMap bridge:} \code{lib/mork.metta} as
  primary user surface, stable C ABI with space/program/context
  families, orthogonal discipline$\times$engine axes.
