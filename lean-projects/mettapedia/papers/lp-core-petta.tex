...
implementation of \MeTTa{}---comprising 17{,}114 lines across 49 files with zero
...
language (\code{PrologGoal}) with 14~goal forms including conjunctive/disjunctive goals,
cut, negation, if-then-else, unification, and \code{findall},
together with an ISO conformance harness and a compilation pass
...
The bridge lives in \code{Logic/LP/MeTTaILBridge.lean} (847 lines).
...
\code{Languages/MeTTa/PeTTa/SpaceSemantics.lean} (160 lines) defines the atomspace structure:
...
\code{Languages/MeTTa/PeTTa/Eval.lean} (187 lines) defines the inductive evaluation judgment:
...
\code{Languages/MeTTa/PeTTa/LPSoundness.lean} (255 lines) defines the compilation:
...
\code{Logic/LP/PropositionalConnectionChainer.lean} (1{,}245 lines) provides a
...
\hline
\multicolumn{4}{l}{\textit{Prolog Goal Language (\code{Logic/Prolog/})}} \\
\code{Core.lean}              & 220 & \code{PrologGoal} inductive (14 ctors) & 0 \\
\code{Eval.lean}              & 443 & Prolog evaluation relation & 0 \\
\code{Prolog.lean}            & 54  & module re-export & 0 \\
\code{FixtureCorpus.lean}     & 2{,}379 & ISO/Logtalk conformance fixtures & 0 \\
\code{RuntimeErrorSpec.lean}  & 62  & ISO runtime-error boundary & 0 \\
\hline
\multicolumn{4}{l}{\textit{PeTTa Specification (\code{Languages/MeTTa/PeTTa/})}} \\
...
\hline
\textbf{Total (49 files)} & \textbf{17{,}114} & & \textbf{0} \\
\hline
\end{tabularx}
\normalsize
...
\code{Languages/MeTTa/PeTTa/Effects.lean} (301 lines) extends the pure \code{PeTTaEval}
...
\code{Languages/MeTTa/PeTTa/TypeSystem.lean} (374 lines) formalizes the static fragment of
...
\paragraph{Non-dependent fragment.}  We do not model type-level computation or
the \code{Expression} type's special structural role in the full HE
\MeTTa{} evaluation path.  Rich dependent-type style extensions are treated as
post-spec extensions, not part of current HE alignment.
...
goals before evaluation.  We formalize this pipeline in five files.

\subsection{Prolog Goal Language}

\code{Logic/Prolog/Core.lean} (220 lines) defines a Prolog-style goal language
with 14~constructors covering the ISO Prolog control and meta-predicate
repertoire (re-exported via \code{Logic/Prolog/Prolog.lean} and referenced by
\code{PrologBridge.lean}):
...
\noindent
The constructors mirror ISO Prolog goal forms:
\code{succeed}/\code{fail}/\code{cut} for control,
\code{conj}/\code{disj}/\code{ite} for logical connectives,
\code{once}/\code{neg} for determinism and negation-as-failure,
\code{unify}/\code{notUnify} for unification predicates,
\code{findall} for \code{findall/3},
\code{spaceMatch} for \MeTTaIL{} space queries, and
\code{reduceCall} for \PeTTa{}'s \code{reduce/2} predicate.
...
%% ====================================================================
\section{ISO Conformance Harness}
\label{sec:iso-conformance}
%% ====================================================================

A three-tier conformance suite validates \code{PrologEval} against the
ISO Prolog standard, providing executable evidence that the formal
semantics agrees with real Prolog implementations on a substantial
fragment.

\subsection{Lean-Aligned Fixtures}

\code{FixtureCorpus.lean} (2{,}379 lines) contains 242 proven fixture
theorems, each sourced from a specific Logtalk ISO test ID
(e.g.\ \code{iso\_true\_0\_01}, \code{iso\_unify\_2\_17}) and proven
by constructing the corresponding \code{PrologEval} derivation.
A SWI-Prolog runner (\code{swi\_fixture\_runner.pl}) executes the same
goals; a parity checker (\code{check\_lean\_swi\_parity.py}) enforces
that every \code{lean\_aligned} case has both a Lean theorem and a
passing SWI execution.

Current count: 183 \code{lean\_aligned} cases covering 63 unique ISO IDs
drawn from 9~upstream Logtalk test files (control: \code{true/0},
\code{fail/0}, conjunction, disjunction; predicates: \code{once/1},
\code{$\backslash$+/1}, \code{=/2}, \code{$\backslash$=/2},
\code{findall/3}).

\subsection{Runtime-Error Boundary Probes}

\code{RuntimeErrorSpec.lean} (62 lines) handles the boundary between
the typed \code{PrologGoal} AST and ISO runtime errors.  The AST
intentionally excludes non-callable terms; four ISO cases that require
runtime error detection are formalized as theorem-level boundary
declarations mapping each ISO~ID to its expected
\code{RuntimeErrorClass}:

\begin{itemize}
\item \code{iso\_not\_1\_06} $\mapsto$ \code{typeErrorCallable} (\code{$\backslash$+ 3})
\item \code{iso\_not\_1\_07} $\mapsto$ \code{instantiationError} (\code{$\backslash$+ G} uninstantiated)
\item \code{iso\_findall\_3\_07} $\mapsto$ \code{instantiationError} (\code{findall(\_, G, \_)} uninstantiated)
\item \code{iso\_findall\_3\_08} $\mapsto$ \code{typeErrorCallable} (\code{findall(\_, 4, \_)})
\end{itemize}

\noindent
All four are proved by \code{rfl}.

\subsection{Logtalk ISO-ID Coverage Gate}

\code{report\_logtalk\_iso\_coverage.py} extracts all \code{iso\_*}
identifiers from 9~upstream Logtalk test files
(\url{https://github.com/LogtalkDotOrg/logtalk3}) and checks that every
ID is represented by both a Lean theorem and a \code{lean\_aligned}
case.  Hard threshold: 63/63 exact coverage for both theorem and case
IDs.

\subsection{Orchestration}

The full suite is orchestrated by \code{scripts/prolog/run\_conformance.sh},
which sequentially: (1)~runs SWI fixture execution, (2)~checks
Lean$\leftrightarrow$SWI parity, (3)~validates runtime-error boundary
probes, and (4)~reports Logtalk ISO-ID coverage with hard pass/fail
thresholds.

\subsection{PeTTa Runtime Unit Suite}
\label{sec:petta-runtime-unit}

The Lean conformance in Sections~\ref{sec:iso-conformance}
and~\ref{sec:prolog} proves properties of the \emph{formal model}.
A separate runtime suite tests the \emph{concrete interpreter}: 5~files
totalling 69~assertions, each exercising a distinct slice of \PeTTa{}
semantics (rewrite dispatch, constrained function-head matching,
arithmetic and control flow, superposition/collapse non-determinism, and
atomspace mutation/query).  The files are distilled from canonical
\PeTTa{} examples (\code{functionhead3}, \code{spaces},
\code{matchnested}, \code{collapse}, \code{if2}--\code{if4}) and
reduced to focused unit-scale witnesses.

A shell runner executes each file through the real \PeTTa{} entrypoint
and enforces two hard checks per file: zero failures and an exact pass
count.
...
\noindent
Together with the Lean theorem layer, this gives both proof-level and
implementation-level validation of the \PeTTa{} semantics.
Appendix~\ref{app:runtime-unit-matrix} maps each file to the spec
...
\code{Languages/MeTTa/PeTTa/MeTTaEval.lean} (398 lines) refines \code{PeTTaEval} with three
...
\code{Languages/MeTTa/PeTTa/OSLFInstance.lean} (144 lines) and
\code{Languages/MeTTa/PeTTa/GSLTVertex.lean} (103 lines) compose the \PeTTa{} specification
...
17{,}114 lines across 49 files with zero sorries and zero custom axioms.
...
\item \textbf{First-order connection search refinement.} The propositional
  ...
  ongoing work.
\item \textbf{HE type-system completion.} The current type-system fragment
  covers annotation lookup, arrow application, and pass-through types.
  Completing full HE alignment requires formalizing the remaining
  \code{check\_if\_function\_type\_is\_applicable} /
  \code{check\_argument\_type} branches (including
  \code{BadArgType}/\code{BadType} paths, non-symbol operator handling, and
  \code{Expression} return-type behavior).
...
\codepath{compileExpr_reduceCall_iff} &
proved \\
...
\section{Runtime Unit Coverage Matrix}
\label{app:runtime-unit-matrix}
...
\hline
\textbf{File} & \textbf{Tests} & \textbf{Features exercised} \\
\hline
\code{01\_core\_rewrite.metta} & 15 &
Deterministic rewrite, symbol/term pass-through,
basic \code{superpose}/\code{collapse} \\
\code{02\_function\_head.metta} & 8 &
Constrained function-head matching (\code{in}), guarded arithmetic,
membership filtering \\
\code{03\_arith\_control.metta} & 17 &
Grounded arithmetic/comparison, \code{if}, \code{let}, \code{let*} \\
\code{04\_superpose\_nondet.metta} & 10 &
Nested non-determinism, \code{superpose}/\code{collapse},
sorted answer normalization \\
\code{05\_atomspace\_match.metta} & 19 &
\code{add-atom}/\code{remove-atom}/\code{match} over
\code{\&self} and \code{\&wuspace}, join-like queries \\
\hline
\end{tabularx}
\normalsize
\caption{\PeTTa{} runtime unit suite: 5 files, 69 tests.}
\label{tab:petta-runtime-unit-matrix}
\end{table}
