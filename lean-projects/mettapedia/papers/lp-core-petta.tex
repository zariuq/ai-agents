...
\subsection{ISO Conformance Harness}

A three-tier conformance suite validates \code{PrologEval} against the
ISO Prolog standard:

\begin{enumerate}
\item \textbf{Lean-aligned fixtures}
  (\code{FixtureCorpus.lean}, 2{,}379 lines).
  Each fixture theorem is sourced from a specific Logtalk ISO test ID
  ...
  Current count: 194 fixture theorems covering 63 unique ISO IDs drawn
  from 9~upstream Logtalk test files (control: \code{true/0},
  \code{fail/0}, conjunction, disjunction; predicates: \code{once/1},
  \code{$\backslash$+/1}, \code{=/2}, \code{$\backslash$=/2},
  \code{findall/3}).

\item \textbf{Runtime-error boundary probes}
  (\code{RuntimeErrorSpec.lean}, 62 lines).
  The typed \code{PrologGoal} AST intentionally excludes non-callable
  terms; four ISO cases that require runtime error detection
  (\code{iso\_not\_1\_06}, \code{iso\_not\_1\_07},
  \code{iso\_findall\_3\_07}, \code{iso\_findall\_3\_08}) are formalized
  as theorem-level boundary declarations mapping each ISO~ID to its
  expected \code{RuntimeErrorClass} (instantiation error or
  type-error-callable), all proved by \code{rfl}.

\item \textbf{Logtalk ISO-ID coverage}
  (\code{report\_logtalk\_iso\_coverage.py}).
  Extracts all \code{iso\_*} identifiers from the 9~upstream Logtalk
  test files and checks that every ID is represented by both a Lean
  theorem and a \code{lean\_aligned} case.
  Hard threshold: 63/63 exact coverage for both theorem and case IDs.
\end{enumerate}
