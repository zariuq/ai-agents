...
\item \textbf{Multi-relational backward chaining and discovery sweep.}
  ...
  Sildenafil) while also generating hypotheses absent from the
  ...
  \toprule
  Gene & Disease & SNPs & Tissue & Effect & GWAS Catalog \\
  \midrule
  CDO1   & hypertension      & 58  & broad    & strong & \textbf{0 hits} \\
  CLU    & hypertension      & 63  & moderate & strong & \textbf{0 hits} \\
  PTGER4 & hematologic cancer & 93 & rare     & moderate & \textbf{0 hits} \\
  HCN1   & epilepsy          & 208 & moderate & strong & \textbf{0 hits} \\
  \bottomrule
  \end{tabular}
  \end{center}

  Each chain carries a machine-checkable proof term. The
  eQTL evidence provides causal direction (SNP $\to$
  expression), while the gene--disease links are associational
  (Hetionet); formal causal validation via Mendelian
  Randomization with full summary statistics is future work.

  CDO1 $\to$ hypertension is the strongest novelty candidate:
  CDO1 catalyzes the rate-limiting step in taurine biosynthesis
  (cysteine $\to$ cysteine sulfinic acid), taurine modulates
  blood pressure via vasodilation, yet no GWAS has linked
  CDO1 expression variants to blood pressure traits.
  The 58 eQTL SNPs, tissue-broad expression, and strong
  effect size provide the genetic instruments for future
  MR validation.
\end{enumerate}
