...
-- This is the FIRST real theorem connecting algorithms to semantics.
-- Proof strategy: case split on node, unfold eval, apply EvalSem constructors.
-- The userCall case is the hardest — needs the list version + rule lookup alignment.
theorem eval_sound {rules : List EvalRule} {fuel : Nat} {node : EvalNode} {v : EvalValue}
    (h : eval rules fuel node = some v) : EvalSem rules node v := by
  sorry -- TODO: well-founded induction on (fuel, sizeOf node)
  -- Leaf cases (litInt, litBool): trivial — eval returns some, match EvalSem constructor
  -- Binary ops (eqInt, addInt, subInt, mulInt): unfold eval, split on sub-results,
  --   apply IH on each operand, construct EvalSem.{eqOp,addOp,...}
  -- ifCond: unfold eval, split on condition result, apply IH
  -- userCall: unfold eval, split on fuel, apply IH on body with decreased fuel
  --   Need: evalList_sound for arg evaluation + lookupRule alignment
