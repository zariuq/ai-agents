...
/-- Extract the explicit quantum trace carried by a rewrite path under a chosen
weight map and cost map. -/
def rewritePathTrace (wm : WeightMap S Complex) (cm : CostMap S A k) :
    {t u : S.Term} → S.RewritePath t u → QuantumTrace S A k
  | _, _, .nil _ => []
  | _, _, .cons h rest =>
      { source := _
        target := _
        ...
        cost := cm.cost h } :: rewritePathTrace wm cm rest
...
@[simp] theorem traceAccount_singleton [AddMonoid A]
    (e : QuantumTraceEntry S A k) :
    traceAccount (S := S) (A := A) (k := k) [e] = e.cost := by
  funext i
  change e.cost i + 0 = e.cost i
  simp
...
theorem traceAccount_eq_sum_map_cost [AddCommMonoid A]
    (history : QuantumTrace S A k) :
    traceAccount (S := S) (A := A) (k := k) history =
      (history.map QuantumTraceEntry.cost).sum := by
  induction history with
  | nil =>
      simp [traceAccount]
  | cons e rest ih =>
      simp [traceAccount, ih]

theorem rewritePathTrace_append
    (wm : WeightMap S Complex) (cm : CostMap S A k)
    {t u v : S.Term} (p : S.RewritePath t u) (q : S.RewritePath u v) :
    rewritePathTrace (S := S) (A := A) (k := k) wm cm (rewritePathAppend p q) =
      rewritePathTrace (S := S) (A := A) (k := k) wm cm p ++
        rewritePathTrace (S := S) (A := A) (k := k) wm cm q := by
  induction p with
  | nil _ =>
      simp [rewritePathAppend, rewritePathTrace]
  | cons h rest ih =>
      simp [rewritePathAppend, rewritePathTrace, ih]

theorem traceAccount_rewritePathTrace [AddCommMonoid A]
    (wm : WeightMap S Complex) (cm : CostMap S A k)
    {t u : S.Term} (p : S.RewritePath t u) :
    traceAccount (S := S) (A := A) (k := k)
      (rewritePathTrace (S := S) (A := A) (k := k) wm cm p) =
        totalCost cm p := by
  induction p with
  | nil _ =>
      simp [rewritePathTrace, totalCost]
  | cons h rest ih =>
      simp [rewritePathTrace, totalCost, ih]
...
@[simp] theorem oneStepPath_length {t u : S.Term} (h : S.Step t u) :
    (oneStepPath (S := S) h).length = 1 := by
  rfl

@[simp] theorem totalCost_oneStepPath {A : Type*} {k : Nat} [AddMonoid A]
    (cm : CostMap S A k) {t u : S.Term} (h : S.Step t u) :
    totalCost cm (oneStepPath (S := S) h) = cm.cost h := by
  funext i
  change cm.cost h i + 0 = cm.cost h i
  simp
