...
inductive ReducesStar : Pattern → Pattern → Prop where
...
/-- Star closure is transitive -/
theorem trans {p q r : Pattern} (h1 : p ⇝* q) (h2 : q ⇝* r) : p ⇝* r := by
  induction h1 with
  | refl => exact h2
  | step h_pq _ ih => exact step h_pq (ih h2)

/-- Single step gives star closure -/
theorem single {p q : Pattern} (h : Reduces p q) : p ⇝* q :=
  step h (refl q)
...
inductive ReducesN : ℕ → Pattern → Pattern → Prop where
...
/-- Star closure includes n-step for all n -/
theorem reducesN_to_star {n : ℕ} {p q : Pattern} (h : p ⇝[n] q) :
    p ⇝* q := by
  induction h with
...
theorem star_of_reducesN_exists (p q : Pattern) :
    (∃ n, p ⇝[n] q) → (p ⇝* q) := by
  intro ⟨n, h⟩
  exact reducesN_to_star h

/-- Concatenation of n-step reductions -/
theorem reducesN_concat {n m : ℕ} {p q r : Pattern}
...
theorem reducesN_trans_via_concat {p q r : Pattern}
    (h1 : ∃ n, p ⇝[n] q) (h2 : ∃ m, q ⇝[m] r) :
    ∃ k, p ⇝[k] r := by
  obtain ⟨n, hn⟩ := h1
  obtain ⟨m, hm⟩ := h2
...
theorem reducesN_succ_iff {n : ℕ} {p r : Pattern} :
  ...
  · -- Forward: decompose n+1 step into 1 + n
    ...
    | succ h_step h_rest =>
      ...
      -- Need to show: ∃q, p ⇝[1] q ∧ q ⇝[n] r
      -- Take q = the middle pattern from the reduction
      refine ⟨_, ?_, h_rest⟩
      exact ReducesN.succ h_step (ReducesN.zero _)
  · -- Backward: build n+1 step from 1 + n
    intro ⟨q, h1, hn⟩
    -- h1 : p ⇝[1] q means ∃q', p ⇝ q' ∧ q' ⇝[0] q
    cases h1 with
    | succ h_step h_zero =>
