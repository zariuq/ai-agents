...
Shared optimization-permission surface for MeTTa-family runtimes.

This file deliberately starts with the narrowest high-value contract lane:
lookup-query certificates. The vocabulary is shared and backend-neutral, while
the first honest backend witness remains the current MORK/MM2 runtime kernel.

Positive example:
- a PeTTa `match &self` entry can say it is a `query` over the default
  atomspace, has `read_only_lookup` effect, supports `scalar` and
  `outcome_set` memoization, and carries lookup-plan contracts plus theorem
  references.

Negative example:
- this file does not claim that all execution heads share one flat schema, and
  it does not collapse grounded builtins, state effects, and source-rule
  execution into a single undifferentiated permission bit.
-/
...
def mkCoreIntrinsicContract (spec : CoreIntrinsicSpec) : IntrinsicBuiltinContract where
  head := spec.head
  relation := Algorithms.MeTTa.Simple.intrinsicRelationName spec.head
...
theorem mkCoreIntrinsicContract_relation (spec : CoreIntrinsicSpec) :
    (mkCoreIntrinsicContract spec).relation =
      Algorithms.MeTTa.Simple.intrinsicRelationName spec.head := rfl
