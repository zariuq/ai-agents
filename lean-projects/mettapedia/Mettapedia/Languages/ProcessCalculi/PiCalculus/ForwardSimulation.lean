...
/-!
# Forward Simulation for Restriction-Free π→ρ Encoding
...
- `forward_single_step_rf`: single-step forward simulation (fully proved)
- `forward_multi_step_rf`: multi-step forward simulation (fully proved)
...
namespace Mettapedia.Languages.ProcessCalculi.PiCalculus.ForwardSimulation
...
local notation:50 p " ≡ᵨ " q => Mettapedia.Languages.ProcessCalculi.RhoCalculus.StructuralCongruence p q
...
private theorem lc_at_rhoPar {k : Nat} {P Q : Pattern}
  ...
  · simp only [lc_at, lc_at_list, hp, hq, Bool.and_true]
...
theorem forward_comm_rf {P : Process} (hrf : RestrictionFree P)
  ...
  apply Mettapedia.Languages.ProcessCalculi.RhoCalculus.Reduction.Reduces.equiv
...
private noncomputable def ReducesStar_two_left {A T B : Pattern}
  ...
  | step h_red _ ih =>
    exact ReducesStar.step
      (Mettapedia.Languages.ProcessCalculi.RhoCalculus.Reduction.Reduces.par h_red) ih
...
private theorem ReducesStar_equiv_pre {p p' q : Pattern}
  ...
  | .step h_first h_rest =>
    exact ⟨q, ⟨ReducesStar.step
      (Mettapedia.Languages.ProcessCalculi.RhoCalculus.Reduction.Reduces.equiv hsc h_first (refl _))
      h_rest⟩, refl _⟩
...
theorem forward_par_left_rf (P₁' : Process) {Q : Process}
    (_hrfQ : RestrictionFree Q) (n v : String)
...
theorem forward_par_right_rf {P₁ : Process} (Q' : Process)
    (_hrf₁ : RestrictionFree P₁) (n v : String)
...
/-! ## Safe Generic Reductions to RF -/

/-- RF structural congruence embeds into full structural congruence. -/
theorem rfsc_to_sc : (P ≡ᵣ Q) → Nonempty (P ≡ Q) := by
  intro h
  induction h with
  | refl P => exact ⟨StructuralCongruence.refl P⟩
  | symm P Q h ih =>
      rcases ih with ⟨h'⟩
      exact ⟨StructuralCongruence.symm P Q h'⟩
  | trans P Q R hPQ hQR ihPQ ihQR =>
      rcases ihPQ with ⟨hPQ'⟩
      rcases ihQR with ⟨hQR'⟩
      exact ⟨StructuralCongruence.trans P Q R hPQ' hQR'⟩
  | par_cong P P' Q Q' hPP' hQQ' ihPP' ihQQ' =>
      rcases ihPP' with ⟨hPP''⟩
      rcases ihQQ' with ⟨hQQ''⟩
      exact ⟨StructuralCongruence.par_cong P P' Q Q' hPP'' hQQ''⟩
  | par_comm P Q => exact ⟨StructuralCongruence.par_comm P Q⟩
  | par_assoc P Q R => exact ⟨StructuralCongruence.par_assoc P Q R⟩
  | par_nil_left P => exact ⟨StructuralCongruence.par_nil_left P⟩
  | par_nil_right P => exact ⟨StructuralCongruence.par_nil_right P⟩

/-- Generic π-reductions that stay inside the RF-friendly fragment:
    no `res`, RF structural-congruence wrappers only, and COMM safety data. -/
inductive ReducesSafe : Process → Process → Type where
  | comm (x : Name) (y z : Name) (P : Process)
      (hyz : y ≠ z) (hb : BarendregtFor y z P) :
      ReducesSafe
        (Process.par (Process.input x y P) (Process.output x z))
...
notation:50 P " ⇝ₛ " Q => ReducesSafe P Q

/-! ### Safe-Trace Cookbook

Use this layer to build compact traces for the forward π→ρ theorem:

1. Build one-step safe reductions with:
   `ReducesSafe.of_comm`, `ReducesSafe.of_par_left`, `ReducesSafe.of_par_right`,
   `ReducesSafe.of_struct`.
2. Lift to multi-step with `MultiStepSafe.single`.
3. Compose traces with `MultiStepSafe.trans`.
4. Convert to RF with `MultiStepSafe.toRF`, then apply the RF-forward theorem.
-/

/-- Replication + communication pattern in the derived ρ layer.

One replication unfold exposes an input; one core COMM (wrapped via `equiv`)
consumes the matching output and keeps the replicated listener.
-/
theorem rep_comm_after_unfold (n q p : Pattern) :
    Nonempty
      ((.collection .hashBag
          [.apply "POutput" [n, q],
  ...
  have h₁ : src ⇝ᵈ* mid := by
    simpa [src, mid, inp, rep] using
      (Mettapedia.Languages.ProcessCalculi.RhoCalculus.DerivedRepNu.rep_unfold_par_any
        (before := [.apply "POutput" [n, q]]) (after := ([] : List Pattern)) inp)
  have h₂core : mid ⇝ tgt := by
    have hpre : mid ≡ᵨ (.collection .hashBag [.apply "POutput" [n, q], inp, rep] none) := by
      simpa [mid, inp, rep] using
  ...
  have h₂ : mid ⇝ᵈ* tgt := ReducesDerivedStar.single (.core h₂core)
  exact ⟨ReducesDerivedStar.trans h₁ h₂⟩

/-- Ergonomic constructor for safe COMM steps. -/
def ReducesSafe.of_comm (x : Name) (y z : Name) (P : Process)
    (hyz : y ≠ z) (hb : BarendregtFor y z P) :
    (Process.par (Process.input x y P) (Process.output x z)) ⇝ₛ (P.substitute y z) :=
  ReducesSafe.comm x y z P hyz hb

/-- Ergonomic constructor for safe left parallel context steps. -/
def ReducesSafe.of_par_left (P P' Q : Process) (h : P ⇝ₛ P') :
    (P ||| Q) ⇝ₛ (P' ||| Q) :=
  ReducesSafe.par_left P P' Q h

/-- Ergonomic constructor for safe right parallel context steps. -/
def ReducesSafe.of_par_right (P Q Q' : Process) (h : Q ⇝ₛ Q') :
    (P ||| Q) ⇝ₛ (P ||| Q') :=
  ReducesSafe.par_right P Q Q' h

/-- Ergonomic constructor for safe structural-congruence wrapped steps. -/
def ReducesSafe.of_struct (P P' Q Q' : Process)
    (hsc₁ : P ≡ᵣ P') (hred : P' ⇝ₛ Q') (hsc₂ : Q' ≡ᵣ Q) :
    P ⇝ₛ Q :=
  ReducesSafe.struct P P' Q Q' hsc₁ hred hsc₂

/-- Common trace pattern: one COMM step under a left parallel context. -/
def ReducesSafe.comm_par_left (x y z : Name) (P Q : Process)
    (hyz : y ≠ z) (hb : BarendregtFor y z P) :
    ((Process.par (Process.input x y P) (Process.output x z)) ||| Q) ⇝ₛ
      ((P.substitute y z) ||| Q) :=
  ReducesSafe.of_par_left _ _ _ (ReducesSafe.of_comm x y z P hyz hb)

/-- Common trace pattern: one COMM step under a right parallel context. -/
def ReducesSafe.comm_par_right (x y z : Name) (P Q : Process)
    (hyz : y ≠ z) (hb : BarendregtFor y z P) :
    (Q ||| (Process.par (Process.input x y P) (Process.output x z))) ⇝ₛ
      (Q ||| (P.substitute y z)) :=
  ReducesSafe.of_par_right _ _ _ (ReducesSafe.of_comm x y z P hyz hb)

/-- A safe generic step converts to an RF step, carrying `CommSafe` evidence. -/
theorem ReducesSafe.toRF : (h : P ⇝ₛ Q) → ∃ hrf : P ⇝ᵣ Q, CommSafe hrf := by
  intro h
  induction h with
  | comm x y z R hyz hb =>
      exact ⟨ReducesRF.comm x y z R, ⟨hyz, hb⟩⟩
  | par_left P P' Q h ih =>
      rcases ih with ⟨hRF, hSafe⟩
      exact ⟨ReducesRF.par_left P P' Q hRF, by simpa [CommSafe] using hSafe⟩
  | par_right P Q Q' h ih =>
      rcases ih with ⟨hRF, hSafe⟩
      exact ⟨ReducesRF.par_right P Q Q' hRF, by simpa [CommSafe] using hSafe⟩
  | struct P P' Q Q' hsc₁ hred hsc₂ ih =>
      rcases ih with ⟨hRF, hSafe⟩
      exact ⟨ReducesRF.struct P P' Q Q' hsc₁ hRF hsc₂, by simpa [CommSafe] using hSafe⟩

/-- A safe generic step is also a standard π-step. -/
theorem ReducesSafe.toReduces : (h : P ⇝ₛ Q) → Nonempty (P ⇝ Q) := by
  intro h
  induction h with
  | comm x y z R _ _ =>
      exact ⟨Reduces.comm x y z R⟩
  | par_left P P' Q h ih =>
      rcases ih with ⟨ih'⟩
      exact ⟨Reduces.par_left P P' Q ih'⟩
  | par_right P Q Q' h ih =>
      rcases ih with ⟨ih'⟩
      exact ⟨Reduces.par_right P Q Q' ih'⟩
  | struct P P' Q Q' hsc₁ hred hsc₂ ih =>
      rcases rfsc_to_sc hsc₁ with ⟨hsc₁'⟩
      rcases ih with ⟨ih'⟩
      rcases rfsc_to_sc hsc₂ with ⟨hsc₂'⟩
      exact ⟨Reduces.struct P P' Q Q' hsc₁' ih' hsc₂'⟩

/-- Multi-step closure of safe generic reductions. -/
inductive MultiStepSafe : Process → Process → Type where
  | refl (P : Process) : MultiStepSafe P P
  | step {P Q R : Process} :
      ReducesSafe P Q →
      MultiStepSafe Q R →
      MultiStepSafe P R

notation:50 P " ⇝ₛ* " Q => MultiStepSafe P Q

/-- Single safe step embeds into safe multi-step. -/
def MultiStepSafe.single {P Q : Process} (h : P ⇝ₛ Q) : P ⇝ₛ* Q :=
  MultiStepSafe.step h (MultiStepSafe.refl Q)

/-- Transitivity for safe multi-step reduction. -/
noncomputable def MultiStepSafe.trans {P Q R : Process} :
    (P ⇝ₛ* Q) → (Q ⇝ₛ* R) → (P ⇝ₛ* R) := by
  intro hPQ hQR
  induction hPQ with
  | refl _ => exact hQR
  | @step P X Q hPX hXQ ih =>
      exact MultiStepSafe.step hPX (ih hQR)

/-- Common repeated-left-context chain:
    perform one COMM redex reduction inside two nested left parallel contexts. -/
def MultiStepSafe.of_comm_par_left_chain
    (x y z : Name) (P Q R : Process)
    (hyz : y ≠ z) (hb : BarendregtFor y z P) :
    (((Process.par (Process.input x y P) (Process.output x z)) ||| Q) ||| R) ⇝ₛ*
      (((P.substitute y z) ||| Q) ||| R) :=
  MultiStepSafe.single
    (ReducesSafe.of_par_left _ _ _ (ReducesSafe.comm_par_left x y z P Q hyz hb))

/-- A safe generic multi-step converts to an RF multi-step with safety proof. -/
theorem MultiStepSafe.toRF : (h : P ⇝ₛ* Q) → ∃ hrf : MultiStepRF P Q, MultiCommSafe hrf := by
  intro h
  induction h with
  | refl P =>
      exact ⟨MultiStepRF.refl P, trivial⟩
  | @step P Q R hstep hrest ih =>
      rcases ReducesSafe.toRF hstep with ⟨hRF, hSafe⟩
      rcases ih with ⟨restRF, restSafe⟩
      exact ⟨MultiStepRF.step hRF restRF, ⟨hSafe, restSafe⟩⟩

/-- A safe generic multi-step is also a standard π multi-step. -/
theorem MultiStepSafe.toMultiStep : (h : P ⇝ₛ* Q) → Nonempty (P ⇝* Q) := by
  intro h
  induction h with
  | refl P =>
      exact ⟨MultiStep.refl P⟩
  | @step P Q R hstep hrest ih =>
      rcases ReducesSafe.toReduces hstep with ⟨hstep'⟩
      rcases ih with ⟨ih'⟩
      exact ⟨MultiStep.step P Q R hstep' ih'⟩

end Mettapedia.Languages.ProcessCalculi.PiCalculus.ForwardSimulation
