...
namespace Mettapedia.Logic.MarkovLogicInfiniteUniqueness
...
theorem exists_fixedRegionCylinderDLR_of_equiv
  ...
  exact
    Mettapedia.Logic.MarkovLogicInfiniteExistence.RegionExhaustion.exists_fixedRegionCylinderDLR_of_equiv
...
theorem singletonTrueProbability_toReal_eq_boundaryPMFExpectation_of_fixedRegionDLR
  ...
  have hlim :
    ...
    simpa [J, S] using
      (Mettapedia.Logic.MarkovLogicInfiniteFixedRegionDLR.RegionExhaustion.limitMarginal_lintegral_cylinderBoundaryKernelValue
...
theorem singleSiteHeatBathKernelPMF_toMeasure_apply
  ...
  rw [PMF.toMeasure_map_apply
    ...
    (hs := hS)]
  change MarkovLogicInfiniteWorldMeasures.finiteVolumeWorldMeasure
      (M := M'.toInfiniteGroundMLNSpec)
...
theorem finiteRegionCouplingExpectedEventDisagreement_le_assignmentDisagreementProbability
    {Δ : Region Atom}
    (q : PMF (LocalAssignment Atom Δ × LocalAssignment Atom Δ))
    (S : Set (LocalAssignment Atom Δ)) :
    finiteRegionCouplingExpectedEventDisagreement (Atom := Atom) q S ≤
  ...
  · by_cases hz₁ : z.1 ∈ S
    · have hz₂ : z.2 ∈ S := by simpa [hxy] using hz₁
      simp [finiteRegionEventIndicator, hxy, hz₂]
    · have hz₂ : z.2 ∉ S := by
        intro hz₂
        exact hz₁ (by simpa [hxy] using hz₂)
      simp [finiteRegionEventIndicator, hxy, hz₂]
  · have hbound :
      |finiteRegionEventIndicator (Atom := Atom) S z.1 -
          finiteRegionEventIndicator (Atom := Atom) S z.2| ≤ 1 := by
      by_cases hz₁ : z.1 ∈ S <;> by_cases hz₂ : z.2 ∈ S <;>
        simp [finiteRegionEventIndicator, hz₁, hz₂]
    have hqnonneg : 0 ≤ ENNReal.toReal (q z) := ENNReal.toReal_nonneg
    have hmul :
        |finiteRegionEventIndicator (Atom := Atom) S z.1 -
            finiteRegionEventIndicator (Atom := Atom) S z.2| *
            ENNReal.toReal (q z) ≤
          1 * ENNReal.toReal (q z) :=
      mul_le_mul_of_nonneg_right hbound hqnonneg
    simpa [hxy] using hmul

theorem finiteRegionSetProbabilityDiscrepancy_le_couplingAssignmentDisagreementProbability
    (M : ClassicalInfiniteGroundMLNSpec Atom ClauseId)
    (μ ν : ProbabilityMeasure (InfiniteWorld Atom))
    (Δ : Region Atom)
    (S : Set (LocalAssignment Atom Δ))
    ...
    M.finiteRegionSetProbabilityDiscrepancy μ ν Δ S ≤
      finiteRegionCouplingAssignmentDisagreementProbability (Atom := Atom) q := by
  exact le_trans
    (M.finiteRegionSetProbabilityDiscrepancy_le_of_limitMarginalCoupling μ ν Δ S q hqfst hqsnd)
    (finiteRegionCouplingExpectedEventDisagreement_le_assignmentDisagreementProbability
      (Atom := Atom) q S)
...
theorem finiteRegionLocalQueryDiscrepancy_eq_setProbabilityDiscrepancy
  ...
  rw [Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal_apply_localConstraintSet
      (μ := (μ : Measure (InfiniteWorld Atom))) (Λ := Δ) (q := q),
    Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal_apply_localConstraintSet
...
theorem finiteRegionLocalQueryDiscrepancy_le_couplingAssignmentDisagreementProbability
    (M : ClassicalInfiniteGroundMLNSpec Atom ClauseId)
    (μ ν : ProbabilityMeasure (InfiniteWorld Atom))
    (Δ : Region Atom)
    (qv : LocalConstraintQuery Atom Δ)
    ...
    M.finiteRegionLocalQueryDiscrepancy μ ν Δ qv ≤
      finiteRegionCouplingAssignmentDisagreementProbability (Atom := Atom) q := by
  rw [M.finiteRegionLocalQueryDiscrepancy_eq_setProbabilityDiscrepancy μ ν Δ qv]
  exact M.finiteRegionSetProbabilityDiscrepancy_le_couplingAssignmentDisagreementProbability
    μ ν Δ (localConstraintSet Δ qv) q hqfst hqsnd
  ...
  have hlim :
    ...
    simpa using congrArg
      (fun ρ : Measure (InfiniteWorld Atom) =>
        Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal ρ Δ)
      hμν
  have hpmf :
      p =
        (Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal
...
theorem limitMarginal_toPMF_singleSiteHeatBathUpdatePMF_eq_of_interior
  ...
  let M' := M.toStrictlyPositiveInfiniteGroundMLNSpec
  let pμ := (Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal
    (μ : Measure (InfiniteWorld Atom)) Δ).toPMF
  ext y
  have hsingleton : MeasurableSet ({y} : Set (LocalAssignment Atom Δ)) :=
    MeasurableSet.singleton y
        ...
        _ =
          (μ : Measure (InfiniteWorld Atom))
            ((Finset.restrict Δ) ⁻¹' ({y} : Set (LocalAssignment Atom Δ))) := by
              rw [Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal]
    ...
    calc
      ...
      _ =
        ∫⁻ ω, f (Finset.restrict Δ ω) ∂ (μ : Measure (InfiniteWorld Atom)) := by
              simpa [Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal] using
    ...
    _ =
      ((Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal
        ...
        ((restrictAssignment hΓΔ) ⁻¹' ({x} : Set (LocalAssignment Atom Γ))) := by
          rw [PMF.toMeasure_map_apply
            (p := (Mettapedia.Logic.MarkovLogicInfiniteLimitFamily.RegionExhaustion.limitMarginal
              (μ : Measure (InfiniteWorld Atom)) Δ).toPMF)
...
This is the core boundary-insensitivity estimate used in the final uniqueness
proof, and it is also the reusable theorem object for open-ended ontology
stability: local disagreement decays geometrically with shell depth. -/
theorem exists_limitMarginalCoupling_sup_le_pow_of_uniformConstant
    (M : ClassicalInfiniteGroundMLNSpec Atom ClauseId)
    {C : ℝ}
    (hC_nonneg : 0 ≤ C)
    (hC_lt_one : C < 1)
...
end Mettapedia.Logic.MarkovLogicInfiniteUniqueness
