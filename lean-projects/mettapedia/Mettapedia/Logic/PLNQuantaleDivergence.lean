...
2. **Tensor preserves quantale structure** but doesn't distribute over strength
   - BinaryEvidence forms a proper quantale
...
theorem revision_neq_supremum :
  ...
  use ⟨2, 1⟩, ⟨1, 2⟩
  intro h
  have hnegL : ((⟨2, 1⟩ : BinaryEvidence) + ⟨1, 2⟩).neg = 3 := by
    norm_num [BinaryEvidence.hplus_def]
  have hnegR : (((⟨2, 1⟩ : BinaryEvidence) ⊔ ⟨1, 2⟩).neg) = 2 := by
    change max (1 : ℝ≥0∞) 2 = 2
    norm_num
  have hneg : ((⟨2, 1⟩ : BinaryEvidence) + ⟨1, 2⟩).neg = (((⟨2, 1⟩ : BinaryEvidence) ⊔ ⟨1, 2⟩).neg) :=
...
Even though BinaryEvidence is a commutative quantale, tensor on BinaryEvidence doesn't
generally equal pointwise multiplication of strengths.
-/

/-- Tensor product on BinaryEvidence doesn't equal strength multiplication.
...
theorem tensor_neq_strength_product :
  ...
  · norm_num [BinaryEvidence.total]
  · norm_num [BinaryEvidence.total]
  · norm_num [BinaryEvidence.total, BinaryEvidence.tensor_def]
  · norm_num [BinaryEvidence.total]
  · norm_num [BinaryEvidence.total]
  · norm_num [BinaryEvidence.total, BinaryEvidence.tensor_def]
  · intro hEq
    have hEqR : ENNReal.toReal (BinaryEvidence.toStrength ((⟨3, 1⟩ : BinaryEvidence) * ⟨2, 2⟩)) =
        ENNReal.toReal (BinaryEvidence.toStrength (⟨3, 1⟩ : BinaryEvidence) * BinaryEvidence.toStrength (⟨2, 2⟩)) := by
      exact congrArg ENNReal.toReal hEq
    norm_num [BinaryEvidence.toStrength, BinaryEvidence.total, BinaryEvidence.tensor_def] at hEqR
...
theorem sup_eq_dominated (e₁ e₂ : BinaryEvidence) (h : e₁ ≤ e₂) :
    e₁ ⊔ e₂ = e₂ := by
  ext
