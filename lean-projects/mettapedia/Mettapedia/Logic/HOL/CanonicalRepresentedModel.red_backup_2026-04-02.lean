              ...
              change
                ((lowerLiftedRepresented
                  ...
                  (fromLiftedHeyting
                    ...
                    (Const := Const)
                    .prop
                    (HeytingPreModel.Eqv (liftedRepresentedPreModel D) .prop
                      (ULift.up a)
                      (ULift.up b))) : HenkinTruthVal Base Const) : Set (HenkinWorld Base Const)) =
                ((RepEqv
                  (Base := Base)
                  (Const := Const)
                  .prop
                  ...
                  (lowerLiftedRepresented
                    ...
                    (fromLiftedHeyting
                      ...
                      (ULift.up b))) : HenkinTruthVal Base Const) : Set (HenkinWorld Base Const))
              ext W
              simp [HeytingPreModel.eqv_prop, repEqv_prop, lowerLiftedRepresented,
                representedEquiv, fromLiftedHeyting, liftedHeytingEquiv,
                henkinTruthSet_and_eq_sup, henkinTruthSet_imp_eq_sdiff, sup_comm]
  | .base b, x, y => by
      cases x with
      | up tx =>
          cases y with
          | up ty =>
              change
                ((lowerLiftedRepresented
                  ...
                  (fromLiftedHeyting
                    ...
                    (Const := Const)
                    .prop
                    (HeytingPreModel.Eqv (liftedRepresentedPreModel D) (.base b)
                      (ULift.up tx)
                      (ULift.up ty))) : HenkinTruthVal Base Const) : Set (HenkinWorld Base Const)) =
                ((RepEqv
                  (Base := Base)
                  (Const := Const)
                  (.base b)
              ...
              simp [HeytingPreModel.eqv_base, repEqv_base, liftedRepresentedPreModel,
                lowerLiftedRepresented, representedEquiv, fromLiftedHeyting,
                liftedHeytingEquiv, liftedRepresentedBaseEq, representedBaseEq]
  | .arr σ τ, f, g => by
      let pEq :
          {x :
            Ty.denoteHeyting
...
theorem represents_subst_of_liftedRepresentedPreModel
  ...
  | _, .prop, .ex φ, σs, ρ, hσ => by
      ...
      · intro hEx
        ...
        have hsEq : x.down = OrderDual.toDual s' := hxEq
        have hxRaw :
            Admissible (Base := Base) (Const := Const) _
              (lowerLiftedRepresented
                (Base := Base)
        ...
        have hBody :
          ...
          represents_subst_of_liftedRepresentedPreModel
            ...
            (ρ := HeytingPreModel.extend M ρ x.1)
            hσx
        simpa [Mettapedia.Logic.HOL.HenkinConstInfinity.denoteFormula, subst,
          (represents_prop_iff (Base := Base) (Const := Const)).1 hBody] using hWs
