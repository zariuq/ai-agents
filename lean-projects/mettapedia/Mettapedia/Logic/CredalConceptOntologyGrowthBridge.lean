...
robust credal inheritance obligation is supported inside a region whose MLN
semantics is stable under distant ontology growth, then threshold-validity of
that seed family is preserved under the growth step. Generic WM consequence
closure can then be re-applied on the grown side without re-proving the robust
semantic slice.
-/

namespace Mettapedia.Logic.CredalConceptOntologyGrowthBridge
...
variable {Obj : Type v} {Attr : Type w} {Q : Type x} {Gate : Type y}
variable [Preorder Q] [Fintype Gate] [Nonempty Gate] [Fintype Obj] [Fintype Attr]

/- For an encoded robust credal inheritance query whose support lies inside a
region of local MLN agreement, ontology growth preserves the induced WM query
strength exactly. -/
omit [Fintype Gate] [Nonempty Gate] in
theorem lowerFormedConceptQueryStrength_eq_of_specAgreesOnRegion
    ...
    (subConcept superConcept : ConceptOntology.LowerFormedConcept GateFamily M)
    (hSupport :
...
/- If a robust credal seed family is threshold-valid in one MLN world-model
state, and every encoded seed query is supported inside a region of local MLN
agreement, then the same robust seed family remains threshold-valid after
ontology growth to the agreeing specification. -/
omit [Fintype Gate] [Nonempty Gate] in
theorem thresholdValid_lowerFormedConceptQuerySet_stable_of_specAgreesOnRegion
    {M₁ M₂ : MarkovLogicInfiniteUniqueness.ClassicalInfiniteGroundMLNSpec Atom ClauseId}
    {Γ : MarkovLogicInfiniteSpecification.Region Atom}
    (hagree : SpecAgreesOnRegion M₁ M₂ Γ)
    (hclosed₁ : InteractionClosed M₁ Γ)
    ...
    (encode :
      ConceptOntology.LowerFormedConcept GateFamily M →
        ConceptOntology.LowerFormedConcept GateFamily M →
          ConstraintQuery Atom)
    (seed :
      Set (CredalConceptFixpointClosureBridge.LowerFormedConceptPair GateFamily M))
    (hSeed :
      thresholdValid
        (State := MassState (ConstraintQuery Atom))
        (Query := ConstraintQuery Atom)
        ({infiniteMLNMassSemantics M₁ μ₁ hμ₁}) tau
        (CredalConceptFixpointClosureBridge.lowerFormedConceptQuerySet
          GateFamily M encode seed))
    (hSupport :
      ∀ p : CredalConceptFixpointClosureBridge.LowerFormedConceptPair GateFamily M, p ∈ seed →
...
threshold-validity once the encoded robust credal seed family is locally stable
under ontology growth. -/
omit [Fintype Gate] [Nonempty Gate] in
theorem leastRuleClosure_thresholdValid_lowerFormedConceptQuerySet_of_specAgreesOnRegion
    (R :
      RuleSet
        (MassState (ConstraintQuery Atom))
        (ConstraintQuery Atom))
...
variable [Preorder Q] [Fintype Gate] [Nonempty Gate] [Fintype Obj] [Fintype Attr] [Preorder Cost]

/- If ontology growth preserves the local semantics of an encoded robust
lower-formed seed family, and the grown state's available region is covered by
the grown closure of that family, then every available query becomes
WM-admissible at the same threshold on the grown ontology. -/
omit [Fintype Gate] [Nonempty Gate] in
...
stable robust lower-formed seed family, the grown WM-admissible region
...
theorem generic_wmAdmissibleRegionAt_eq_availableRegionAt_of_specAgreesOnRegion
  ...
  exact thresholdValid_mono
    ...
    (τ := tau)
    hAvail
    (leastRuleClosure_thresholdValid_lowerFormedConceptQuerySet_of_specAgreesOnRegion
      R hagree hclosed₁ hclosed₂ hbudget₁ hbudget₂ μ₁ μ₂ hμ₁ hμ₂
      GateFamily M tau encode seed hSeed hSupport)
...
variable {Atom ClauseId : Type u} [DecidableEq Atom] [DecidableEq ClauseId]
variable {Obs : Type v} {Obj : Type w} {Attr : Type x} {Q : Type*} {Gate : Type y}
variable [AddCommMonoid Q] [Preorder Q] [Fintype Gate] [Nonempty Gate] [Fintype Obj] [Fintype Attr]
...
end Mettapedia.Logic.CredalConceptOntologyGrowthBridge
