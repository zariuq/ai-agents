/-
# BinaryEvidence Interval Bounds

This file connects PLN BinaryEvidence incomparability to probability interval bounds,
creating a bridge between the BinaryEvidence quantale and Heyting K&S interval theory.

## The Key Insight

When two BinaryEvidence values are **incomparable** in the partial order (neither ≤ the other),
...
For BinaryEvidence counts, we derive similar bounds from the partial order structure.
...
/-! ## Strength Bounds from BinaryEvidence -/

/-- The strength of evidence: s = n⁺ / (n⁺ + n⁻).
    Returns 0 if total evidence is 0 (undefined case handled as zero). -/
noncomputable def strength (e : BinaryEvidence) : ℝ≥0∞ :=
  e.pos / (e.pos + e.neg)

/-- Strength is at most 1. -/
theorem strength_le_one (e : BinaryEvidence) : strength e ≤ 1 := by
  unfold strength
...
/-! ## BinaryEvidence Incomparability
...
theorem incomparable_characterization (e₁ e₂ : BinaryEvidence) :
    Incomparable e₁ e₂ ↔
...
noncomputable def strengthInterval (e : BinaryEvidence) : Set ℝ :=
  Set.Icc (strength e).toReal (strength e).toReal

/-- The strength interval is a single point. -/
theorem strengthInterval_singleton (e : BinaryEvidence) :
...
noncomputable def strengthIntervalOfSet (S : Set BinaryEvidence) : Set ℝ :=
  Set.Icc (⨅ e ∈ S, (strength e).toReal) (⨆ e ∈ S, (strength e).toReal)

/-! ## Fréchet-Style Bounds for BinaryEvidence Combination -/

/-- Meet of two evidence values (coordinatewise minimum).
    This is a lower bound in the partial order. -/
def evidenceMeet (e₁ e₂ : BinaryEvidence) : BinaryEvidence :=
...
def evidenceJoin (e₁ e₂ : BinaryEvidence) : BinaryEvidence :=
  ⟨max e₁.pos e₂.pos, max e₁.neg e₂.neg⟩

/-- Meet is below both inputs. -/
theorem evidenceMeet_le_left (e₁ e₂ : BinaryEvidence) : evidenceMeet e₁ e₂ ≤ e₁ := by
  simp only [evidenceMeet, le_def]
  exact ⟨min_le_left _ _, min_le_left _ _⟩

theorem evidenceMeet_le_right (e₁ e₂ : BinaryEvidence) : evidenceMeet e₁ e₂ ≤ e₂ := by
  simp only [evidenceMeet, le_def]
  exact ⟨min_le_right _ _, min_le_right _ _⟩

/-- Join is above both inputs. -/
theorem le_evidenceJoin_left (e₁ e₂ : BinaryEvidence) : e₁ ≤ evidenceJoin e₁ e₂ := by
  simp only [evidenceJoin, le_def]
  exact ⟨le_max_left _ _, le_max_left _ _⟩

theorem le_evidenceJoin_right (e₁ e₂ : BinaryEvidence) : e₂ ≤ evidenceJoin e₁ e₂ := by
  simp only [evidenceJoin, le_def]
  exact ⟨le_max_right _ _, le_max_right _ _⟩

/-- Meet is below Join. -/
theorem evidenceMeet_le_evidenceJoin (e₁ e₂ : BinaryEvidence) :
    evidenceMeet e₁ e₂ ≤ evidenceJoin e₁ e₂ := by
  simp only [evidenceMeet, evidenceJoin, le_def]
  exact ⟨min_le_max, min_le_max⟩

/-! ## The "BinaryEvidence Gap" -/
...
theorem incomparable_implies_ne (e₁ e₂ : BinaryEvidence)
...
def comparableSet (e : BinaryEvidence) : Set BinaryEvidence :=
  { e' | e ≤ e' ∨ e' ≤ e }

/-- Any evidence is comparable to itself. -/
theorem self_mem_comparableSet (e : BinaryEvidence) : e ∈ comparableSet e := by
  simp [comparableSet]

/-- If e' is above e and e' ≤ e'', then e'' is above e (transitivity). -/
theorem comparableSet_closed_above (e e' e'' : BinaryEvidence)
    (h : e ≤ e') (hle : e' ≤ e'') : e ≤ e'' :=
  le_trans h hle
...
/-- When we have uncertainty about which evidence is "true", represented by
    ...
    The BinaryEvidence partial order does NOT induce a strength ordering:
...
-/
theorem strength_in_interval (eLower eUpper : BinaryEvidence)
...
1. **Strength function**: Maps BinaryEvidence to [0,1] via s = n⁺ / (n⁺ + n⁻)

2. **Incomparability**: Two BinaryEvidence values may be incomparable in the partial order,
...
5. **Bridge to Heyting K&S**: BinaryEvidence incomparability → strength uncertainty →
   interval-valued probability.

## Key Observation

The BinaryEvidence partial order does NOT directly induce a strength ordering:
