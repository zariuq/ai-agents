...
In a linearly ordered monoid with strict monotonicity (without assuming identity is minimum),
define \emph{bilateral separation}:
\begin{itemize}
\item \textbf{(Positive)} For $0 < a$ and $0 < x < y$: $\exists\, m>0,\, n\ge 0$ with $x^{(m)} < a^{(n)} \le y^{(m)}$.
\item \textbf{(Negative)} For $a < 0$ and $x < y < 0$: $\exists\, m>0,\, n\ge 0$ with $x^{(m)} < a^{(n)} \le y^{(m)}$.
\end{itemize}
Then: \textbf{NAP $\Leftrightarrow$ Bilateral Separation.}

The key constraint is that $a$, $x$, $y$ must lie on the \emph{same side} of identity;
cross-sign sandwiching is impossible since rationals cannot approximate sign-mismatched ratios.
\end{theorem}
...
\begin{remark}
In probability applications, identity represents impossibility ($\bot$) and is the minimum of the order,
so the negative branch of bilateral separation is vacuously satisfied.
\end{remark}
