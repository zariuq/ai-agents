...
\begin{itemize}
\item \textbf{High-level view:} Start with Section~12 (Summary) for the consolidated theorem list and key discoveries.
...
\item \textbf{Audit Appendix~C (entropy / variational theorem):} See Section~7 and the counterexamples in Section~11.
\item \textbf{Build commands:} See Section~13 (Build Instructions) at the end.
\end{itemize}
...
The $\mathbb{Z}$ counterexample (\texttt{AppendixA/Counterexamples/NegativeWithoutIdentity.lean}) shows:
\begin{itemize}
...
\textbf{File}: \texttt{Separation/HolderEmbedding.lean}
...
\textbf{Lines 182--185, HolderEmbedding.lean}
...
\textbf{Lines 297--303, HolderEmbedding.lean}
...
\textbf{Lines 269--276, HolderEmbedding.lean}
...
\begin{lstlisting}
-- ProductTheorem/DirectProduct.lean, line 42
...
\begin{itemize}
\item Lattice level: \texttt{DirectProduct.prod\_sup\_left} (ProductTheorem/DirectProduct.lean)
\item Graded level: \texttt{DistributesOverAdd} (ProductTheorem/Basic.lean)
\item Bridge: \texttt{distributes\_over\_add\_from\_lattice} (ProductTheorem/DistributivityDerivation.lean)
\end{itemize}
...
\begin{itemize}
\item \texttt{representation\_semigroup} (\texttt{Separation/HolderEmbedding.lean:297}) --- $\exists \Theta$ preserving order and addition (identity-free)
\item \texttt{representation\_from\_noAnomalousPairs} (\texttt{Separation/HolderEmbedding.lean:269}) --- Full representation with $\Theta(\text{ident}) = 0$
\item \texttt{associativity\_representation} (\texttt{RepresentationTheorem/Main.lean:34}) --- Public API for Appendix A
\item \texttt{op\_archimedean\_of\_separation} (\texttt{Separation/SandwichSeparation.lean:166}) --- Archimedean is derivable
\end{itemize}

\subsubsection*{Product Rule (Appendix B)}
\begin{itemize}
\item \texttt{psi\_is\_exponential} (\texttt{ProductTheorem/ProductExp.lean:156}) --- $\Psi(x \otimes y) = \Psi(x) \cdot \Psi(y)$
\item \texttt{product\_rule\_normalized} (\texttt{ProductTheorem/Main.lean:198}) --- $p(xy|I) = p(x|I) \cdot p(y|xI)$
\item \texttt{ScaledMultRep} (\texttt{ProductTheorem/ScaledMultRep.lean:44}) --- Common interface for both proof paths
\end{itemize}

\subsubsection*{Variational (Appendix C)}
\begin{itemize}
\item \texttt{variational\_implies\_entropy\_form} (\texttt{VariationalTheorem.lean:167}) --- Measurable solution $\Rightarrow$ $H(m) = B + C \log m$
\item \texttt{entropy\_form\_satisfies\_variational} (\texttt{VariationalTheorem.lean:201}) --- Converse: entropy form satisfies the equation
\item \texttt{entropy\_form\_deriv\_correct} (\texttt{VariationalTheorem.lean:234}) --- Derivative is $B + C \log m$
\end{itemize}

\subsubsection*{Conditional Probability (Section 7)}
\begin{itemize}
\item \texttt{chainProductRule} (\texttt{ConditionalProbability/Basic.lean:348}) --- $p(xy|z) = p(x|z) \cdot p(y|xz)$
\item \texttt{bayesTheorem} (\texttt{ConditionalProbability/Basic.lean:424}) --- $p(x|yz) \cdot p(y|z) = p(y|xz) \cdot p(x|z)$
\item \texttt{prob\_eq\_measure\_ratio} (\texttt{ConditionalProbability/Basic.lean:714}) --- $p(x|y) = m(x \wedge y) / m(y)$
\item \texttt{baseMeasure\_satisfies\_measure\_axioms} (\texttt{ConditionalProbability/Basic.lean:559}) --- $m$ is a probability measure
\end{itemize}

\subsubsection*{Divergence \& Entropy (Sections 6, 8)}
\begin{itemize}
\item \texttt{atomDivergence\_nonneg} (\texttt{Divergence.lean:89}) --- $D(w \| u) \geq 0$
\item \texttt{atomDivergence\_eq\_zero\_iff} (\texttt{Divergence.lean:123}) --- $D(w \| u) = 0 \Leftrightarrow w = u$
\item \texttt{klDivergence} (\texttt{InformationEntropy.lean:184}) --- KL divergence from atom divergence
\item \texttt{shannonEntropy} (\texttt{InformationEntropy.lean:267}) --- $H = -\sum p \log p$
\end{itemize}

\subsubsection*{Quantum Theory Classification (Section 4)}
\begin{itemize}
\item \texttt{selection\_theorem} (\texttt{SymmetricalFoundation.lean:312}) --- $\mu < 0$ gives QM Born rule
\item \texttt{mean\_bornRule\_sum\_unit\_phases} (\texttt{SymmetricalFoundation.lean:401}) --- Born rule from averaging
\item \texttt{classification\_theorem} (\texttt{TwoDimClassification.lean:178}) --- Exactly 3 algebra classes
\end{itemize}
...
\textbf{The $\mathbb{Z}$ counterexample} (\texttt{AppendixA/Counterexamples/NegativeWithoutIdentity.lean}):
\begin{itemize}
