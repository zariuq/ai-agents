...
Supplementary modules (Cox, Shore--Johnson, Shannon/Faddeev) are present but intentionally not imported by default by the FOI reviewer entrypoint; they provide alternative derivation paths and may assume additional regularity gates.
\end{abstract}
...
\textbf{Path A (rigidity / solution)}: assume the functional equation holds globally and add an explicit regularity gate (we use measurability of $H'$). Then the only solutions are logarithmic (see \texttt{variationalEquation\_solution\_measurable}, lines 312--378 in \texttt{Variational/Main.lean}).

\textbf{Path B (derivation / motivation)}: compute the coordinate derivative of a separable product Lagrangian at a stationary point and obtain the separated equation that motivates the functional equation (see \texttt{section PathB} starting at line 622 and \texttt{lagrange\_coordinate\_deriv\_eq} at line 696 in \texttt{Variational/Main.lean}). Upgrading ``holds at a stationary point'' to a global equation is an explicit universality/richness premise (Gate~G).
...
\begin{itemize}
\item \textbf{Unified finite entropy/KL entrypoint}: \texttt{Mettapedia/InformationTheory/EntropyKL.lean}
\item \textbf{Axiomatic entropy (Faddeev, Shannon, Shannon--Khinchin)}: \texttt{Mettapedia/InformationTheory/ShannonEntropy/Main.lean}
\item \textbf{Shore--Johnson $\to$ KL (derivation glue)}: \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnson/KLDerivation.lean} (full import surface: \texttt{.../ShoreJohnson/Main.lean})
\item \textbf{Cox probability calculus derivation}: \texttt{Mettapedia/ProbabilityTheory/Cox.lean}
\item \textbf{Algorithmic probability (Solomonoff) $\leftrightarrow$ K\&S KL gate}: \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/Bridges/AlgorithmicProbabilityBridge.lean}
\end{itemize}
