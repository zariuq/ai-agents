...
\begin{abstract}
This document provides a systematic walkthrough of the Lean 4 formalization of Knuth \& Skilling's ``Foundations of Inference''~\cite{KnuthSkilling2012}.
...
Supplementary modules (Cox~\cite{Cox1946}, Shore--Johnson~\cite{ShoreJohnson1980}, Shannon~\cite{Shannon1948}/Faddeev~\cite{Faddeev1956}) are present but intentionally not imported by default by the FOI reviewer entrypoint; they provide alternative derivation paths and may assume additional regularity gates.
\end{abstract}

\begin{center}
\noindent\fbox{%
\begin{minipage}{0.97\textwidth}
\textbf{Two-track reading (choose your level).}
This walkthrough is structured so that a mathematician can audit \emph{statements} quickly,
while a proof-assistant user can audit \emph{proofs} precisely.

\medskip
\textbf{Track A: theorem-level audit (skim).}
\begin{itemize}
\item Start with Section~13 (Summary), then jump to any topic and read only the definitions + theorem statements.
\item Use file/line pointers for rapid spot-checking without reading every proof.
\end{itemize}

\textbf{Track B: proof-level audit (deep).}
\begin{itemize}
\item Run \texttt{lake build} \path{Mettapedia.ProbabilityTheory.KnuthSkilling.FoundationsOfInference}.
\item Open the referenced Lean files at the cited line numbers to audit the full proof scripts.
\end{itemize}

\textbf{Scope note.} The reviewer entrypoint \texttt{FoundationsOfInference.lean} intentionally excludes
optional derivation paths (Cox, Shore--Johnson, Shannon/Faddeev, algorithmic probability), which are present as opt-in modules.
\end{minipage}}
\end{center}
...
\item \textbf{Connection}: \texttt{iterate\_op\_pnat x n = iterate\_op x n.val} when $n \geq 1$
      (\path{Core/Algebra.lean:198})
\end{itemize}
\end{remark}
...
\begin{itemize}
\item \textbf{No Anomalous Pairs (NAP)}: The classical condition from ordered semigroup theory~\cite{Holder1901,Alimov1950,Fuchs1963}.
...
This classical result is formalized in Eric Luap's \texttt{OrderedSemigroups} library~\cite{Luap2024} and imported
...
\textbf{Files}:
\begin{itemize}
\item \texttt{Core/Algebra.lean} --- \texttt{SeparationGeneralizedProp} definition
\item \texttt{Additive/Proofs/GridInduction/Comparison.lean} --- proofs
\end{itemize}

Without the assumption that identity is a minimum, elements can exist \emph{below} identity
(mapping to negative reals under $\Theta$). The separation axiom extends naturally to a
\textbf{bilateral} form:

\begin{definition}[\texttt{SeparationGeneralizedProp}]
\textbf{Lines 400--409, Core/Algebra.lean}

Bilateral separation consists of two branches:
\begin{itemize}
\item \textbf{(Positive)} For $\texttt{ident} < a$ and $\texttt{ident} < x < y$: sandwich condition.
\item \textbf{(Negative)} For $a < \texttt{ident}$ and $x < y < \texttt{ident}$: same sandwich condition.
\end{itemize}
The key constraint: $a$, $x$, $y$ must lie on the \emph{same side} of identity.
\end{definition}

\begin{theorem}[\texttt{separationGeneralizedProp\_of\_noAnomalousPairs}]
\textbf{Lines 563--570, Additive/Proofs/GridInduction/Comparison.lean}

\texttt{NoAnomalousPairs} implies \texttt{SeparationGeneralizedProp}.
\end{theorem}

\textbf{Proof idea (negative branch)}:
From NAP, obtain $\Theta : \alpha \to \mathbb{R}$.
For $a, x, y < \texttt{ident}$, we have $\Theta(a), \Theta(x), \Theta(y) < 0$.
Setting $A = -\Theta(a)$, $X = -\Theta(y)$, $Y = -\Theta(x)$ (note the swap!) gives $0 < X < Y$.
By Archimedean property, pick $m$ with $A \leq m(Y - X)$, then $n := \lceil mX/A \rceil$
gives $mX \leq nA < mY$. Translating back yields the sandwich $x^{(m)} < a^{(n)} \leq y^{(m)}$.

\begin{remark}[Probability setting]
Under \texttt{IdentIsMinimum} (the probability setting), no elements exist below identity,
so the negative branch is vacuously true and \texttt{SeparationGeneralizedProp} reduces to
the standard one-sided \texttt{KSSeparation}.
\end{remark}

\begin{remark}[Why NAP is Primary]
The formalization treats \texttt{NoAnomalousPairs} as the primary axiom for three reasons:
\begin{enumerate}
\item \textbf{Historical precedent}: NAP predates K\&S by 60+ years~\cite{Holder1901}.
...
\item \texttt{Additive/Proofs/GridInduction/Core/} ($\sim$17,700 lines: grid infrastructure)
  ...
  \item \texttt{Core/Induction/}: Inductive extension theorems (\texttt{Construction}, \texttt{ThetaPrime},\newline \texttt{DeltaShift})
  \item \texttt{Core/OneDimensional.lean}: Base case (single atom)
  \item \texttt{Core/Prelude.lean}: Foundational lemmas
  \end{itemize}
\end{itemize}

\textbf{Architecture}: \path{Additive/Proofs/GridInduction/Globalization.lean} imports
\path{Additive/Proofs/GridInduction/Core/All.lean} and orchestrates the grid machinery to prove global representation.
...
\begin{enumerate}
\item \textbf{Hölder embedding} (MAIN ROUTE, weakest assumptions): Uses the NoAnomalousPairs condition and
classical ordered semigroup theory~\cite{Holder1901,Alimov1950,Fuchs1963}.
Formalized via Eric Luap's \texttt{OrderedSemigroups}~\cite{Luap2024}.
...
\subsection{Two Logical Steps: Path A and Path B}

Appendix C separates cleanly into two steps.

\textbf{Path A (rigidity / solution)}: assume the functional equation holds globally and add an explicit regularity gate (we use measurability of $H'$). Then the only solutions are logarithmic (see \texttt{variationalEquation\_solution\_measurable}, lines 315--380 in \texttt{Variational/Main.lean}).

\textbf{Path B (derivation / motivation)}: compute the coordinate derivative of a separable product Lagrangian at a stationary point and obtain the separated equation that motivates the functional equation (see \texttt{section PathB} starting at line 625 and \texttt{lagrange\_coordinate\_deriv\_eq} at line 699 in \texttt{Variational/Main.lean}). Upgrading ``holds at a stationary point'' to a global equation is an explicit universality/richness premise (Gate~G).
...
\textbf{Proof strategy}: Transform the multiplicative equation $H'(m_x \cdot m_y) = \lambda(m_x) + \mu(m_y)$ into Cauchy's additive equation $f(u+v) = f(u) + f(v)$ by setting $u = \log m$. Measurable solutions to Cauchy's equation are linear~\cite{Aczel1966}, giving $H'(m) = B + C \log m$.
...
\textbf{Unified view (project-wide, optional)}: The repo also includes axiomatic entropy (Faddeev / Shannon--Khinchin) and bridges to mathlib's measure-theoretic KL; the curated entrypoint \path{Mettapedia/InformationTheory/EntropyKL.lean} imports the unified finite-discrete story and records the main equivalence theorems (see Appendix: Supplementary Derivation Paths).
...
with the convention $0 \cdot \log(0) = 0$ (from continuity: $\lim_{x \to 0^+} x \log x = 0$).\\
Justified by \texttt{zero\_mul\_log\_zero} (line 70).
...
\textbf{Directory}: \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/Additive/Counterexamples/}

A key discovery during formalization: the \textbf{separation property is independent} of the
base K\&S axioms. This was initially unexpected---our early formalization (December 2025)
assumed separation was derivable, matching the K\&S paper's informal presentation.

\begin{theorem}[KSSeparation Independence]
\leavevmode\\
\textbf{File}: \texttt{Counterexamples/KSSeparationNotDerivable.lean}\\
\textbf{Lean name}: \texttt{exists\_knuthskilling\_not\_separation}

There exists a structure satisfying all base K\&S axioms (associativity, strict monotonicity,
identity-as-minimum) that \textbf{fails} the separation property.
\end{theorem}

\subsubsection{The Semidirect Product Countermodel}

\textbf{File}: \texttt{Counterexamples/SemidirectNoSeparation.lean}

\textbf{Construction}:
\begin{align*}
\text{SD} &:= \text{WithBot}(\mathbb{N}^+ \times_{\text{lex}} \mathbb{N}) \\
(u, x) \oplus (v, y) &:= (u + v,\; x + 2^u \cdot y)
\end{align*}

This is a semidirect product: the first component acts on the second via $2^u$ scaling.

\begin{lstlisting}[caption={Semidirect product countermodel (SemidirectNoSeparation.lean)}]
abbrev SD := WithBot (PNat *_[lex] Nat)
...
-- Verified properties:
instance : KnuthSkillingAlgebra SD  -- All base axioms satisfied

-- But:
theorem SD.not_commutative : exists x y : SD, op x y != op y x

theorem SD.not_KSSeparation : ¬ KSSeparation SD
\end{lstlisting}

\textbf{Properties verified}:
\begin{itemize}
\item[\checkmark] Associative (the semidirect product construction preserves associativity)
\item[\checkmark] Strictly monotone in each argument (lexicographic order)
\item[\checkmark] Has identity ($\bot$)
\item[\checkmark] Identity is minimum
\item[$\times$] \textbf{NOT commutative}: $(1,1) \oplus (2,1) \neq (2,1) \oplus (1,1)$
\item[$\times$] \textbf{Fails KSSeparation}: proven in \texttt{not\_KSSeparation}
\end{itemize}

\begin{theorem}[Independence Theorems]
\leavevmode\\
\textbf{File}: \texttt{Counterexamples/KSSeparationNotDerivable.lean}

\begin{lstlisting}
theorem exists_knuthskilling_noncomm :
    exists (alpha : Type) (_ : KnuthSkillingAlgebra alpha),
      exists x y : alpha, op x y != op y x
...
\textbf{Implications}:
\begin{enumerate}
\item Any proof attempt deriving KSSeparation from the base K\&S axioms is \textbf{doomed to fail}.
\item Commutativity cannot be derived from the base axioms alone (it requires separation).
\item Separation (or an equivalent like NAP) must be \textbf{postulated as an independent axiom}.
\end{enumerate}

\begin{remark}[Formalization Exposing Hidden Assumptions]
This is a concrete example of formalization's value. The K\&S paper's presentation suggests
separation ``follows naturally'' from the base axioms. Rigorous formalization revealed this
is false: an explicit countermodel blocks all derivation attempts. See
Appendix~\ref{app:separation-history} for the discovery timeline.
\end{remark}
...
\item \texttt{op\_archimedean\_of\_separation}\\
      \path{Additive/Axioms/SandwichSeparation.lean:134}\\
...
\item \texttt{complement\_rule}\\
      \path{Probability/ProbabilityDerivation.lean:972}\\
...
%==============================================================================
\section*{Appendix: Supplementary Derivation Paths (References)}
\addcontentsline{toc}{section}{Appendix: Supplementary Derivation Paths (References)}
%==============================================================================

These modules are not needed to audit K\&S FOI itself, but they provide additional derivation routes
and equivalence theorems that cross-check the entropy/KL story:

\begin{itemize}
\item \textbf{Unified finite entropy/KL entrypoint}:\\
      \path{Mettapedia/InformationTheory/EntropyKL.lean}
\item \textbf{Axiomatic entropy (Faddeev~\cite{Faddeev1956}, Shannon~\cite{Shannon1948}, Shannon--Khinchin)}:\\
      \path{Mettapedia/InformationTheory/ShannonEntropy/Main.lean}
\item \textbf{Shore--Johnson~\cite{ShoreJohnson1980} $\to$ KL (derivation glue)}:\\
      \path{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnson/KLDerivation.lean}\\
      (full import surface: \path{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnson/Main.lean})
\item \textbf{Shore--Johnson scope note (Uffink~\cite{Uffink1995})}:\\
      {\footnotesize Uffink critiques the claim that Shore--Johnson ``consistency'' alone uniquely singles out KL:
      under weaker formulations (notably weaker forms of system independence), a broader R\'enyi-family of update rules
      is compatible. Our Shore--Johnson development therefore treats \emph{product additivity} of the divergence under
      product distributions as an explicit hypothesis, and derives the KL atom under a standard regularity gate
      (measurability).}
\item \textbf{Shore--Johnson $\leftrightarrow$ Appendix C gate bridge}:\\
      {\footnotesize\path{Mettapedia/ProbabilityTheory/KnuthSkilling/Bridges/ShoreJohnsonVariationalBridge.lean}}
\item \textbf{Cox~\cite{Cox1946} probability calculus derivation}:\\
      \path{Mettapedia/ProbabilityTheory/Cox.lean}
\item \textbf{Algorithmic probability (Solomonoff~\cite{Solomonoff1964}) $\leftrightarrow$ K\&S KL gate}:\\
      {\footnotesize\path{Mettapedia/ProbabilityTheory/KnuthSkilling/Bridges/AlgorithmicProbabilityBridge.lean}}
\end{itemize}
...
\subsection*{Phase 3: Ben Goertzel's Contributions (Dec 19--20, 2025)}
...
\subsection*{Phase 4: The Counterexample (Jan 8, 2026)}

The breakthrough: rather than continuing proof attempts, we constructed a \textbf{countermodel}.
The semidirect product $\text{SD} = \text{WithBot}(\mathbb{N}^+ \times_{\text{lex}} \mathbb{N})$
...
\item \textbf{Consolidation}: Formalized Jan 13, 2026, combining both contributions
\end{itemize}
