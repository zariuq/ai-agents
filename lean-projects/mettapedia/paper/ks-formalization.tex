...
A key finding: the \textbf{NoAnomalousPairs} condition (from classical ordered semigroup theory) is the weakest assumption sufficient for the representation theorem, via Eric Luap's formalized H\"older embedding.
\end{abstract}
...
\item \textbf{Three independent proof paths} for the representation theorem:
  ...
  \item H\"older embedding via Eric Luap's OrderedSemigroups library
  \end{enumerate}
\item Explicit treatment of implicit assumptions (linear order, separation property)
\item Discovery of the \textbf{NoAnomalousPairs} condition as the weakest sufficient assumption
...
\subsubsection{Approach 3: H\"older Embedding (Weakest Assumptions)}

\textbf{Assumptions}: \texttt{NoAnomalousPairs} only!

\textbf{Technique}: Leverage Eric Luap's formalized H\"older theorem for ordered semigroups:
\begin{enumerate}
...
\textbf{Pros}: \textbf{Weakest assumptions}; shortest local code; most elegant \\
\textbf{Cons}: Depends on external library

\begin{lstlisting}[caption={H\"older-based representation theorem (weakest assumptions)}]
...
Assumptions & Medium & Medium+ & \textbf{Weakest} \\
...
\item It matches what K\&S's convolution ``blurring'' argument implicitly requires
\end{enumerate}
...
This is formalized in \texttt{Divergence.lean} with proofs of non-negativity, zero iff equal, and asymmetry.
...
\begin{enumerate}
\item \textbf{Three independent proof paths}: Grid-based (K\&S style), Dedekind cuts (classical), and H\"older embedding (via Eric Luap's library).
...
Eric Luap's Lean 4 formalization of this classical theory provides a clean, minimal proof path.
...
\begin{itemize}
\item Uses the weakest assumptions (\texttt{NoAnomalousPairs} only)
\item Shortest local codebase ($\sim$420 lines vs.\ $\sim$4000 for grid approach)
\item Most mathematically elegant
\item Leverages Eric Luap's battle-tested library
\end{itemize}
...
\begin{itemize}
\item \textbf{NoAnomalousPairs} (weakest, used by H\"older approach)
...
\begin{itemize}
\item Commutativity (via Eric Luap's \texttt{not\_anomalous\_pair\_commutative})
...
\textbf{Key finding}: \texttt{NoAnomalousPairs} is sufficient and is the \emph{weakest} known condition for the representation theorem.
...
Fuchs (1963) & No anomalous pairs $\Rightarrow$ commutative & Classical text \\
\textbf{Luap (2024)} & \textbf{Lean 4 formalization of the above} & \textbf{OrderedSemigroups} \\
K\&S + Separation & Ordered monoids with separation & Embed in $(\mathbb{R}, +)$ \\
\end{tabular}
\end{center}

\textbf{Key insight}: Eric Luap's OrderedSemigroups library (github.com/ericluap/OrderedSemigroups) provides a \textbf{complete Lean 4 formalization} of the Alimov-H\"older embedding theorem:

\begin{lstlisting}[caption={Eric Luap's H\"older theorem (from OrderedSemigroups)}]
...
\item \texttt{Pow ℕ+}: $x^n := \text{iterate\_op}\, x\, n$
