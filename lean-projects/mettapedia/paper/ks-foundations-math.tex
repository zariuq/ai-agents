...
\begin{table}[h]
\centering
\begin{tabular}{|p{3.0cm}|p{3.0cm}|p{4.2cm}|p{4.6cm}|}
\hline
\textbf{Route} & \textbf{Gate~G (Globality)} & \textbf{Gate~R (Regularity)} & \textbf{Forced normal form} \\
\hline
K\&S Appendix C (Path B $\to$ Path A) &
Universality / richness premise upgrades stationarity to a global equation &
Measurability of $H'$ (e.g.\ $H'=\mathrm{deriv}\,H$) excludes Hamel solutions &
$H'(m)=B+C\log m$ and thus $H(m)=A+Bm+C(m\log m - m)$ \\
\hline
Shore--Johnson (Path C) &
System-independence tests extract a multiplicative Cauchy equation for $g(q)=d(1,q)$ &
Measurability/continuity/monotonicity of $g$ yields $g(q)=C\log q$ &
$d(p,q)=C\,p\log(p/q)$ (KL atom) and hence KL divergence \\
\hline
Faddeev / Shannon--Khinchin (axioms on finite distributions) &
N/A (global axioms) &
Continuity / expansibility-type axioms exclude exotic solutions &
$H(p)=-K\sum_i p_i \log p_i$ (up to constant/log base) \\
\hline
\end{tabular}
\caption{Entropy/KL derivations share the same two logical gates: a globality premise (Gate~G) and a regularity gate (Gate~R).}
\end{table}
...
Algorithmic probability (Solomonoff induction) is complementary in a different sense: it does not
aim to derive the probability calculus, but rather to provide a canonical \emph{assignment} of prior
weight by summing $2^{-|p|}$ over programs whose output has a given prefix.  The resulting universal
object is naturally a \emph{semimeasure} on prefixes, satisfying $M(x) \ge M(x0)+M(x1)$; the ``missing
mass'' corresponds to non-continuation/halting after $x$.  Nonetheless it induces honest predictive
distributions (either by conditioning on continuation or by adjoining a third ``halt'' outcome), and
its universality/dominance theorem under standard computability hypotheses yields convergence and regret
bounds in log-loss, placing
Kullback--Leibler divergence at the shared information-theoretic ``gate'' between algebraic
structure and algorithmic assignment. \cite{Solomonoff1964,Hutter2005}
...
\hline
Ordered scale $(S,\oplus)$ & \texttt{KSSemigroupBase} & \texttt{Core/Basic.lean} \\
\hline
$+$ identity as minimum & \texttt{KnuthSkillingAlgebraBase} & \texttt{Core/Basic.lean} \\
\hline
Separation axiom & \texttt{KSSeparation} & \texttt{Core/Algebra.lean} \\
\hline
No anomalous pairs & \texttt{NoAnomalousPairs} & \texttt{Additive/Axioms/AnomalousPairs.lean} \\
\hline
H\"older embedding & \texttt{holder\_embedding} & \texttt{Additive/Proofs/.../HolderEmbedding.lean} \\
\hline
$\oplus$, $\otimes$, $\star$ & \texttt{op}, \texttt{tensor}, \texttt{condOp} & \texttt{Core/Basic.lean} \\
\hline
$x^{(n)}$ (iteration) & \texttt{iterate\_op\_pnat} & \texttt{Core/Basic.lean} \\
\hline
Additive representation & \texttt{representation\_theorem} & \texttt{Additive/Main.lean} \\
\hline
Product representation & \texttt{product\_representation} & \texttt{Multiplicative/Main.lean} \\
\hline
Variational theorem & \texttt{variational\_classification} & \texttt{Variational/Main.lean} \\
\hline
KL divergence & \texttt{klDiv} & \texttt{Information/DivergenceMathlib.lean} \\
\hline
Regraduation & order isomorphism & various \\
\hline
Sigma-complete events & \texttt{SigmaCompleteEvents} & \texttt{Core/ScaleCompleteness.lean} \\
\hline
Scale completeness & \texttt{KSScaleComplete} & \texttt{Core/ScaleCompleteness.lean} \\
\hline
Scott continuity & \texttt{KSScottContinuous} & \texttt{Core/ScaleCompleteness.lean} \\
\hline
$\sigma$-additivity theorem & \texttt{ks\_sigma\_additive} & \texttt{Core/ScaleCompleteness.lean} \\
\hline
$\oplus \to +$ (derived) & \texttt{op\_is\_addition\_via\_Theta} & \texttt{Additive/Axioms/OpIsAddition.lean} \\
\hline
Mathlib bridge & \texttt{toProbabilityMeasure} & \texttt{Bridges/MathlibProbability.lean} \\
\hline
\end{tabular}
\end{center}
