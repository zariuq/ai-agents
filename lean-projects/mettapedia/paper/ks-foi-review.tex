...
scale} (see \texttt{NoFiniteModel.lean}). The event lattice can be infinite; one simply
...
\midrule
\endhead
Appendix A & \texttt{Separation/HolderEmbedding.lean} & \texttt{holder\_embedding} \\
Appendix A & \texttt{AppendixA/Main.lean} & \texttt{appendixA\_representation} \\
Appendix B & \texttt{ProductTheorem/ScaledMultRep.lean} & \texttt{psi\_is\_exponential} \\
Appendix C & \texttt{VariationalTheorem.lean} & \texttt{entropy\_form\_solution} \\
Section 7 & \texttt{ConditionalProbability/Basic.lean} & \texttt{chainProductRule} \\
Section 7 & \texttt{ConditionalProbability/Basic.lean} & \texttt{bayesTheorem} \\
Section 8 & \texttt{InformationEntropy.lean} & \texttt{klDivergence} \\
\bottomrule
\end{longtable}
