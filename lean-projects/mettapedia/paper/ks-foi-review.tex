...
Our Lean~4 development is not a mere endorsement of the paper: it turns the informal
derivations into precise \emph{conditional theorems}, and it forces an explicit accounting of
which hypotheses are doing which work.
Several key assumptions are implicit (or justified only informally) in K\&S's exposition; without
making them explicit, some of the paper's generality claims are simply not correct.

In particular:
\begin{itemize}[noitemsep]
\item The Appendix~A representation theorem requires an explicit density/Archimedean-style
assumption (separation / no anomalous pairs), and it is \emph{not} derivable from the base axioms:
we give a concrete countermodel satisfying the base scale axioms but failing separation.
\item Positivity and normalization require a bounded-below normalization hypothesis (a bottom element);
without it, a valid additive representation can take negative values.
\item The Appendix~C variational route requires an explicit regularity gate (e.g.\ measurability) to rule
out Cauchy-pathology solutions; our formalization bypasses K\&S's ``blurring'' justification and
instead assumes such a gate explicitly.
\end{itemize}

With these hypotheses clearly separated, the formalization provides machine-checked versions of the
sum rule, product rule, and the entropy/KL normal form, and it localizes exactly where each
assumption is used.
...
\textbf{Formalization finding}: Appendix~A repeatedly appeals to trichotomy in the separation
arguments, sandwich constructions, and induction steps.  To make these steps fully precise, one
must treat the value scale as \emph{linearly ordered}.
...
\begin{lstlisting}[language=lean,caption={Machine-checked statement of the integer counterexample.}]
theorem Int.holder_embedding_has_negatives :
    Exists (fun (G : Subsemigroup (Multiplicative Real)) =>
      Exists (fun (Theta : OrderMonoidIso (Multiplicative Int) G) =>
        Multiplicative.toAdd (Theta (Multiplicative.ofAdd (-1)) : Multiplicative Real) < 0)) :=
...
\textbf{Countable additivity (scope and extension).}
K\&S’s paper focuses on finitely generated lattices and derives \emph{finite} additivity.
Our formal development also includes a clean extension to $\sigma$-additivity:
by adding explicit completeness/continuity axioms on the event structure and value scale
($\sigma$-complete events, sequential completeness of the scale, and Scott continuity of the valuation),
the induced real-valued measure becomes countably additive on pairwise-disjoint unions.
Counterexamples show these extension axioms are genuinely needed and are not consequences of the finitary symmetries alone.

\textbf{Note on finiteness:} while the event lattice may be finite or infinite, any nontrivial model has an
\textbf{infinite value scale} (the set of possible plausibility values cannot be finite).

\subsubsection{K\&S and the \texorpdfstring{$\sigma$}{sigma}-Additivity Question}
\label{sec:sigma-additivity}

K\&S (like Cox) derives \emph{finite} additivity from structural symmetries.
...
\end{enumerate}
Under these hypotheses, $\sigma$-additivity becomes a theorem, and the finite theory is recovered as a special case.
...
K\&S’s framework is best read as an algebraic explanation of the finite probability calculus:
it isolates symmetries that force additivity, multiplication of independent products, and (under a
regularity gate) the logarithmic information measures.  The central lesson of formalization is that
these results are correct \emph{as conditional theorems}, but their scope depends on making the
requisite density/boundedness/regularity hypotheses explicit.

%=============================================================================
\section{Counterexamples Clarifying Scope}
%=============================================================================

The most useful byproducts of formalization are concrete counterexamples that pinpoint exactly what
