...
-- ═══════════════════════════════════════════════════════════════════════════
-- § Memoization Correctness Theorems
-- ═══════════════════════════════════════════════════════════════════════════
-- Key theorem: evalMemo computes the same result as eval.
-- Memoization is a pure optimization — it does not change observable behavior.

/-- A memo table is sound if every cached entry agrees with eval. -/
def MemoSound (rules : List EvalRule) (fuel : Nat) (memo : MemoTable) : Prop :=
  ∀ h vs v, (h, vs, v) ∈ memo →
    eval rules fuel (.userCall h (vs.map EvalValue.toNode)) = some v

/-- The agreement theorem: evalMemo produces the same result as eval,
    and maintains memo soundness.
    Proof proceeds by well-founded induction on (fuel, sizeOf node). -/
theorem evalMemo_correct (rules : List EvalRule) (fuel : Nat) (memo : MemoTable)
    (node : EvalNode) (hm : MemoSound rules fuel memo) :
    (evalMemo rules fuel memo node).1 = eval rules fuel node ∧
    MemoSound rules fuel (evalMemo rules fuel memo node).2 := by
  sorry -- TODO: mutual induction on (fuel, sizeOf node)

/-- List version of the agreement theorem. -/
theorem evalMemoList_correct (rules : List EvalRule) (fuel : Nat) (memo : MemoTable)
    (nodes : List EvalNode) (hm : MemoSound rules fuel memo) :
    (evalMemoList rules fuel memo nodes).1 = evalList rules fuel nodes ∧
    MemoSound rules fuel (evalMemoList rules fuel memo nodes).2 := by
  sorry -- TODO: mutual induction on (fuel, sizeOf nodes)

/-- The public corollary: with an empty memo table, evalMemo agrees with eval. -/
theorem evalMemo_agrees (rules : List EvalRule) (fuel : Nat) (node : EvalNode) :
    (evalMemo rules fuel [] node).1 = eval rules fuel node :=
  (evalMemo_correct rules fuel [] node
    (fun _ _ _ h => by simp at h)).1
...
mutual
  inductive EvalTrace (rules : List EvalRule)
    ...
    | callMiss :
        ...
        rule ∈ rules → rule.head = head → rule.params.length = args.length →
        EvalTrace rules
...
-- Memo soundness: the output memo of a trace contains only correct entries
theorem trace_memo_sound (rules : List EvalRule) (node : EvalNode) (v : EvalValue)
    (m m' : MemoTable) (t : EvalTrace rules node v m m')
    (hm : MemoSound rules 0 m) :  -- TODO: fuel-independent MemoSound
    ∀ h vs val, (h, vs, val) ∈ m' →
      ∃ fuel, eval rules fuel (.userCall h (vs.map EvalValue.toNode)) = some val := by
  sorry -- induction on t
