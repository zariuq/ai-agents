...
The verification claims in this note are snapshot-scoped; for current repository
status, consult the project claim trackers and build targets.
Key results include:
...
\paragraph{Mirror note.} This dated file mirrors \texttt{AIrxiv/spice-calculus-formalization.tex} for multi-session traceability.
\end{abstract}
...
\begin{itemize}[nosep]
  \item \textbf{Mechanized and machine-checked}: theorem statements are proved in Lean
    for the reported snapshot.
...
Compilation/build status should be read as snapshot-scoped for this draft.
...
We have presented a mechanized formalization of the $\rho$-calculus
...
All verification/build status claims in this note are snapshot-scoped.
