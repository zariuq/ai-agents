...
\end{itemize}
Additional infrastructure proved in subsequent work (0 \texttt{sorry}):
\begin{itemize}[leftmargin=2em]
\item \lean{rawSwap\_selfInverse}: the per-element swap is self-inverse
  (constructs output visit-time proofs internally)
\item \lean{segmentSwap\_carrier\_mem\_adjacent\_reverse}: reverse
  carrier membership (carrier${}_{n+1} \to$ carrier${}_n$)
\item \lean{nthVisitTimeExists\_of\_succ}: visit-time monotonicity
\item \lean{extractVisitTime\_segmentSwap\_n/n1/n2}: visit-time
  stability under swap
\end{itemize}
