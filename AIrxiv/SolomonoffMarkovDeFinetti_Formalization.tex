...
\begin{tabular}{llll}
\hline
\textbf{Sub-gap} & \textbf{Nature} & \textbf{Difficulty} & \textbf{Resources} \\
\hline
Extension with PE (Bridge A) & Infinite-product measure theory & Hard & --- \\
Carrier transport bijection & Finite combinatorics & Medium & Archived Euler trail code \\
Joint de Finetti extraction & Measure-theoretic independence & Medium--Hard & PE infrastructure exists \\
\hline
\end{tabular}
\end{center}

The carrier transport bijection remains the most tractable attack point.
The archived Euler trail / copy-permutation code (1{,}225 lines, 0
\texttt{sorry}) may provide the combinatorial core.

The joint de Finetti extraction (establishing cross-row conditional
independence from PE) is needed for the induction step of the
cross-anchor product identity.  If resolved, it would also collapse
the ``\lean{hInvFromPE}'' sub-gap into the carrier transport route.

The extension with PE (Bridge A) is the hardest piece: it requires
either the Kolmogorov extension theorem or a direct lifting argument
from finite-dimensional Markov exchangeability to infinite-product PE.
