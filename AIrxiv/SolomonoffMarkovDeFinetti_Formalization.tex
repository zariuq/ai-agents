...
measure---is fully proved (zero \texttt{sorry}).  The \emph{literature-facing}
theorem, which derives the row-kernel data from bare Markov exchangeability
and recurrence, requires three sub-obligations that remain open: (1)~constructing
an extension with successor-matrix partial exchangeability, (2)~a finite
carrier-transport bijection for start-restricted row-kernel factorization,
and (3)~extracting cross-row conditional independence from the joint de Finetti
theorem.  A structural audit (April 2026) revealed that prior descriptions
understated the gap by omitting item~(3).

This note presents the mathematical content, the Lean definitions and theorems
(with verbatim listings), and a precise description of the gap, intended
for mathematicians who may wish to contribute to closing it.

The formalization comprises approximately 50{,}000 lines of Lean across
10 active files, with an additional 43{,}000 lines formalizing the classical
...
\begin{openproblem}[Carrier Transport Bijection]
\label{prob:carrier}
Construct, for each pair of visit indices $n, n'$ and each horizon $N$,
an explicit bijection between the corresponding prefix carriers that
preserves Markov evidence.

This is a \emph{finite combinatorial} problem: given two finite sets of
trajectories (of length $N+1$ over $\Fin(k)$) defined by visit-time
conditions, exhibit a bijection that preserves the transition-count matrix.

The key technique is \emph{trajectory segment swapping}: two trajectories
that differ only in which visit to state $i$ produces successor $b$
have the same transition counts (the transition $i \to b$ is counted once
in both cases).  The bijection permutes trajectory segments between the
$n$-th and $n'$-th visits to $i$.

Mathlib provides \lean{Finset.card\_bij} for cardinality arguments and
\lean{Nat.multinomial} for counting trajectories with prescribed
transition counts.  The project includes a
\lean{FortiniFiniteCheckers} module with decidable
evidence-preservation checkers that could automate small cases.
\end{openproblem}
...
\item \textbf{Euler trail approach} (\lean{EulerTrails}, \lean{EulerTrailFiber},
  ...
  \end{itemize}
  \textbf{This is closely related to the carrier transport bijection}
  (Open Problem~\ref{prob:carrier}).  The copy-permutation idea
  amounts to swapping which visit to a state produces a given
  successor---exactly the segment-swapping bijection described there.
  Bridging the archived Euler-trail infrastructure to the current
  carrier framework may be the most efficient route to closing the
  combinatorial gap.
...
\hline
Carrier transport & Finite combinatorics & Medium & \textbf{903/${\sim}$ 1100 lines} & Ported from archive \\
...
\paragraph{Carrier transport (partially closed, April 2026).}
The archived excursion-swap code (\lean{segmentSwap} from
\lean{ExcursionBridge.lean}) was ported to two new active files:
\lean{MarkovDeFinettiCarrierTransportCore.lean} (319 lines) and
\lean{MarkovDeFinettiCarrierTransport.lean} (584 lines), both 0
\texttt{sorry}.  The mathematical content is proved:
\begin{itemize}[leftmargin=2em]
\item \lean{segmentSwap\_evidenceOf}: segment swap preserves Markov evidence
\item \lean{visitCountBefore\_segmentSwap\_at\_midpoint}: visit count
  at the new excursion boundary equals $n+1$
\item \lean{segmentSwap\_carrier\_mem\_adjacent}: swapped trajectory
  lands in the target carrier
\item \lean{carrierTransport\_forward\_adjacent}: complete forward map
  with evidence preservation
\end{itemize}
What remains ($\sim$200 lines) is the \texttt{Equiv} packaging: constructing
the per-element bijection from the forward map + involutivity of
\lean{segmentSwap}, then composing adjacent transports for general $n \to n'$.
The obstruction is Lean bookkeeping around \lean{extractVisitTime}
(\lean{Nat.find} under \lean{classical}) and \lean{prefixExtend},
...
\node[gap, right=2.5cm of counts] (bridge) {Bridge A\\+ carrier transport\\+ joint de Finetti};
\node[proved, right=of bridge] (internal) {Internal\\representation};
...
\draw[arr, dashed, gapred] (counts) -- node[above, font=\scriptsize\itshape]
  {open} (bridge);
...
Closing the three sub-obligations (Bridge A, carrier transport, and
joint de Finetti extraction) would complete the chain from Markov
exchangeability to a mixture representation with Solomonoff-style
dominance guarantees.
...
Carrier bridge & \texttt{MarkovDeFinettiCarrierTransport.lean} & 584 & {\color{provedgreen}0 sorry} \\
...
representation theorem \emph{only under the two bridge assumptions}
stated as hypotheses.  No file contains a sorry; the gap manifests as
hypotheses on theorems, not as incomplete proofs.
