...
\large (Lean/Mettapedia technical summary; archived failed-attempt record)}
...
\begin{abstract}
This note is archived as a record of a formalization direction that was not completed
to a final accepted proof package.
...
\item An attempted Lean formalization of a \emph{Markov de Finetti} / \emph{Diaconis--Freedman} theorem on finite state spaces: Markov-exchangeable prefix measures satisfying a recurrence hypothesis are mixtures of Markov chains.
\end{enumerate}

The purpose of the note is didactic: it records the core mathematical ideas,
the engineering decomposition used in Lean, and the blocker that caused archival
before completion of the hard-direction proof.
...
\paragraph{Mirror note.} This dated file mirrors \texttt{AIrxiv/SolomonoffMarkovDeFinetti\_Formalization.tex} for multi-session traceability.
\end{abstract}
...
Solomonoff induction is usually presented as a universal Bayesian mixture over computable hypotheses (Solomonoff 1964).
...
For i.i.d.\ sequences, de Finetti gives exchangeability $\Rightarrow$ mixture of i.i.d.\ laws (de Finetti 1931/1937).
For Markov sequences, Diaconis--Freedman show a corresponding representation for Markov-exchangeability (partial exchangeability of trajectories), with an additional structural recurrence hypothesis (Diaconis \& Freedman 1980).

Once such structure theorems are formalized, they can be used to justify \emph{structured universal mixtures}:
Solomonoff-like prediction can be instantiated by mixing over the parametric space of Markov models, with hyperpriors that retain dominance-style guarantees in the restricted model class.
