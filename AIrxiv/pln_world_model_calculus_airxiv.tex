...
\paragraph{Proof-scope note.} In this draft, Lean-proven statements are those mapped to explicit modules in \S10 (e.g., WM kernel/no-go/BN compilation artifacts). Other sections are research synthesis, design rationale, or roadmap.
\end{abstract}
...
when it is violated, and how do you quantify drift~\cite{Pearl}?
...
and regraduation principles~\cite{KnuthSkilling},
Kohlas--Shenoy give algebraic local computation~\cite{KohlasShenoy},
Tao-style methodology emphasizes hypotheses stated cleanly,
...
\footnote{For the concrete PathMap/\texttt{Finset}$\,\alpha$ instance, this law is now Lean-proven as
\texttt{pjoin\_evidence\_additive} in
\texttt{Mettapedia/OSLF/PathMap/PLNBridge.lean}: for disjoint stores $W_1 \perp W_2$,
$\ev(W_1 \cup W_2, q) = \ev(W_1,q) \oplus \ev(W_2,q)$, where
$\ev(W,q) = (|W \cap q|,\, |W \setminus q|) \in \mathbb{R}_{\geq 0}^\infty \times \mathbb{R}_{\geq 0}^\infty$
is the K\&S-grounded evidence extraction function.
The proof also establishes: K\&S sum rule ($n^+ + n^- = |W|$), monotonicity, the Bayesian conditioning
partition ($\ev(W,q) = \ev(W\cap\mathit{ctx},q) \oplus \ev(W\setminus\mathit{ctx},q)$), and the
STV connection ($\mathrm{strength} = |W\cap q|/|W|$). Axioms: standard Lean kernel only.}
...
(see the no-go artifact listed in \S10):
...
and compile them into admissible rewrite rules~\cite{Pearl}.
...
(Classical VE background:~\cite{Dechter}.)
...
Knuth--Skilling (K\&S) ``foundations of inference'' axiomatize valuation schemes on lattices~\cite{KnuthSkilling}.
...
\[
  \chi_P : U \to \Evidence,
\]
analogous to a subobject classifier in a topos~\cite{MacLaneMoerdijk}.
...
Kyburg argues that ``second-order probabilities'' add no conceptual power~\cite{Kyburg}:
...
This can yield completeness metatheorems analogous in spirit to first-order settings,
and matches ``representable concept'' quantification under explicit model assumptions.
...
\item \texttt{Mettapedia/Logic/PLNWorldModelITV.lean}: explicit ITV semantics (Bayes-normal, Bayes-exact, Walley-IDM).
\item \texttt{Mettapedia/Logic/PLNWorldModelITVHypercube.lean}: interval-selector layer and typed ITV transport.
\item \texttt{Mettapedia/Logic/PLNWMOSLFBridgeITVTyped.lean}: typed OSLF ITV bridge + selector-specialized quantale coherence theorems.
\item \texttt{Mettapedia/Logic/PLNXiDerivedBNRulesTyped.lean}: typed BN constructor lifts (including native tagged/sort-tagged families).
\item \texttt{Mettapedia/Logic/PLNCanonicalAPI.lean}: one-call end-to-end bundle/finalBundle endpoints.
\item \texttt{Mettapedia/OSLF/Framework/QuantaleCoherence.lean}: quantale transport/coherence infrastructure used by one-call endpoints.
...
\item \texttt{Mettapedia/OSLF/PathMap/PLNBridge.lean}: PathMap $\leftrightarrow$ PLN evidence bridge.
  Proves $\ev(W_1\cup W_2,q)=\ev(W_1,q)\oplus\ev(W_2,q)$ for disjoint \texttt{Finset}\,$\alpha$ stores
  (the WM kernel additivity spine), K\&S sum rule, Bayesian conditioning partition,
  STV strength connection, and registers the \texttt{PathMapWorldModel} typeclass instance.
  Zero axioms beyond standard Lean kernel (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}).
\end{itemize}

\section{Roadmap: ``final steps'' that actually matter}

Current status update: the Lean stack now includes explicit Bayes-vs-Walley ITV semantics,
typed WM/OSLF ITV bridges, and one-call composed endpoints that chain selector transport,
rewrite/query transport, and quantale coherence for chain/fork/collider BN paths.
On the Walley side, the interval branch follows IDM predictive semantics in the
``bag of marbles'' lineage~\cite{Walley1996BagOfMarbles}; on the Bayesian side,
the interval branch uses Beta-posterior credible-interval semantics, with robust
multinomial design links to MLE-dominance analyses~\cite{BenavoliDeCampos2009MLEDominance}.
...
\bibitem{Walley1996BagOfMarbles}
P.\ Walley.
\newblock Inferences from multinomial data: Learning about a bag of marbles.
\newblock \emph{Journal of the Royal Statistical Society: Series B (Methodological)},
58(1):3--34, 1996. (With discussion pages 34--57.)

\bibitem{BenavoliDeCampos2009MLEDominance}
A.\ Benavoli and C.\ P.\ de Campos.
\newblock Inference from multinomial data based on a MLE-dominance criterion.
\newblock In \emph{ECSQARU 2009}, LNCS 5590, pages 22--33. Springer, 2009.
